...
final class VectorOps[A](val self: Vector[A]) extends AnyVal {
  ...
  final def takeUntilM[M[_]: Monad](p: A => M[Boolean]): M[Vector[A]] =
    v.takeUntilM(self)(p)

  final def filterM[M[_]: Applicative](p: A => M[Boolean]): M[Vector[A]] =
    v.filterM(self)(p)
  ...
  final def mapAccumLeft[B, C](c: C, f: (C, A) => (C, B)): (C, Vector[B]) =
    v.mapAccumLeft(self)(c, f)

  final def mapAccumRight[B, C](c: C, f: (C, A) => (C, B)): (C, Vector[B]) =
    v.mapAccumRight(self)(c, f)
