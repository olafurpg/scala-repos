...
sealed abstract class FingerTree[V, A](implicit measurer: Reducer[A, V]) {
  ...
  def foldMap[B](f: A => B)(implicit s: Monoid[B]): B =
    fold(v => s.zero,
         (v, x) => f(x),
         (v, pr, m, sf) =>
           s.append(s.append(pr.foldMap(f), m.foldMap(x => x.foldMap(f))),
                    sf.foldMap(f)))
  ...
  def |-:(a: => A): FingerTree[V, A] = {
    lazy val az = a
    fold(v => sys.error("Replacing first element of an empty FingerTree"),
         (v, b) => single(az),
  ...
  def :-|(a: => A): FingerTree[V, A] = {
    lazy val az = a
    fold(v => sys.error("Replacing last element of an empty FingerTree"),
         (v, b) => single(az),
    ...
    fold(
      ...
      (v1, pr1, m1, sf1) =>
        rightz.fold(
          ...
          (v, x) => this :+ n :+ x,
          (v2, pr2, m2, sf2) =>
            deep(measurer.append((measurer.snoc(v1, n)), v2),
                 pr1,
  ...
  private def split1(pred: V => Boolean,
  ...
  def traverseTree[F[_], V2, B](f: A => F[B])(
    ...
    fold(
      ...
      (v, pr, m, sf) => {
        ...
        val fmap2 = F.apply2(traverseFinger(pr)(f),
                             m.traverseTree(n => traverseNode(n)(f)))((a, b) =>
          mkDeep(a)(b) _)
  ...
  private def traverseFinger[F[_], A, B, V2](digit: Finger[V, A])(
      f: A => F[B])(implicit ms: Reducer[B, V2],
...
object FingerTree extends FingerTreeInstances {
  ...
  def deepR[V, A](pr: Finger[V, A],
                  m: => FingerTree[V, Node[V, A]],
                  msf: Option[Finger[V, A]])(
      implicit ms: Reducer[A, V]): FingerTree[V, A] =
    msf match {
      case None => rotR(pr, m)
