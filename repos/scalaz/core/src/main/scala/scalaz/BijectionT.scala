...
final class BijectionT[F[_], G[_], A, B] private[scalaz](
  ...
  def bimap[C, X[_, _], D](
      g: Bijection[C, D])(implicit F: Bifunctor[X],
                          evF: F[B] =:= Id[B],
  ...
  def ***[C, D](
      g: Bijection[C, D])(implicit evF: F[B] =:= Id[B],
  ...
  def andThen[C](that: BijectionT[F, G, B, C])(
      implicit M: Bind[F], GM: Bind[G]): BijectionT[F, G, A, C] =
    that compose this
...
object BijectionT extends BijectionTInstances {
  ...
  def curryB[A, B, C]: Bijection[(A, B) => C, A => B => C] =
    bijection[Id, Id, (A, B) => C, A => B => C](
        _.curried, Function.uncurried(_))
...
private trait BijectionTSplit[F[_], G[_]]
  ...
  def compose[A, B, C](bc: BijectionT[F, G, B, C],
