...
sealed trait TrampolineInstances {
  implicit val trampolineInstance: Monad[Trampoline] with Comonad[Trampoline] with BindRec[
      Trampoline] = new Monad[Trampoline] with Comonad[Trampoline]
  with BindRec[Trampoline] {
...
private sealed trait FreeFoldable[F[_]] extends Foldable[Free[F, ?]] {
  ...
  override final def foldRight[A, B](fa: Free[F, A],
                                     z: => B)(f: (A, => B) => B): B =
    fa.resume match {
...
private sealed trait FreeFoldable1[F[_]] extends Foldable1[Free[F, ?]] {
  ...
  override final def foldMapRight1[A, B](fa: Free[F, A])(
      z: A => B)(f: (A, => B) => B): B =
    fa.resume match {
      case -\/(s) =>
  ...
  override final def foldMapLeft1[A, B](fa: Free[F, A])(
      z: A => B)(f: (B, A) => B): B =
    fa.resume match {
      case -\/(s) =>
...
private sealed abstract class FreeTraverse1[F[_]]
  ...
  override final def traverse1Impl[G[_], A, B](
      fa: Free[F, A])(f: A => G[B])(implicit G: Apply[G]): G[Free[F, B]] =
    fa.resume match {
      case -\/(s) =>
