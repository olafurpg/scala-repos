...
private trait CompositionApply[F[_], G[_]]
    extends Apply[λ[α => F[G[α]]]] with CompositionFunctor[F, G] {
...
private trait CompositionApplicative[F[_], G[_]]
    extends Applicative[λ[α => F[G[α]]]] with CompositionApply[F, G] {
...
private trait CompositionPlusEmpty[F[_], G[_]]
    extends PlusEmpty[λ[α => F[G[α]]]] with CompositionPlus[F, G] {
...
private trait CompositionApplicativePlus[F[_], G[_]]
    extends ApplicativePlus[λ[α => F[G[α]]]] with CompositionApplicative[F, G]
...
private trait CompositionFoldable[F[_], G[_]]
  ...
  override def foldMap[A, B](fa: F[G[A]])(f: A => B)(
...
private trait CompositionTraverse[F[_], G[_]]
    extends Traverse[λ[α => F[G[α]]]] with CompositionFunctor[F, G]
...
private trait CompositionFoldable1[F[_], G[_]]
    extends Foldable1[λ[α => F[G[α]]]] with CompositionFoldable[F, G] {
  ...
  override final def foldMapLeft1[A, B](fa: F[G[A]])(
      z: A => B)(f: (B, A) => B) =
...
private trait CompositionTraverse1[F[_], G[_]]
    extends Traverse1[λ[α => F[G[α]]]] with CompositionFoldable1[F, G] {
...
private trait CompositionDistributive[F[_], G[_]]
    extends Distributive[λ[α => F[G[α]]]] with CompositionFunctor[F, G] {
...
private trait CompositionBifoldable[F[_, _], G[_, _]]
  ...
  override def bifoldRight[A, B, C](fa: F[G[A, B], G[A, B]], z: => C)(
      f: (A, => C) => C)(g: (B, => C) => C): C =
    F.bifoldRight(fa, z)((a, b) => G.bifoldRight(a, b)(f)(g))((a,
        b) => G.bifoldRight(a, b)(f)(g))
  override def bifoldLeft[A, B, C](
      fa: F[G[A, B], G[A, B]], z: C)(f: (C, A) => C)(g: (C, B) => C): C =
    F.bifoldLeft(fa, z)((b, a) => G.bifoldLeft(a, b)(f)(g))((b,
        a) => G.bifoldLeft(a, b)(f)(g))
}
...
private trait CompositionFoldableBifoldable[F[_], G[_, _]]
  ...
  override def bifoldRight[A, B, C](fa: F[G[A, B]], z: => C)(
      f: (A, => C) => C)(g: (B, => C) => C) =
...
private trait CompositionBifoldableFoldables[F[_, _], G[_], H[_]]
  ...
  override def bifoldRight[A, B, C](
      fgahb: F[G[A], H[B]], z: => C)(f: (A, => C) => C)(g: (B, => C) => C) =
    F.bifoldRight(fgahb, z)((ga, c) => G.foldRight(ga, c)(f))(
        (hb, c) => H.foldRight(hb, c)(g))
  override def bifoldLeft[A, B, C](fgahb: F[G[A], H[B]], z: C)(f: (C, A) => C)(
      g: (C, B) => C) =
    F.bifoldLeft(fgahb, z)((c, ga) => G.foldLeft(ga, c)(f))(
