...
import com.twitter.algebird.{
  InclusiveUpper, Intersection, Interval, ExclusiveUpper
}
import com.twitter.summingbird.batch.{
  Batcher, BatchID, PrepareState, RunningState, Timestamp, WaitingState
}
  ...
  private class VersionedPrepareState
    ...
    lazy val requested: Interval[Timestamp] = {
      val beginning: BatchID =
        startDate.map(batcher.batchOf(_)).orElse(newestCompleted).getOrElse {
