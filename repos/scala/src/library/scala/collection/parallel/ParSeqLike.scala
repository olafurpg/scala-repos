...
trait ParSeqLike[
    ...
    if (from >= length) -1
      ...
      tasksupport.executeAndWaitResult(
          new IndexWhere(
              p,
              realfrom,
              splitter.psplitWithSignalling(realfrom,
                                            length - realfrom)(1) assign ctx))
    }
    ...
    if (end < 0) -1
      ...
      tasksupport.executeAndWaitResult(
          new LastIndexWhere(
              p,
              0,
              splitter.psplitWithSignalling(until,
  ...
  def startsWith[S](that: GenSeq[S], offset: Int): Boolean =
    that ifParSeq { pthat =>
      if (offset < 0 || offset >= length) offset == length && pthat.length == 0
        ...
        tasksupport.executeAndWaitResult(
            new SameElements(
                splitter.psplitWithSignalling(offset,
  ...
  def endsWith[S](that: GenSeq[S]): Boolean =
    that ifParSeq { pthat =>
      if (that.length == 0) true
        ...
        tasksupport.executeAndWaitResult(
            new SameElements(splitter.psplitWithSignalling(length - tlen,
                                                           tlen)(1) assign ctx,
                             pthat.splitter))
      }
  ...
  protected[this] class Corresponds[S](corr: (T, S) => Boolean,
                                       protected[this] val pit: SeqSplitter[T],
                                       val otherpit: SeqSplitter[S])
