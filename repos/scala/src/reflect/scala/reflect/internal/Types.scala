...
// Subclasses ------------------------------------------------------------
  ...
  private def embeddedSymbol(tp: Type, name: Name): Symbol =
    ...
    if (tp.typeSymbol.isRefinementClass) tp.normalize.decls.lookup(name)
      ...
      debuglog(
        s"no embedded symbol $name found in ${showRaw(tp)} --> ${tp.normalize.decls
          .lookup(name)}")
      NoSymbol
    }
  ...
  private final class AliasArgsTypeRef(pre: Type,
                                       sym: Symbol,
                                       args: List[Type])
  ...
  private final class ClassArgsTypeRef(pre: Type,
                                       sym: Symbol,
                                       args: List[Type])
      extends ArgsTypeRef(pre, sym, args)
...
//      maybeRewrap(underlying.instantiateTypeParams(formals, actuals))
    ...
    private def quantifierOwner =
      quantified
        .collectFirst { case q if q.owner.exists => q.owner }
        .getOrElse(NoSymbol)
    ...
    def isRepresentableWithWildcards = {
      ...
      underlying match {
        case TypeRef(pre, sym, args) =>
          ...
          wildcardArgs.distinct == wildcardArgs && !(otherArgs.exists(arg =>
            isQuantified(arg))) && !(wildcardArgs.exists(arg =>
    ...
    def registerBound(tp: Type,
      ...
      if (suspended) // constraint accumulation is disabled
        ...
        isRelatable(tp) && {
          ...
          tp.baseTypeSeq.toList.tail
            .filterNot(tp.parents contains _)
            .exists(unifyFull))))
        }
    }
    ...
    if (false && isDefinitionsInitialized) {
      assert(
        isUseableAsTypeArgs(args), {
          ...
          val arg_s =
            args
              .filterNot(isUseableAsTypeArg)
              .map(t => t + "/" + t.getClass) mkString ", "
...
// Helper Classes ---------------------------------------------------------
  ...
  def typeParamsToExistentials(clazz: Symbol,
    ...
    val eparams = mapWithIndex(tparams)(
      (tparam, i) =>
        clazz
          .newExistential(newTypeName("?" + i), clazz.pos)
          .setInfo(tparam.info.bounds))
...
// Helper Methods  -------------------------------------------------------------
  ...
  /** Compute lub (if `variance == Covariant`) or glb (if `variance == Contravariant`) of given list
    ...
    */
  def mergePrefixAndArgs(tps: List[Type],
                         variance: Variance,
