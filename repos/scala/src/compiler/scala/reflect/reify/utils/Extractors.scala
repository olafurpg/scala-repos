  ...
  private def mkCreator(
    ...
    val reifierBody = {
      ...
      val mirrorAlias = ValDef(
          ...
          TypeApply(Select(Ident(nme.MIRROR_UNTYPED), nme.asInstanceOf_),
  ...
  object ReifiedType {
    def apply(universe: Tree,
      ...
      val ctor = TypeApply(
          Select(Select(Ident(nme.UNIVERSE_SHORT), tagFactory), nme.apply),
          List(mkTarg(tpe)))
      val args = List(Ident(nme.MIRROR_SHORT),
                      mkCreator(tpnme.REIFY_TYPECREATOR_PREFIX, symtab, rtpe))
    ...
    def unapply(tree: Tree): Option[(Tree, TermName, Tree, Long, String)] = {
      ...
      tree match {
        case ValDef(
            ...
            Apply(
            Select(
            Select(Select(uref1 @ Ident(_), internal1), rs1), freeTermFactory),
            _ :+ ApplyCall(
            Select(
            Select(Select(uref2 @ Ident(_), internal2), rs2), flagsRepr),
    ...
    def unapply(tree: Tree): Option[(Tree, TermName, Long, Boolean)] =
      tree match {
        case ValDef(
            ...
            Apply(
            Select(
            Select(Select(uref1 @ Ident(_), internal1), rs1), newNestedSymbol),
            List(_,
                 _,
                 _,
                 ApplyCall(
