...
trait Reshape { self: Reifier =>
  ...
  val reshape = new Transformer {
    ...
    private def toPreTyperLazyVal(ddef: DefDef): ValDef = {
      ...
      val mods1 =
        Modifiers(flags1, privateWithin0, annotations0).setPositions(
          mods0.positions)
    ...
    private def trimAccessors(deff: Tree, stats: List[Tree]): List[Tree] = {
      val symdefs = (stats
        .collect { case vodef: ValOrDefDef => vodef }
        .map(vodeff => vodeff.symbol -> vodeff))
        .toMap
      val accessors = scala.collection.mutable.Map[ValDef, List[DefDef]]()
      stats
        .collect { case ddef: DefDef => ddef }
        .foreach(defdef => {
          val valdef =
            symdefs
              .get(defdef.symbol.accessedOrSelf)
              .collect {
    ...
    private def trimSyntheticCaseClassCompanions(
      ...
      stats.diff(
        stats
          .collect { case moddef: ModuleDef => moddef }
          .filter(moddef => {
