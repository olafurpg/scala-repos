...
trait Validators { self: DefaultMacroCompiler =>
  ...
  trait Validator { self: MacroImplRefCompiler =>
    ...
    private case class MacroImplSig(tparams: List[Symbol],
      ...
      private def paramss_s =
        paramss map
          (ps =>
                ps.map(s => s"${s.name}: ${s.tpe_*}").mkString("(", ", ", ")")) mkString ""
    ...
    private lazy val referenceMacroImplSig: MacroImplSig = {
      ...
      object SigGenerator {
        ...
        val ctxTpe =
          if (isBlackbox) BlackboxContextClass.tpe
          else WhiteboxContextClass.tpe
        val ctxPrefix =
          if (isImplMethod)
            singleType(
                NoPrefix,
