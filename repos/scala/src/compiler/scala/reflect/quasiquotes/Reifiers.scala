...
trait Reifiers { self: Quasiquotes =>
  ...
  abstract class Reifier(val isReifyingExpressions: Boolean) extends {
                ...
                SyntacticNew(
                    ...
                    List(
                        DefDef(
                            ...
                            TypeTree(),
                            SyntacticBlock(
                                unlifters.preamble() :+ Match(Ident(nme.tree),
                                                              cases))))),
                nme.unapply),
            args)
      }
    ...
    override def reifyTreeSyntactically(tree: Tree) = tree match {
      ...
      case SyntacticClassDef(mods,
        ...
        mirrorBuildCall(nme.SyntacticClassDef,
                        ...
                        reify(selfdef),
                        reify(body))
      case SyntacticPackageObjectDef(name,
                                     earlyDefs,
                                     parents,
                                     selfdef,
                                     body) =>
    ...
    def reifyEarlyDef(hole: Hole) =
      reifyConstructionCheck(nme.mkEarlyDef, hole)
