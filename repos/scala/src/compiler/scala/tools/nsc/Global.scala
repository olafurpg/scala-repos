...
// ------------ Phases -------------------------------------------}
  ...
  /** Merges new classpath entries into the symbol table
    ...
    */
  private def mergeNewEntries(newEntries: PlatformClassPath,
                              root: ClassSymbol,
  ...
  def showDef(fullName: Name, declsOnly: Boolean, ph: Phase) = {
    ...
    val syms = findMemberFromRoot(fullName) match {
      ...
      case NoSymbol =>
        phased(
          currentRun.symSource.keys
            .map(sym => findNamedMember(fullName, sym))
            .filterNot(_ == NoSymbol) toList)
...
object Global {
  ...
  private def reporter(settings: Settings): Reporter = {
    ...
    loader.create[Reporter](settings.reporter.value, settings.errorFn)(
      settings)
  }
}
