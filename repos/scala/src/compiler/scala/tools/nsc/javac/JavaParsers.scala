...
trait JavaParsers extends ast.parser.ParsersCommon with JavaScanners {
  ...
  abstract class JavaParser extends ParserCommon {
    ...
    def termDecl(mods: Modifiers, parentToken: Int): List[Tree] = {
      ...
      if (in.token == LPAREN && rtptName != nme.EMPTY && !inInterface) {
        ...
        if (in.token == LPAREN) {
          ...
          val body = if (bodyOk && in.token == LBRACE) {
            methodBody()
          } else {
            if (parentToken == AT && in.token == DEFAULT) {
              val annot = atPos(pos) {
                New(Select(scalaDot(nme.runtime), tpnme.AnnotationDefaultATTR),
                    Nil)
              }
    ...
    def classDecl(mods: Modifiers): List[Tree] = {
      ...
      val superclass = if (in.token == EXTENDS) {
        in.nextToken()
        typ()
      } else {
        javaLangObject()
    ...
    def interfaceDecl(mods: Modifiers): List[Tree] = {
      ...
      val name = identForType()
      val tparams = typeParams()
      val parents = if (in.token == EXTENDS) {
        in.nextToken()
        repsep(typ, COMMA)
      } else {
        List(javaLangObject())
    ...
    def enumDecl(mods: Modifiers): List[Tree] = {
      ...
      val (statics, body) = if (in.token == SEMI) {
        in.nextToken()
    ...
    def compilationUnit(): Tree = {
      var pos = in.currentPos
      val pkg: RefTree = if (in.token == AT || in.token == PACKAGE) {
        annotations()
        pos = in.currentPos
        accept(PACKAGE)
        val pkg = qualId()
