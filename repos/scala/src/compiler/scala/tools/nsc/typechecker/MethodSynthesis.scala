...
trait MethodSynthesis { self: Analyzer =>
  ...
  class ClassMethodSynthesis(val clazz: Symbol, localTyper: Typer) {
    ...
    private def cloneInternal(original: Symbol,
      ...
      val m =
        original
          .cloneSymbol(clazz, newMethodFlags(original), name)
          .setPos(clazz.pos.focus)
  ...
  trait MethodSynth { self: Namer =>
    ...
    private def warnForDroppedAnnotations(tree: Tree) {
      ...
      annotations
        .filterNot(retained contains _)
        .foreach(ann => issueAnnotationWarning(tree, ann, targetClass))
    }
    ...
    sealed abstract class BeanAccessor(bean: String)
        extends DerivedFromValDef {
