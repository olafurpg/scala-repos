  ...
  def transformNamedApplication(typer: Typer, mode: Mode, pt: Type)(
    ...
    def baseFunBlock(baseFun: Tree): Tree = {
      ...
      def blockWithQualifier(qual: Tree, selected: Name) = {
        val sym =
          blockTyper.context.owner
            .newValue(unit.freshTermName(nme.QUAL_PREFIX), newFlags = ARTIFACT)
      ...
      val symPs = map2(args, paramTypes)((arg, paramTpe) =>
        arg match {
          ...
          case _ =>
            ...
            val s =
              context.owner
                .newValue(unit.freshTermName(nme.NAMEDARG_PREFIX),
                          arg.pos,
                          newFlags = ARTIFACT)
                .setInfo {
        ...
        case (Some((sym, byName, repeated)), arg) =>
          val body =
            if (byName) {
              ...
              new ChangeOwnerTraverser(context.owner, res.symbol)
                .traverse(arg) // fixes #2290
              res
            } else {
              new ChangeOwnerTraverser(context.owner, sym)
                .traverse(arg) // fixes #4502
              if (repeated)
                arg match {
