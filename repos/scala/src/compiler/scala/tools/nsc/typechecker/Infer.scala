  ...
  abstract class Inferencer
    ...
    def makeFullyDefined(tp: Type): Type = {
      ...
      def addTypeParam(bounds: TypeBounds): Type = {
        val tparam =
          context.owner
            .newExistential(newTypeName("_" + tparams.size),
                            context.tree.pos.focus)
            .setInfo(bounds)
        tparams ::= tparam
        tparam.tpe
      }
    ...
    def methTypeArgs(tparams: List[Symbol],
      ...
      def canWarnAboutAny = {
        ...
        val hasAny =
          (pt :: restpe :: formals ::: argtpes ::: loBounds)
            .exists(_.dealiasWidenChain.exists(containsAny))
        !hasAny
      }
