      ...
      if (isMacroImplRef(tree)) {
        ...
        tree.tpe match {
          ...
          case tp if mode.typingExprNotLhs && isExistentialType(tp) =>
            adapt(tree setType tp.dealias.skolemizeExistential(context.owner,
                                                               tree),
                  mode,
                  pt,
                  original)
        ...
        for (p <- allParams) {
          for (n <- p.deprecatedParamName) {
            if (allParams.exists(p1 =>
                      p != p1 &&
                        (p1.name == n || p1.deprecatedParamName.exists(
                                _ == n))))
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
          ...
          while ((e1 ne null) && e1.owner == scope) {
            if (!accesses(e.sym, e1.sym) && !accesses(e1.sym, e.sym) &&
              ...
              if (!e.sym.isErroneous && !e1.sym.isErroneous &&
                ...
                scope
                  .unlink(e1) // need to unlink to avoid later problems with lub; see #2779
              }
    ...
    object dyna {
      import treeInfo.{
        isApplyDynamicName, DynamicUpdate, DynamicApplicationNamed
      }
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
        ...
        def samePackedTypes =
          (!isPastTyper && thenp1.tpe.annotations.isEmpty &&
                elsep1.tpe.annotations.isEmpty && packedType(
                  thenp1,
                  context.owner) =:= packedType(elsep1, context.owner))
          ...
          case SilentResultValue(fun1) =>
            ...
            val noSecondTry =
              (isPastTyper || context.inSecondTry || (fun2.symbol ne null) &&
                    fun2.symbol.isConstructor || isImplicitMethodType(
                      fun2.tpe))
        ...
        if (!reallyExists(sym)) {
          ...
          result match {
            ...
            case SelectFromTypeTree(qual @ TypeTree(), name)
              ...
              treeCopy
                .SelectFromTypeTree(result, (TypeTreeWithDeferredRefCheck() {
                  () =>
    ...
    def computeMacroDefType(ddef: DefDef, pt: Type): Type = {
      ...
      val rhs1 = if (transformed contains ddef.rhs) {
