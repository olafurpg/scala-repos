...
abstract class ClassfileParser {
                ...
                while (sig.charAt(index) != '>') {
                  sig.charAt(index) match {
                    case variance @ ('+' | '-' | '*') =>
                      ...
                      val newtparam =
                        sym
                          .newExistential(newTypeName("?" + i), sym.pos)
                          .setInfo(bounds)
    ...
    def enterClassAndModule(entry: InnerClassEntry, file: AbstractFile) {
      ...
      val (innerClass, innerModule) =
        if (file == NoAbstractFile) {
          ...
          val cls =
            owner
              .newClass(name.toTypeName, NoPosition, sflags)
              .setInfo(completer)
          val mod =
            owner
              .newModule(name.toTermName, NoPosition, sflags)
              .setInfo(completer)
  ...
  class LazyAliasType(alias: Symbol)
    ...
    override def complete(sym: Symbol) {
      sym.setInfo(
        createFromClonedSymbols(alias.initialize.typeParams, alias.tpe)(
          typeFun))
    }
  }
