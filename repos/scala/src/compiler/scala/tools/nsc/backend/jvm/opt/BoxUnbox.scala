      ...
      insn match {
        case mi: MethodInsnNode =>
          if (isRefCreate(mi))
            ...
            checkKind(mi).map(
                (StaticFactory(
                     mi, loadInitialValues = Some(loadZeroValue(mi))),
                 _))
          else None
