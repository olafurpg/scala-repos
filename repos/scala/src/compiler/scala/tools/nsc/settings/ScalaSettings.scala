...
trait ScalaSettings
  ...
  val jvmargs = PrefixSetting(
      "-J<flag>", "-J", "Pass <flag> directly to the runtime system.")
  ...
  val noassertions =
    BooleanSetting("-Xdisable-assertions",
  ...
  val browse = PhasesSetting(
      "-Ybrowse", "Browse the abstract syntax tree after")
  val check = PhasesSetting("-Ycheck", "Check the tree at the end of")
  val Yshow = PhasesSetting(
      "-Yshow", "(Requires -Xshow-class or -Xshow-object) Show after")
  ...
  val Ymemberpos =
    StringSetting("-Yshow-member-pos",
                  "output style",
  ...
  object YoptChoices extends MultiChoiceEnumeration
    ...
    val lDefault = Choice("l:default",
                          "Enable default optimizations: " +
                          defaultChoices.mkString("", ",", "."),
                          expandsTo = defaultChoices)

    private val methodChoices = List(unreachableCode,
                                     simplifyJumps,
                                     compactLocals,
                                     copyPropagation,
                                     redundantCasts,
  ...
  val Yopt = MultiChoiceSetting(name = "-Yopt",
  ...
  def YoptWarningsSummaryOnly =
    YoptWarnings.value subsetOf Set(
        YoptWarningsChoices.none, YoptWarningsChoices.atInlineFailedSummary)
  ...
  val YpatmatExhaustdepth = IntSetting(
      ...
      str =>
        Some(if (str.equalsIgnoreCase("off")) Int.MaxValue else str.toInt))
