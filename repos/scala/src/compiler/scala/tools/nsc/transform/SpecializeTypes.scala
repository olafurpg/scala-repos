...
abstract class SpecializeTypes extends InfoTransform with TypingTransformers {
    ...
    def specializedClass(env0: TypeEnv, normMembers: List[Symbol]): Symbol = {
      ...
      val specializedInfoType: Type = {
        ...
        def applyContext(tpe: Type) =
          subst(env, tpe).instantiateTypeParams(oldClassTParams,
          ...
          if (nme.isLocalName(m.name)) {
            ...
            clazz.caseFieldAccessors.find(_.name.startsWith(m.name)) foreach {
              cfa =>
                ...
                debuglog("override case field accessor %s -> %s"
  ...
  private def specializeMember(owner: Symbol,
    ...
    def specializeOn(tparams: List[Symbol]): List[Symbol] =
      specializations(tparams) map { spec0 =>
        ...
        if (wasSpec.nonEmpty)
          debuglog("specialized overload for %s in %s"
      ...
      (clazz.info.decls flatMap { overriding =>
            needsSpecialOverride(overriding) match {
              ...
              case (overridden, env) =>
                ...
                debuglog("specialized overload %s for %s in %s: %s"
  ...
  private def unify(tp1: Type,
    ...
    case _ =>
      debuglog("don't know how to unify %s [%s] with %s [%s]"
  ...
  def specializeCalls(unit: CompilationUnit) = new TypingTransformer(unit) {
    ...
    def transform1(tree: Tree) = {
      ...
      tree match {
        case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
          def transformNew = {
            debuglog("Attempting to specialize new %s(%s)"
    ...
    private def addBody(tree: DefDef, source: Symbol): DefDef = {
      ...
      copyDefDef(tree)(vparamss = List(newSyms map ValDef.apply),
                       rhs = newBody)
    }
