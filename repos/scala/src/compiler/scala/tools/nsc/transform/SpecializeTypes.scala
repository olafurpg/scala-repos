...
abstract class SpecializeTypes extends InfoTransform with TypingTransformers {
  ...
  case class NormalizedMember(target: Symbol) extends SpecializedInfo {
    ...
    override lazy val degenerate = {
      ...
      debuglog(
          "degenerate: " + target + " stv tparams: " + stvTypeParams +
            " stv info: " + stvResult)
  ...
  def specializeClass(clazz: Symbol, outerEnv: TypeEnv): List[Symbol] = {
    def specializedClass(env0: TypeEnv, normMembers: List[Symbol]): Symbol = {
      ...
      sClass
        .setAnnotations(clazz.annotations) // SI-8574 important that the subclass picks up @SerialVersionUID, @strictfp, etc.
  ...
  private def specializedOverload(owner: Symbol,
    ...
    (sym
      .cloneSymbol(owner, newFlags, newName = specname) modifyInfo (info =>
                                                                      subst(
                                                                          env,
                                                                          info.asSeenFrom(
                                                                              owner.thisType,
      ...
      (tp1 zip tp2).foldLeft(env) { (env, args) =>
        if (!strict) unify(args._1, args._2, env, strict)
          ...
          if (env.keySet intersect nenv.keySet isEmpty) env ++ nenv
            ...
            debuglog(
                "could not unify: u(" + args._1 + ", " + args._2 +
                  ") yields " + nenv + ", env: " + env)
            unifyError(tp1, tp2)
          }
        }
      ...
      (treeType =:= memberType) || {
        ...
        memberType match {
          case PolyType(_, resTpe) =>
            debuglog(
                "Conformance for anyref - polytype with result type: " +
                  resTpe + " and " + treeType + "\nOrig. sym.: " + origSymbol)
            try {
    ...
    def transform1(tree: Tree) = {
      ...
      tree match {
        ...
        case Select(Super(_, _), _)
          ...
          val pos = tree.pos
          debuglog(pos.source.file.name + ":" + pos.line +
            ...
            if (symbol.isConstructor) {
              ...
              info(symbol) match {
                ...
                case SpecialOverload(original, env) =>
                  debuglog(
                      "completing specialized " + symbol.fullName +
                        " calling " + original)
      ...
      trees flatMap {
        case tree @ ClassDef(_, _, _, impl) =>
          ...
          for (((sym1, env), specCls) <- specializedClass
            ...
            debuglog(
                "created synthetic class: " + specCls + " of " + sym1 +
                  " in " + pp(env))
