      ...
      tp.typeSymbol match {
        ...
        case sym if sym.isSealed =>
          ...
          if (grouped) {
            ...
            List(
              debug
                .patmatResult(s"enum sealed tp=$tp, tpApprox=$tpApprox as") {
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case other @ ListExample(_) =>
            this == other ||
              ((elems.length == other.elems.length) && (elems
                .zip(other.elems))
                .forall {
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case TupleExample(otherArgs) =>
            this == other ||
              ((ctorArgs.length == otherArgs.length) && (ctorArgs
                .zip(otherArgs))
                .forall {
                  case (a, b) => a.coveredBy(b)
                })
                  ...
                  val examples = (0 until argLen)
                    .map(
                      i =>
                        fields
                          .get(caseFieldAccs(i))
                          .map(_.toCounterExample(brevity))
                          .getOrElse(Some(WildcardExample)))
                    .toList
                  sequence(examples)
                }
