...
object scalac extends Command
  ...
  val name = Section(
      "NAME",
      MBold(command) & " " & NDash & " Compiler for the " & Link(
  ...
  val parameters = Section(
      ...
      DefinitionList(
          ...
          Definition(
              ...
              "One or more source files to be compiled (such as " & Mono(
                  "MyClass.scala") & ").")))

  val description = Section(
      "DESCRIPTION",
      "The " & MBold(command) & " tool reads class and object definitions, " & "written in the Scala programming language, and compiles them into " & "bytecode class files.",
      "By default, the compiler puts each class file in the same directory " & "as its source file. You can specify a separate destination directory " & "with -d (see " & Link(
          Bold("OPTIONS"), "#options") & ", below).")

  val options = Section(
      "OPTIONS",
      "The compiler has a set of standard options that are supported on the " & "current development environment and will be supported in future " & "releases. An additional set of non-standard options are specific to " & "the current virtual machine implementation and are subject to change " & "in the future.  Non-standard options begin with " & MBold(
          "-X") & ".",
      Section(
          "Standard Options",
          DefinitionList(
              ...
              Definition(CmdOption("X"),
                         "Print a synopsis of advanced options."),
              Definition(
                  CmdOption("bootclasspath", Argument("path")),
                  "Override location of bootstrap class files (where to find the " & "standard built-in classes, such as \"" & Mono(
                      "scala.List") & "\")."),
              Definition(
                  CmdOption("classpath", Argument("path")),
                  SeqPara(
                      "Specify where to find user class files (on Unix-based systems " & "a colon-separated list of paths, on Windows-based systems, a " & "semicolon-separated list of paths). This does not override the " & "built-in (" & Mono(
                          "\"boot\"") & ") search path.",
                      "The default class path is the current directory. Setting the " & Mono(
                          "CLASSPATH") & " variable or using the " & Mono(
                          "-classpath") & " " & "command-line option overrides that default, so if you want to " & "include the current directory in the search path, you must " & "include " & Mono(
                          "\".\"") & " in the new settings.")),
              ...
              Definition(
                  ...
                  SeqPara(
                      ...
                      "The default value is platform-specific (Linux: " & Mono(
                          "\"UTF8\"") & ", Windows: " & Mono("\"Cp1252\"") & "). Executing the following " & "code in the Scala interpreter will return the default value " & "on your system:",
                      MBold("    scala> ") & Mono(
              ...
              Definition(
                  ...
                  SeqPara(
                      ...
                      Mono("\"vars\"") & " generates source, line number and local " & "variable information,",
                      Mono("\"notailcalls\"") & " generates all of the above and " & Italic(
                          "will not") & " perform tail call optimization.")),
              Definition(CmdOption("help"),
              ...
              Definition(CmdOption("version"),
                         "Print product version and exit."),
              Definition(Mono(Bold("@") & Argument("file")),
      ...
      Section(
          ...
          DefinitionList(
              ...
              Definition(
                  ...
                  "Calls to " & MItalic("@elidable") & " methods are omitted if method priority is lower than argument."),
              Definition(CmdOption("Xexperimental"),
                         "Enable experimental extensions"),
              Definition(CmdOption("Xfatal-warnings"),
                         "Fail the compilation if there are any warnings."),
              Definition(CmdOption("Xfull-lubs"),
                         "Retain pre 2.10 behavior of less aggressive truncation of least upper bounds."),
              Definition(CmdOption("Xfuture"),
              ...
              Definition(CmdOption("Xlog-implicit-conversions"),
              ...
              Definition(CmdOptionBound("Xmacro-settings:",
                                        Argument("option")),
              ...
              Definition(CmdOption("Xmax-classfile-name", Argument("n")),
      ...
      Section("Compilation Phases",
              DefinitionList(
                  ...
                  Definition(MItalic("packageobjects"),
                             "load package objects"),
                  Definition(MItalic("typer"),
  ...
  val environment = Section(
      ...
      DefinitionList(
          ...
          Definition(MBold("JAVA_OPTS"),
                     SeqPara(
                         "Specify the options to be passed to the " & MBold(
  ...
  val examples = Section(
      ...
      DefinitionList(
          Definition("Compile a Scala program to the current directory",
                     CmdLine("HelloWorld")),
          Definition(
              "Compile a Scala program to the destination directory " & MBold(
                  "classes"),
              CmdLine(CmdOption("d", "classes") & "HelloWorld.scala")),
          Definition("Compile a Scala program using a user-defined " & MBold(
                         "java") & " " & "command",
                     MBold("env JAVACMD") & Mono("=/usr/local/bin/cacao ") & CmdLine(
                         CmdOption("d", "classes") & "HelloWorld.scala")),
          Definition(
              "Compile all Scala files found in the source directory " & MBold(
                  "src") & " to the destination directory " & MBold("classes"),
              CmdLine(CmdOption("d", "classes") & "src/*.scala"))))

  val exitStatus = Section(
      "EXIT STATUS",
      MBold(command) & " returns a zero exist status if it succeeds to " & "compile the specified input files. Non zero is returned in case " & "of failure.")
