...
class DotDiagramGenerator(settings: doc.Settings, dotRunner: DotRunner)
  ...
  private def generateDot(d: Diagram) = {
    ...
    d match {
      case InheritanceDiagram(_thisNode,
                              _superClasses,
                              _subClasses,
                              _incomingImplicits,
                              _outgoingImplicits) =>
        ...
        superClasses =
          if (_superClasses.length > settings.docDiagramsMaxNormalClasses.value) {
            ...
            List(
                NormalNode(textTypeEntity(_superClasses.length + MultiSuffix),
        ...
        subClasses =
          if (_subClasses.length > settings.docDiagramsMaxNormalClasses.value) {
            ...
            List(
                NormalNode(textTypeEntity(_subClasses.length + MultiSuffix),
  ...
  private def transform(e: scala.xml.Node): scala.xml.Node = e match {
    ...
    case Elem(prefix, "svg", attribs, scope, child @ _ *) => {
      ...
      Elem(prefix, "svg", attribs, scope, true, child map (x =>
                transform(x)): _*) % new UnprefixedAttribute(
          "id",
          "graph" + counter,
    ...
    case g @ Elem(prefix, "g", attribs, scope, children @ _ *)
      ...
      var res = new Elem(prefix, "g", attribs, scope, true, (children map (x =>
                    transform(x))): _*)
    ...
    case Elem(prefix, label, attribs, scope, child @ _ *) =>
      Elem(prefix, label, attribs, scope, true, child map (x =>
                transform(x)): _*)
    case x => x
  }
