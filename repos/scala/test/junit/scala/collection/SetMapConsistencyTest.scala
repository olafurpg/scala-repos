...
class SetMapConsistencyTest {
  ...
  def boxIhm[A] =
    new BoxImmutableMap[A, ci.HashMap[A, Int]](new ci.HashMap[A, Int],
                                               "immutable.HashMap")
  ...
  val longKeys =
    intKeys.map(_.toLong) ++ Array(Long.MaxValue,
                                   Long.MinValue,
                                   1397198789151L,
                                   -41402148014L)
  ...
  def extraMutableLongMapTests() {
    ...
    assert {
      ...
      lm2.size == 1000000 && (0 to 1100000 by 100000).forall(i =>
        (lm2 contains i) == i < 1000000)
    }
  ...
  def extraMutableAnyRefMapTests() {
    ...
    assert {
      arm
        .map { case (k, v) => (if (k == null) "" else k + k) -> v.toString }
