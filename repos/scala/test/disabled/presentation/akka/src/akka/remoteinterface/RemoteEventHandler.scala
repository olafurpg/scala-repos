...
class RemoteEventHandler extends Actor {
  ...
  def receive = {
    ...
    case RemoteServerWriteFailed(request, cause, server, clientAddress) =>
      EventHandler.error(cause,
                         server,
                         "RemoteServerWriteFailed - Request[%s] Address[%s]"
                           .format(request, clientAddress.toString))
    ...
    case RemoteServerClientClosed(server, clientAddress) =>
      EventHandler.info(server,
                        "RemoteServerClientClosed - Address[%s]".format(
                          clientAddress.toString))
