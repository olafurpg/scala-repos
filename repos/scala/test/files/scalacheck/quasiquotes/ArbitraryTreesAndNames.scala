...
trait ArbitraryTreesAndNames {
  ...
  def genAppliedTypeTree(size: Int) =
    for {
      tpt <- genTree(size - 1) if tpt.isType
  ...
  def genFunction(size: Int) =
    for {
      vparams <- smallList(size, genValDef(size - 1))
      body <- genTree(size - 1)
  ...
  def genTemplate(size: Int) =
    for {
      parents <- smallList(size, genTree(size - 1))
