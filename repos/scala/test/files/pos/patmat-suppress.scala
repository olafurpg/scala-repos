...
class TestSealedExhaustive {
  ...
  def ma33(x: Kult) = x match {
    // exhaustive
    case Kult(_) => // exhaustive
  }

  def ma3(x: Mult) = (x, x) match {
    // not exhaustive
  ...
  def ma3u(x: Mult) = ((x, x): @unchecked) match {
    // not exhaustive, but not checked!
    case (Kult(_), Qult()) =>
    case (Qult(), Kult(_)) =>
  }
  ...
  def ma4(x: Deep) = x match {
    // missing cases: Gu, Gp which is not abstract so must be included
    case Ga =>
  }
