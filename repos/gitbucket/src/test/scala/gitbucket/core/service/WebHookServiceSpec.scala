...
class WebHookServiceSpec extends FunSuite with ServiceSpecBase {
  ...
  test("WebHookPullRequestService.getPullRequestsByRequestForWebhook") {
    withTestDB { implicit session =>
      ...
      val (issue1, pullreq1) = generateNewPullRequest("user1/repo1/master1",
                                                      "user2/repo2/master2",
                                                      loginUser = "root")
      val (issue3, pullreq3) = generateNewPullRequest("user3/repo3/master3",
                                                      "user2/repo2/master2",
                                                      loginUser = "root")
      val (issue32, pullreq32) = generateNewPullRequest("user3/repo3/master32",
                                                        "user2/repo2/master2",
                                                        loginUser = "root")
      ...
      assert(
          service.getPullRequestsByRequestForWebhook("user1",
                                                     "repo1",
                                                     "master1") == Map.empty)
      ...
      assert(r.size == 3)
      assert(
          r((issue1, issueUser, pullreq1, user1, user2)) == Set("webhook1-1",
                                                                "webhook1-2"))
      assert(
          r((issue3, issueUser, pullreq3, user3, user2)) == Set("webhook3-1",
                                                                "webhook3-2"))
      assert(r((issue32, issueUser, pullreq32, user3, user2)) == Set(
              "webhook3-1",
              "webhook3-2"))
      ...
      assert(r2.size == 2)
      assert(
          r2((issue3, issueUser, pullreq3, user3, user2)) == Set("webhook3-1",
                                                                 "webhook3-2"))
  ...
  test("add and get and update and delete") {
    withTestDB { implicit session =>
      ...
      assert(
          service.getWebHook("user2", "repo1", "http://example.com") == None)
      assert(
          service.getWebHook("user1", "repo2", "http://example.com") == None)
      service.updateWebHook("user1",
                            "repo1",
                            "http://example.com",
      ...
      assert(
          service.getWebHook("user1", "repo1", "http://example.com") == Some(
              (WebHook("user1", "repo1", "http://example.com", Some("key")),
               Set(WebHook.Push, WebHook.Issues))))
      assert(
          service.getWebHooksByEvent("user1", "repo1", WebHook.PullRequest) == Nil)
      assert(
