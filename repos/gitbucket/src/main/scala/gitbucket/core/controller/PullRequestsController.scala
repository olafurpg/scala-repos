...
trait PullRequestsControllerBase extends ControllerBase {
                ...
                using(Git.open(getRepositoryDir(owner, name))) {
                  git =>
                    ...
                    if (pullreq.branch == defaultBranch) {
                      ...
                      issue.content match {
                        case Some(content) =>
                          closeIssuesFromMessage(content,
                                                 loginAccount.userName,
                                                 owner,
                                                 name)
                        case _ =>
                      }
                      closeIssuesFromMessage(form.message,
                                             loginAccount.userName,
                                             owner,
                                             name)
                    }
    ...
    (form, repository) =>
      defining(repository.owner, repository.name) {
        case (owner, name) =>
          ...
          // record activity
          recordPullRequestActivity(owner,
                                    name,
                                    loginUserName,
                                    issueId,
                                    form.title)
  ...
  /**
    ...
    */
  private def parseCompareIdentifie(value: String,
    ...
    defining(repository.owner, repository.name) {
      case (owner, repoName) =>
        ...
        gitbucket.core.issues.html.list(
            ...
            countIssue(condition.copy(state = "open"),
                       true,
                       owner -> repoName),
            countIssue(condition.copy(state = "closed"),
                       true,
                       owner -> repoName),
            condition,
            repository,
