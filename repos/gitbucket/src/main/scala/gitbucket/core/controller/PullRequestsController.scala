...
class PullRequestsController
    extends PullRequestsControllerBase with RepositoryService
    with AccountService with IssuesService with PullRequestService
    with MilestonesService with LabelsService with CommitsService
    with ActivityService with WebHookPullRequestService
...
trait PullRequestsControllerBase extends ControllerBase
  ...
  get("/:owner/:repository/pulls")(
    ...
    if (Option(q).exists(_.contains("is:issue")))
      redirect(s"/${repository.owner}/${repository.name}/issues?q=" +
            ...
            git =>
              ...
              html.pullreq(
                  ...
                  (commits.flatten
                    ...
                    .sortWith((a,
                      b) => a.registeredDate before b.registeredDate),
            ...
            case (issue, pullreq) =>
              ...
              val mergeStatus = PullRequestService.MergeStatus(
                  ...
                  commitStatues = getCommitStatues(
                        owner, name, pullreq.commitIdTo),
                  branchProtection = branchProtection,
                  branchIsOutOfDate = JGitUtil.getShaByRef(
                        owner, name, pullreq.branch) != Some(
                        pullreq.commitIdFrom),
                  ...
                  hasUpdatePermission = hasWritePermission(
                        ...
                        context.loginAccount) && context.loginAccount.map
                    u =>
              ...
              html.mergeguide(mergeStatus,
                              ...
                              pullreq,
                              repository,
                              getRepository(pullreq.requestUserName,
                                            pullreq.requestRepositoryName).get)
      getOrElse NotFound
  )

  get("/:owner/:repository/pull/:id/delete/*")(collaboratorsOnly
    repository =>
      params("id").toIntOpt.map
        issueId =>
          ...
          createComment(repository.owner,
                        repository.name,
                        userName,
                        issueId,
                        branchName,
  ...
  post("/:owner/:repository/pull/:id/update_branch")(
    ...
    (for
      ...
      (issue, pullreq) <- getPullRequest(
          baseRepository.owner, baseRepository.name, issueId)
                      ...
                      issue.content match
                        case Some(content) =>
                          closeIssuesFromMessage(
                              content, loginAccount.userName, owner, name)
                        case _ =>
                      closeIssuesFromMessage(
                          form.message, loginAccount.userName, owner, name)

                    updatePullRequests(owner, name, pullreq.branch)

                    // call web hook
                    callPullRequestWebHook("closed",
                                           repository,
                                           issueId,
                                           context.baseUrl,
    ...
    (forkedRepository.repository.originUserName,
     forkedRepository.repository.originRepositoryName) match
      case (Some(originUserName), Some(originRepositoryName)) =>
          ...
          getOrElse NotFound
      case _ =>
          using(Git.open(getRepositoryDir(forkedRepository.owner,
                                          forkedRepository.name)))
            git =>
              ...
              getOrElse
                redirect(
                    s"/${forkedRepository.owner}/${forkedRepository.name}")
  )
        ...
        case (owner, name) =>
          ...
          val issueId =
            createIssue(owner = repository.owner,
                        ...
                        assignedUserName = if (writable) form.assignedUserName
                          else None,
          ...
          recordPullRequestActivity(
              owner, name, loginUserName, issueId, form.title)

          // call web hook
          callPullRequestWebHook("opened",
                                 repository,
                                 issueId,
                                 context.baseUrl,
                                 context.loginAccount.get)
  ...
  private def parseCompareIdentifie(
      value: String, defaultOwner: String): (String, String) =
    if (value.contains(':'))
  ...
  private def searchPullRequests(
      userName: Option[String], repository: RepositoryService.RepositoryInfo) =
    defining(repository.owner, repository.name)
      case (owner, repoName) =>
        ...
        gitbucket.core.issues.html.list(
            ...
            countIssue(
                condition.copy(state = "open"), true, owner -> repoName),
            countIssue(
                condition.copy(state = "closed"), true, owner -> repoName),
            condition,
            repository,
