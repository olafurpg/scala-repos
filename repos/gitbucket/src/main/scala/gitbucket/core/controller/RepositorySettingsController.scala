...
trait RepositorySettingsControllerBase extends ControllerBase {
  ...
  case class OptionsForm(repositoryName: String,
  ...
  post("/:owner/:repository/settings/update_default_branch",
       defaultBranchForm)(ownerOnly { (form, repository) =>
    if (repository.branchList.find(_ == form.defaultBranch).isEmpty) {
      redirect(s"/${repository.owner}/${repository.name}/settings/options")
    } else {
      saveRepositoryDefaultBranch(repository.owner,
                                  repository.name,
                                  form.defaultBranch)
  ...
  get("/:owner/:repository/settings/branches/:branch")(ownerOnly {
    repository =>
      ...
      if (repository.branchList.find(_ == branch).isEmpty) {
        ...
        html.branchprotection(repository,
                              branch,
                              protection,
                              knownContexts,
                              flash.get("info"))
  ...
  post("/:owner/:repository/settings/hooks/edit", webHookForm(true))(
    ownerOnly { (form, repository) =>
      updateWebHook(repository.owner,
                    repository.name,
                    form.url,
                    form.events,
                    form.token)
        ...
        LockUtil
          .lock(s"${repository.owner}/${repository.name}") {
            ...
            defining(getRepositoryDir(repository.owner, repository.name)) {
              dir =>
                FileUtils.moveDirectory(dir,
                                        getRepositoryDir(form.newOwner,
                                                         repository.name))
            }
            // Move wiki repository
            defining(getWikiRepositoryDir(repository.owner, repository.name)) {
              dir =>
                FileUtils.moveDirectory(dir,
                                        getWikiRepositoryDir(form.newOwner,
                                                             repository.name))
            }
          }
      }
