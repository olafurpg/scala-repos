...
object ZCount {
  def apply(args: Seq[Array[Byte]]) = {
    ...
    new ZCount(ChannelBuffers.wrappedBuffer(args(0)),
               ZInterval(list(1)),
               ZInterval(list(2)))
  }
}
...
object ZIncrBy {
  def apply(args: Seq[Array[Byte]]) = {
    ...
    new ZIncrBy(ChannelBuffers.wrappedBuffer(key),
                amount,
                ChannelBuffers.wrappedBuffer(list(2)))
  }
}

case class ZInterStore(destination: ChannelBuffer,
                       numkeys: Int,
...
object ZInterStore extends ZStoreCompanion {
  def get(dest: String,
    ...
    new ZInterStore(StringToChannelBuffer(dest),
                    numkeys,
...
case class ZRange(key: ChannelBuffer,
                  start: Long,
                  stop: Long,
...
object ZRange extends ZRangeCmdCompanion {
  override def get(key: ChannelBuffer,
                   start: Long,
                   stop: Long,
...
case class ZRangeByScore(key: ChannelBuffer,
                         min: ZInterval,
                         max: ZInterval,
                         withScores: Option[CommandArgument] = None,
...
case class ZRem(key: ChannelBuffer, members: Seq[ChannelBuffer])
    extends StrictKeyCommand {
  RequireClientProtocol(members != null && members.length > 0,
...
case class ZRevRange(key: ChannelBuffer,
                     start: Long,
                     stop: Long,
                     withScores: Option[CommandArgument] = None)
    extends ZRangeCmd {
...
case class ZRevRangeByScore(key: ChannelBuffer,
                            max: ZInterval,
                            min: ZInterval,
                            withScores: Option[CommandArgument] = None,
                            limit: Option[Limit] = None)
...
object ZScore {
  def apply(args: Seq[Array[Byte]]) = {
    ...
    new ZScore(ChannelBuffers.wrappedBuffer(args(0)),
               ChannelBuffers.wrappedBuffer(args(1)))
  }
}

case class ZUnionStore(destination: ChannelBuffer,
                       numkeys: Int,
                       keys: Seq[ChannelBuffer],
                       weights: Option[Weights] = None,
                       aggregate: Option[Aggregate] = None)
...
abstract class ZStore extends KeysCommand {
  ...
  override protected def validate() {
    ...
    weights match {
      case Some(list) =>
        RequireClientProtocol(list.size == numkeys,
...
trait ZScoredRangeCompanion { self =>
  ...
  def apply(args: Seq[Array[Byte]]) = args match {
    case key :: min :: max :: Nil =>
      get(ChannelBuffers.wrappedBuffer(key),
          ZInterval(min),
          ZInterval(max),
          None,
          None)
    case key :: min :: max :: tail =>
      parseArgs(ChannelBuffers.wrappedBuffer(key),
                ZInterval(min),
                ZInterval(max),
                tail)
    case _ =>
  ...
  protected def parseArgs(key: ChannelBuffer,
                          min: ZInterval,
                          max: ZInterval,
                          args: Seq[Array[Byte]]) = {
    RequireClientProtocol(args != null && !args.isEmpty,
    ...
    args.head match {
      ...
      case _ =>
        (Right(Limit(args.take(3))),
         if (args.length > 3) args.drop(3) else Nil)
    }
  }
...
trait ZRankCmdCompanion {
  ...
  def apply(args: Seq[Array[Byte]]) = {
    ...
    get(ChannelBuffers.wrappedBuffer(args(0)),
        ChannelBuffers.wrappedBuffer(args(1)))
  }
}
