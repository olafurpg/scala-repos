...
final class SortedSetClientServerIntegrationSuite
  ...
  private def zAdd(client: Service[Command, Reply],
                   key: String,
  ...
  test("ZINCRBY should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      ...
      assertMBulkReply(client(ZRange("zincrby1", 0, -1, WithScores)),
                       List("two", "2", "one", "3"))
    }
  }

  test("ZINTERSTORE and ZUNIONSTORE should work correctly",
       ClientServerTest,
       RedisTest) {
    withRedisClient { client =>
      val key = "zstore1"
      ...
      assert(
          Await.result(client(ZInterStore("out",
                                          List(key, key2),
                                          Weights(2, 3)))) == IntegerReply(2))
      assertMBulkReply(client(ZRange("out", 0, -1, WithScores)),
                       List("one", "5", "two", "10"))

      assert(
          Await.result(client(ZUnionStore("out",
                                          List(key, key2),
                                          Weights(2, 3)))) == IntegerReply(3))
      assertMBulkReply(client(ZRange("out", 0, -1, WithScores)),
                       List("one", "5", "three", "9", "two", "10"))
    }
  }

  test("ZRANGE and ZREVRANGE should work correctly",
       ClientServerTest,
       RedisTest) {
    withRedisClient { client =>
      zAdd(client,
      ...
      assertMBulkReply(client(ZRange("zrange1", 0, -1)),
                       List("one", "two", "three"))
      assertMBulkReply(client(ZRange("zrange1", 0, -1, WithScores)),
                       List("one", "1", "two", "2", "three", "3"))
      assertMBulkReply(client(ZRange("zrange1", 2, 3)), List("three"))
      assertMBulkReply(client(ZRange("zrange1", 2, 3, WithScores)),
                       List("three", "3"))
      assertMBulkReply(client(ZRange("zrange1", -2, -1)), List("two", "three"))
      assertMBulkReply(client(ZRange("zrange1", -2, -1, WithScores)),
                       List("two", "2", "three", "3"))

      assertMBulkReply(client(ZRevRange("zrange1", 0, -1)),
                       List("three", "two", "one"))
      assertMBulkReply(client(ZRevRange("zrange1", 2, 3)), List("one"))
      assertMBulkReply(client(ZRevRange("zrange1", -2, -1)),
                       List("two", "one"))
    }
  }

  test("ZRANGEBYSCORE and ZREVRANGEBYSCORE should work correctly",
       ClientServerTest,
       RedisTest) {
    withRedisClient { client =>
      ...
      assertMBulkReply(
          ...
          List("two"))
      assertMBulkReply(client(ZRangeByScore(key,
      ...
      assertMBulkReply(
          ...
          List("two"))
      assertMBulkReply(client(ZRevRangeByScore(key,
                                               ZInterval.exclusive(2f),
                                               ZInterval.exclusive(1f))),
                       List())
    }
