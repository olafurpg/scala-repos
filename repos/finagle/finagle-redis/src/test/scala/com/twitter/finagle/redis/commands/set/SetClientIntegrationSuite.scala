...
final class SetClientIntegrationSuite extends RedisClientTest {
  ...
  test("Correctly add, then pop members of a set", RedisTest, ClientTest) {
    withRedisClient { client =>
      ...
      assert(Await.result(client.sAdd(key, List(baz))) == oneElemAdded,
             oneElemAddErrorMessage)
  ...
  test("Correctly add and pop members from a set, while looking at the set",
    ...
    withRedisClient { client =>
      ...
      assert(Await.result(client.sIsMember(key, baz)) == false,
             "Baz was found in the set")
      assert(Await.result(client.sAdd(key, List(baz))) == oneElemAdded,
             oneElemAddErrorMessage)
      assert(Await.result(client.sIsMember(key, baz)) == true,
  ...
  test("Correctly add members to a set, then remove them",
    ...
    withRedisClient { client =>
      ...
      assert(Await.result(client.sRem(key, List(moo))) == 1,
             "Could not remove one Element")
      assert(Await.result(client.sRem(key, List(boo))) == 1,
             "Could not remove one Element")
  ...
  test("Correctly add member to a set, and return random",
    ...
    withRedisClient { client =>
      ...
      assert(twoMembers.size == 2,
             "The two member set had an incorrect number of members")
      assert(twoMembers.forall(allMembers.contains(_)) == true,
      ...
      assert(setMembers.forall(allMembers.contains(_)) == true,
