...
import com.twitter.util.{
  Future, Time, MockTimer, Promise, Return, Duration, Timer
}
...
class ExpiringServiceTest extends FunSuite with MockitoSugar {
  ...
  class ReleasingExpiringService[Req, Rep](self: Service[Req, Rep],
                                           maxIdleTime: Option[Duration],
                                           maxLifeTime: Option[Duration],
                                           timer: Timer,
                                           stats: StatsReceiver)
      extends ExpiringService[Req, Rep](self,
                                        maxIdleTime,
                                        maxLifeTime,
                                        timer,
                                        stats) {
  ...
  test("cancelling timers on release") {
    Time.withTimeAt(frozenNow) { _ =>
      ...
      val service = new ReleasingExpiringService[Any, Any](underlying,
                                                           Some(10.seconds),
                                                           Some(5.seconds),
                                                           timer,
                                                           NullStatsReceiver)
