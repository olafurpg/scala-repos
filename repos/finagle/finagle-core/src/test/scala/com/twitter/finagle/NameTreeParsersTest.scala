...
class NameTreeParsersTest extends FunSuite with AssertionsForJUnit
  ...
  test("parseNameTree")
    ...
    assert(NameTreeParsers.parseNameTree("/foo/bar") == NameTree.Leaf(
            Path.Utf8("foo", "bar")))
    assert(
    ...
    assert(NameTreeParsers.parseNameTree("  /foo | /bar  ") == NameTree.Alt(
            NameTree.Leaf(Path.Utf8("foo")), NameTree.Leaf(Path.Utf8("bar"))))
    assert(
        NameTreeParsers.parseNameTree("/foo & /bar | /bar & /baz") == NameTree
          .Alt(
    ...
    assert(
        NameTreeParsers.parseNameTree(
            ...
            NameTree.Union(NameTree.Weighted(0.5D,
                                             NameTree.Leaf(Path.Utf8("bar"))),
                           NameTree.Weighted(
                               0.5D, NameTree.Leaf(Path.Utf8("baz"))))))
