...
class ReceiverSchedulingPolicySuite extends SparkFunSuite {
  ...
  test("rescheduleReceiver: empty executors") {
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(0,
                                                  None,
                                                  Map.empty,
                                                  executors = Seq.empty)
  ...
  test("rescheduleReceiver: receiver preferredLocation") {
    ...
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(0,
                                                  ...
                                                  executors)
    assert(
      scheduledLocations.toSet === Set(HostTaskLocation("host1"),
                                       executors(0)))
  }
  ...
  test(
    ...
    val receiverTrackingInfoMap = Map(
      0 -> ReceiverTrackingInfo(0,
                                ReceiverState.ACTIVE,
                                None,
                                Some(executors(0))))
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(1,
                                                  None,
                                                  receiverTrackingInfoMap,
                                                  executors)
    ...
    val receiverTrackingInfoMap =
      Map(
        ...
        1 -> ReceiverTrackingInfo(
          ...
          Some(
            Seq(ExecutorCacheTaskLocation("host2", "2"),
                ExecutorCacheTaskLocation("host3", "3"))),
          None),
        2 -> ReceiverTrackingInfo(
          ...
          ReceiverState.SCHEDULED,
          Some(
            Seq(ExecutorCacheTaskLocation("host1", "1"),
                ExecutorCacheTaskLocation("host3", "3"))),
          None),
        3 -> ReceiverTrackingInfo(
          ...
          ReceiverState.SCHEDULED,
          Some(
            Seq(ExecutorCacheTaskLocation("host4", "4"),
                ExecutorCacheTaskLocation("host5", "5"))),
          None)
      )
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(4,
                                                  None,
                                                  receiverTrackingInfoMap,
                                                  executors)
