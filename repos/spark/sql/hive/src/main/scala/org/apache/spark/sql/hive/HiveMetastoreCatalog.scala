...
private[hive] class HiveMetastoreCatalog(val client: HiveClient,
                                         hive: HiveContext)
    extends Catalog
    with Logging {
  ...
  def createDataSourceTable(tableIdent: TableIdentifier,
    ...
    def newHiveCompatibleMetastoreTable(relation: HadoopFsRelation,
  ...
  private def convertToParquetRelation(
    ...
    val tableIdentifier = QualifiedTableName(metastoreRelation.databaseName,
                                             metastoreRelation.tableName)
    ...
    override def apply(plan: LogicalPlan): LogicalPlan = {
      ...
      plan.transformUp {
        ...
        case InsertIntoTable(r: MetastoreRelation,
          ...
          InsertIntoTable(parquetRelation,
                          partition,
                          child,
                          overwrite,
                          ifNotExists)
...
private[hive] case class MetastoreRelation(databaseName: String,
                                           tableName: String,
                                           alias: Option[String])(
