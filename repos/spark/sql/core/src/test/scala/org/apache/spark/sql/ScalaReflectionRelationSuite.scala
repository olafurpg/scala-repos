...
case class ReflectData(stringField: String,
                       intField: Int,
                       longField: Long,
                       floatField: Float,
                       doubleField: Double,
...
case class NullReflectData(intField: java.lang.Integer,
...
case class OptionalReflectData(intField: Option[Int],
...
case class Data(array: Seq[Int],
                arrayContainsNull: Seq[Option[Int]],
                map: Map[Int, Long],
                mapContainsNul: Map[Int, Option[Long]],
                nested: Nested)

case class ComplexReflectData(arrayField: Seq[Int],
...
class ScalaReflectionRelationSuite
  ...
  test("query case class RDD with Nones") {
    ...
    assert(
      sql("SELECT * FROM reflectOptionalData").collect().head === Row.fromSeq(
        Seq.fill(7)(null)))
  }
