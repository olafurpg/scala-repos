...
case class SetCommand(kv: Option[(String, Option[String])])
  ...
  private val (_output, runFunc): (Seq[Attribute],
  SQLContext => Seq[Row]) = kv match {
    ...
    case Some((key, None)) =>
      val runFunc = (sqlContext: SQLContext) =>
        {
          val value = try sqlContext.getConf(key) catch {
...
case class DescribeCommand(table: TableIdentifier,
