...
case class TestFunction(children: Seq[Expression],
...
class AnalysisErrorSuite extends AnalysisTest {
    ...
    def checkDataType(dataType: DataType, shouldSuccess: Boolean): Unit = {
      val plan = Aggregate(
        ...
        Alias(sum(AttributeReference("b", IntegerType)(exprId = ExprId(1))),
              "c")() :: Nil,
        LocalRelation(AttributeReference("a", dataType)(exprId = ExprId(2)),
  ...
  test("we should fail analysis when we find nested aggregate functions") {
    val plan = Aggregate(
      ...
      Alias(sum(sum(AttributeReference("b", IntegerType)(exprId = ExprId(1)))),
            "c")() :: Nil,
      LocalRelation(AttributeReference("a", IntegerType)(exprId = ExprId(2)),
                    AttributeReference("b", IntegerType)(exprId = ExprId(1)))
    )
  ...
  test("Join can't work on binary and map types") {
    val plan = Join(
      LocalRelation(AttributeReference("a", BinaryType)(exprId = ExprId(2)),
                    AttributeReference("b", IntegerType)(exprId = ExprId(1))),
      LocalRelation(AttributeReference("c", BinaryType)(exprId = ExprId(4)),
                    AttributeReference("d", IntegerType)(exprId = ExprId(3))),
      Inner,
      Some(
        EqualTo(AttributeReference("a", BinaryType)(exprId = ExprId(2)),
    ...
    val plan2 = Join(
      LocalRelation(AttributeReference("a", MapType(IntegerType, StringType))(
                      exprId = ExprId(2)),
                    AttributeReference("b", IntegerType)(exprId = ExprId(1))),
      LocalRelation(AttributeReference("c", MapType(IntegerType, StringType))(
                      exprId = ExprId(4)),
                    AttributeReference("d", IntegerType)(exprId = ExprId(3))),
      Inner,
      Some(
        EqualTo(AttributeReference("a", MapType(IntegerType, StringType))(
                  exprId = ExprId(2)),
