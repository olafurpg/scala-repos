...
abstract class CatalogTestCases extends SparkFunSuite with BeforeAndAfterEach {
  ...
  test("create partitions when database/table does not exist") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.createPartitions("does_not_exist",
                               "tbl1",
                               Seq(),
                               ignoreIfExists = false)
    }
  ...
  test("drop partitions when database/table does not exist") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.dropPartitions("does_not_exist",
                             "tbl1",
                             Seq(),
                             ignoreIfNotExists = false)
    }
  ...
  test("rename partitions") {
    ...
    assert(
      catalog
        .getPartition("db2", "tbl2", newPart2.spec)
        .spec === newPart2.spec)
  ...
  test("rename partitions when database/table does not exist") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.renamePartitions("does_not_exist",
                               "tbl1",
                               Seq(part1.spec),
                               Seq(part2.spec))
    }
  ...
  test("list functions") {
    ...
    assert(
      catalog.listFunctions("db2", "*").toSet == Set("func1",
                                                     "func2",
                                                     "not_me"))
    assert(
