...
https://cwiki.apache.org/confluence/display/Hive/Enhanced+Aggregation%2C+Cube%2C+Grouping+and+Rollup
        ...
        case Token("TOK_INSERT", singleInsert) =>
          ...
          val withSort = (orderByClause,
            ...
            case (None, None, Some(partitionExprs), None) =>
              RepartitionByExpression(partitionExprs.children.map(nodeToExpr),
                                      withDistinct)
            case (None,
  ...
  protected def nodeToRelation(node: ASTNode): LogicalPlan = {
    node match {
      ...
      case Token(laterViewToken(isOuter),
  ...
  protected def nodeToDest(node: ASTNode,
                           query: LogicalPlan,
    ...
    node match {
      ...
      case Token(destinationToken(),
                 Token("TOK_TAB", tableArgs) :: Token("TOK_IFNOTEXISTS",
                                                      ifNotExists) :: Nil) =>
        val Some(tableNameParts) :: partitionClause :: Nil =
          getClauses(Seq("TOK_TABNAME", "TOK_PARTSPEC"), tableArgs)
  ...
  /**
    ...
    */
  private def flattenLeftDeepTree(node: ASTNode,
  ...
  /**
    ...
    */
  private def balancedTree(expr: Seq[Expression],
                           f: (Expression,
  ...
  protected def nodesToWindowSpecification(nodes: Seq[ASTNode]): WindowSpec =
    nodes match {
      ...
      case spec =>
        ...
        // Handle Window Frame
        val windowFrame = if (rowFrame.isEmpty && rangeFrame.isEmpty) {
          UnspecifiedFrame
        } else {
  ...
  protected def nodeToGenerate(node: ASTNode,
                               outer: Boolean,
