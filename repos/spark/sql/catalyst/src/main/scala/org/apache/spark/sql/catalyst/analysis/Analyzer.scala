...
import org.apache.spark.sql.catalyst.{
  CatalystConf, ScalaReflection, SimpleCatalystConf
}
  ...
  object ResolveGroupingAnalytics extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
      ...
      case x: GroupingSets if x.expressions.forall(_.resolved) =>
        ...
        Aggregate(groupByAttributes :+ VirtualColumn.groupingIdAttribute,
                  aggregations,
                  Expand(x.bitmasks,
                         groupByAliases,
                         groupByAttributes,
  ...
  private def commonNaturalJoinProcessing(left: LogicalPlan,
    ...
    val projectList = joinType match {
      ...
      case FullOuter =>
        ...
        joinedCols ++ lUniqueOutput
          .map(_.withNullability(true)) ++ rUniqueOutput.map(
