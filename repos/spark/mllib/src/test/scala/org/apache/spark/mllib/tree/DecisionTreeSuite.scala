...
class DecisionTreeSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("Second level node building with vs. without groups") {
    ...
    val strategyOneNode = new Strategy(Classification,
                                       Entropy,
                                       maxDepth = 1,
                                       numClasses = 2,
                                       maxBins = 100)
    ...
    val treeToNodeToIndexInfo = Map(
        (0,
         Map((rootNode1.leftNode.get.id,
              new RandomForest.NodeIndexInfo(0, None)),
    ...
    val treeToNodeToIndexInfoB = Map(
        (0,
         Map((rootNode2.rightNode.get.id,
              new RandomForest.NodeIndexInfo(0, None)))))
    nodeQueue.clear()
    DecisionTree.findBestSplits(baggedInput,
                                metadata,
...
object DecisionTreeSuite extends SparkFunSuite {
  ...
  /**
    ...
    */
  private[mllib] def checkEqual(a: DecisionTreeModel,
  ...
  private def checkEqual(a: Node, b: Node): Unit = {
    ...
    (a.leftNode, b.leftNode) match {
      ...
      case (None, None) =>
      case _ =>
        throw new AssertionError("Only one instance has leftNode defined. " +
    ...
    (a.rightNode, b.rightNode) match {
      ...
      case (None, None) =>
      case _ =>
        throw new AssertionError("Only one instance has rightNode defined. " +
