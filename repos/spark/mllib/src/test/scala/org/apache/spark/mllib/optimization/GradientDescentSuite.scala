...
import org.apache.spark.mllib.util.{
  LocalClusterSparkContext, MLlibTestSparkContext, MLUtils
}
...
class GradientDescentSuite
  ...
  test("Test the loss and gradient of first iteration with regularization.") {
    ...
    assert(
        loss1(0) ~= (loss0(0) + (math.pow(initialWeightsWithIntercept(0), 2) +
                  math
                    .pow(initialWeightsWithIntercept(1), 2)) / 2) absTol 1E-5,
  ...
  test("iteration should end with convergence tolerance") {
    ...
    assert(loss.length < numIterations,
...
class GradientDescentClusterSuite
  ...
  test("task size should be small") {
    ...
    val (weights, loss) =
      GradientDescent.runMiniBatchSGD(points,
