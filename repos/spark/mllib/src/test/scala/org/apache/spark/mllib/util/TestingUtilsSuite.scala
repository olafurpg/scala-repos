...
class TestingUtilsSuite extends SparkFunSuite
  ...
  test("Comparing doubles using relative error.")
    ...
    assert(10 * Double.MinPositiveValue ~==
          9.5 * Double.MinPositiveValue relTol 0.01)
    assert(10 * Double.MinPositiveValue !~==
  ...
  test("Comparing vectors using relative error.")
    ...
    // Should throw exception with message when test fails.
    intercept[TestFailedException](Vectors.dense(Array(3.1, 3.5)) !~==
          Vectors.dense(Array(3.130, 3.534)) relTol 0.01)
    ...
    // Comparing against zero should fail the test and throw exception with message
    // saying that the relative error is meaningless in this situation.
    intercept[TestFailedException](Vectors.dense(Array(3.1, 0.01)) ~==
          Vectors.dense(Array(3.13, 0.0)) relTol 0.01)
  ...
  test("Comparing vectors using absolute error.")
    ...
    assert(!(Vectors.dense(Array(3.1, 3.5, 0.0)) ~= Vectors.dense(
    ...
    // Comparisons of two sparse vectors
    assert(Vectors.sparse(3, Array(0, 2), Array(3.1, 2.4)) ~== Vectors.sparse(
            3, Array(0, 2), Array(3.1 + 1E-8, 2.4 + 1E-7)) absTol 1E-6)
