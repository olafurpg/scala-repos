...
class SparkJLineCompletion(val intp: SparkIMain)
  ...
  trait CompilerCompletion {
    ...
    def members =
      afterTyper(
        ((effectiveTp.nonPrivateMembers.toList ++ anyMembers))
          .filter(_.isPublic))
  ...
  class TypeMemberCompletion(val tp: Type)
    ...
    override def follow(s: String): Option[CompletionAware] =
      debugging(tp + " -> '" + s + "' ==> ")(
        Some(TypeMemberCompletion(memberNamed(s).tpe))
          .filterNot(_ eq NoTypeCompletion))

    override def alternativesFor(id: String): List[String] =
      debugging(id + " alternatives ==> ") {
        val alts =
          members
            .filter(x => x.isMethod && tos(x) == id)
            .map(methodSignatureString)
      ...
      if (!isReplPower) default
        ...
        intp.runtimeClassAndTypeOfTerm(id) match {
          case Some((clazz, runtimeType)) =>
            ...
            if (sym.isStable) {
              val param =
                new NamedParam.Untyped(id,
                                       intp.valueOfTerm(id).getOrElse(null))
