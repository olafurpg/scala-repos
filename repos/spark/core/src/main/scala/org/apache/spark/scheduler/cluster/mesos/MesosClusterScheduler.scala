...
import org.apache.spark.deploy.rest.{
  CreateSubmissionResponse, KillSubmissionResponse, SubmissionStatusResponse
}
      ...
      if (offerOption.isEmpty) {
        ...
        submission.schedulerProperties
          ...
          .foreach { image =>
            ...
            MesosSchedulerBackendUtil.addDockerInfo(container,
                                                    image,
                                                    volumes = volumes,
                                                    portmaps = portmaps)
      ...
      if (launchedDrivers.contains(taskId)) {
        ...
        if (state.driverDescription.supervise &&
          ...
          val newDriverDescription = state.driverDescription.copy(
              retryState = Some(
                  new MesosClusterRetryState(status,
                                             retries,
                                             nextRetry,
                                             waitTimeSec)))
          ...
          pendingRetryDriversState.persist(taskId, newDriverDescription)
                     ...
                     .isFinished(TaskState.fromMesos(status.getState))) {
          removeFromLaunchedDrivers(taskId)
          state.finishDate = Some(new Date())
