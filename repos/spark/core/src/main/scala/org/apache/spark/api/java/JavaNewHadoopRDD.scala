...
class JavaNewHadoopRDD[K, V](
    rdd: NewHadoopRDD[K, V])(implicit override val kClassTag: ClassTag[K],
  ...
  def mapPartitionsWithInputSplit[R](
      f: JFunction2[
          InputSplit, java.util.Iterator[(K, V)], java.util.Iterator[R]],
      preservesPartitioning: Boolean = false): JavaRDD[R] = {
    new JavaRDD(
        rdd.mapPartitionsWithInputSplit((a, b) => f.call(a, b.asJava).asScala,
