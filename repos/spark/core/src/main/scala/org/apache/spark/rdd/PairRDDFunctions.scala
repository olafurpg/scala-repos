...
/**
  ...
  */
class PairRDDFunctions[K, V](self: RDD[(K, V)])(implicit kt: ClassTag[K],
                                                vt: ClassTag[V],
                                                ord: Ordering[K] = null)
    extends Logging
    with Serializable {
  ...
  def sampleByKey(withReplacement: Boolean,
    ...
    self.withScope {
      ...
      require(fractions.values.forall(v => v >= 0.0),
  ...
  /**
    ...
    */
  def countApproxDistinctByKey(p: Int,
                               sp: Int,
  ...
  def groupByKey(partitioner: Partitioner): RDD[(K, Iterable[V])] =
    self.withScope {
      ...
      val bufs = combineByKeyWithClassTag[CompactBuffer[V]](createCombiner,
                                                            mergeValue,
                                                            mergeCombiners,
                                                            partitioner,
                                                            mapSideCombine =
  ...
  /**
    ...
    */
  def fullOuterJoin[W](other: RDD[(K, W)],
  ...
  /** Alias for cogroup. */
  def groupWith[W1, W2, W3](other1: RDD[(K, W1)],
                            other2: RDD[(K, W2)],
                            other3: RDD[(K, W3)])
    : RDD[(K, (Iterable[V], Iterable[W1], Iterable[W2], Iterable[W3]))] =
    self.withScope {
  ...
  private def maybeUpdateOutputMetrics(
      outputMetricsAndBytesWrittenCallback: Option[
          (OutputMetrics, () => Long)],
