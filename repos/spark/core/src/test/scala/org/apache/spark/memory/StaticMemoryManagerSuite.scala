...
class StaticMemoryManagerSuite extends MemoryManagerSuite {
  ...
  test("basic execution memory") {
    ...
    assert(mm
      .acquireExecutionMemory(10L, taskAttemptId, MemoryMode.ON_HEAP) === 10L)
    ...
    assert(mm
      .acquireExecutionMemory(1L, taskAttemptId, MemoryMode.ON_HEAP) === 0L)
    assert(mm.executionMemoryUsed === maxExecutionMem)
    mm.releaseExecutionMemory(800L, taskAttemptId, MemoryMode.ON_HEAP)
    assert(mm.executionMemoryUsed === 200L)
    // Acquire after release
    assert(mm
      .acquireExecutionMemory(1L, taskAttemptId, MemoryMode.ON_HEAP) === 1L)
  ...
  test("execution and storage isolation") {
    ...
    assert(
        mm.acquireExecutionMemory(1000L, taskAttemptId, MemoryMode.ON_HEAP) === 100L)
    assert(mm.storageMemoryUsed === 0L)
    assert(mm.executionMemoryUsed === 200L)
