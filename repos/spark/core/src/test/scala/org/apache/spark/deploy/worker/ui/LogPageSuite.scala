  ...
  test("get logs simple") {
    ...
    val (stdout, _, _, _) =
      logPage.invokePrivate(
        getLog(workDir.getAbsolutePath, "stdout", None, 100))
    val (stderr, _, _, _) =
      logPage.invokePrivate(
        getLog(workDir.getAbsolutePath, "stderr", None, 100))
    val (error1, _, _, _) =
      logPage.invokePrivate(
        getLog(workDir.getAbsolutePath, "random", None, 100))
    val (error2, _, _, _) =
      logPage.invokePrivate(
    ...
    val (error3, _, _, _) =
      logPage.invokePrivate(
        getLog(tmpDir.getAbsolutePath, "stderr", None, 100))
    val (error4, _, _, _) =
      logPage.invokePrivate(
        getLog(tmpDir.getAbsolutePath, "stdout", None, 100))
    assert(stdout === out)
    assert(stderr === err)
