...
class BlockManagerSuite
  ...
  test("master + 1 manager interaction") {
    ...
    assert(
      store.getSingleAndReleaseLock("a1").isDefined,
      "a1 was not in store")
    assert(
      store.getSingleAndReleaseLock("a2").isDefined,
      "a2 was not in store")
    assert(
      store.getSingleAndReleaseLock("a3").isDefined,
      "a3 was not in store")
  ...
  test("LRU with mixed storage levels") {
    ...
    assert(
      ...
      "a3 was not in store")
    assert(
      store.getSingleAndReleaseLock("a4").isDefined,
      "a4 was not in store")
  }
  ...
  test("in-memory LRU with streams") {
    ...
    store.putIterator(
      ...
      tellMaster = true)
    assert(
      store.getAndReleaseLock("list2").isDefined,
      "list2 was not in store")
    assert(store.get("list2").get.data.size === 2)
    assert(
      store.getAndReleaseLock("list3").isDefined,
    ...
    store.putIterator(
      ...
      tellMaster = true)
    assert(
      store.getAndReleaseLock("list1").isDefined,
  ...
  test("LRU with mixed storage levels and streams") {
    ...
    assert(store.get("list3").get.data.size === 2)
    assert(
      store.getAndReleaseLock("list4").isDefined,
  ...
  test("updated block statuses") {
    ...
    assert(
      store.memoryStore.contains("list3"),
    ...
    assert(
      store.memoryStore.contains("list4"),
