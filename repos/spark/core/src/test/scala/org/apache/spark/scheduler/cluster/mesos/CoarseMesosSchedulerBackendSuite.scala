...
class CoarseMesosSchedulerBackendSuite
  ...
  test("mesos doesn't register twice with the same shuffle service") {
    ...
    verify(externalShuffleClient, times(1)).registerDriverWithShuffleService(
      anyString,
      anyInt,
      anyLong,
      anyLong)
  ...
  private def offerResources(offers: List[(Int, Int)],
                             startId: Int = 1): Unit = {
  ...
  private def createTaskStatus(taskId: String,
                               slaveId: String,
