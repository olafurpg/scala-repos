...
import org.apache.spark.scheduler.{
  LiveListenerBus, SparkListenerExecutorAdded, TaskDescription,
  TaskSchedulerImpl, WorkerOffer
}
...
class MesosSchedulerBackendSuite
  ...
  test("Use configured mesosExecutor.cores for ExecutorInfo") {
    ...
    val resources =
      Arrays.asList(mesosSchedulerBackend.createResource("cpus", 4),
  ...
  test("check spark-class location correctly") {
    ...
    val (executorInfo, _) =
      mesosSchedulerBackend.createExecutorInfo(resources, "test-id")
    assert(
        executorInfo.getCommand.getValue === s" /mesos-home/bin/spark-class ${classOf[MesosExecutorBackend].getName}")
