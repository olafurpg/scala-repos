...
object BuildCommons {
  ...
  assembly, networkYarn, streamingKafkaAssembly, streamingKinesisAslAssembly) =
    Seq("assembly",
        "network-yarn",
...
object OldDeps {
  ...
  lazy val project = Project(
      "oldDeps", file("dev"), settings = oldDepsSettings)
  ...
  lazy val settings =
    assemblySettings ++ Seq(
        ...
        hadoopVersion := {
          sys.props
            .get("hadoop.version")
            .getOrElse(SbtPomKeys.effectivePom.value.getProperties
                  .get("hadoop.version")
                  .asInstanceOf[String])
        },
        jarName in assembly <<=
...
object PySparkAssembly {
  ...
  private def addFilesToZipStream(
      parent: String, source: File, output: ZipOutputStream): Unit = {
    if (source.isDirectory()) {
      output.putNextEntry(new ZipEntry(parent + source.getName()))
      for (file <- source.listFiles()) {
        addFilesToZipStream(
            parent + source.getName() + File.separator, file, output)
      }
    } else {
...
object Unidoc {
  ...
  private def ignoreUndocumentedPackages(
    ...
    packages
      ...
      .map(_.filterNot(
              _.getCanonicalPath.contains("org/apache/spark/network")))
      .map(_.filterNot(
              _.getCanonicalPath.contains("org/apache/spark/shuffle")))
      .map(_.filterNot(
    ...
    scalaJavaUnidocSettings ++ Seq(
        ...
        unidocProjectFilter in (ScalaUnidoc, unidoc) :=
          inAnyProject -- inProjects(
            OldDeps.project, repl, examples, tools, yarn, testTags),
...
object CopyDependencies {
  ...
  val copyDeps = TaskKey[Unit](
      "copyDeps", "Copies needed dependencies to the build directory.")
