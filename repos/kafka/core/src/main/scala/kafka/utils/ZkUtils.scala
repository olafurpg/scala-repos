...
object ZkUtils {
  ...
  def getTopicPartitionLeaderAndIsrPath(topic: String,
...
class ZkUtils(val zkClient: ZkClient,
  ...
  def registerBrokerInZk(
    ...
    info("Registered broker %d at path %s with addresses: %s"
  ...
  def getConsumerPartitionOwnerPath(group: String,
                                    topic: String,
  ...
  /**
    ...
    */
  def makeSurePersistentPathExists(path: String,
  ...
  /**
    ...
    */
  def updatePersistentPath(path: String,
                           data: String,
                           acls: java.util.List[ACL] = DefaultAcls) = {
    try {
  ...
  def getPartitionsForTopics(
    ...
    getPartitionAssignmentForTopics(topics).map { topicAndPartitionMap =>
      ...
      debug("partition assignment of /brokers/topics/%s is %s"
      ...
      case _ =>
        ...
        try {
          ...
          case nne: ZkNoNodeException =>
            ...
            debug("Created path %s with %s for partition reassignment"
    ...
    def processResult(rc: Int,
      ...
      Code.get(rc) match {
        ...
        case Code.INVALIDACL =>
          ...
          setResult(Code.INVALIDACL)
        case _ =>
          warn("ZooKeeper event while getting znode data: %s %s"
                .format(path, Code.get(rc)))
          setResult(Code.get(rc))
      }
    }
