...
import kafka.security.auth.{
  Authorizer, ClusterAction, Group, Create, Describe, Operation, Read,
  Resource, Topic, Write
}
import kafka.utils.{Logging, SystemTime, ZKGroupTopicDirs, ZkUtils}
import org.apache.kafka.common.errors.{
  InvalidTopicException, NotLeaderForPartitionException,
  UnknownTopicOrPartitionException, ClusterAuthorizationException
}
import org.apache.kafka.common.metrics.Metrics
import org.apache.kafka.common.protocol.{ApiKeys, Errors, SecurityProtocol}
import org.apache.kafka.common.requests.{
  ListOffsetRequest, ListOffsetResponse, GroupCoordinatorRequest,
  GroupCoordinatorResponse, ListGroupsResponse, DescribeGroupsRequest,
  DescribeGroupsResponse, HeartbeatRequest, HeartbeatResponse,
  JoinGroupRequest, JoinGroupResponse, LeaveGroupRequest, LeaveGroupResponse,
  ResponseHeader, ResponseSend, SyncGroupRequest, SyncGroupResponse,
  LeaderAndIsrRequest, LeaderAndIsrResponse, StopReplicaRequest,
  StopReplicaResponse, ProduceRequest, ProduceResponse, UpdateMetadataRequest,
  UpdateMetadataResponse, MetadataRequest, MetadataResponse,
...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleStopReplicaRequest(request: RequestChannel.Request) {
    ...
    val response = if (authorize(request.session,
                                 ClusterAction,
                                 Resource.ClusterResource)) {
      ...
      val result = stopReplicaRequest.partitions.asScala
        .map((_, new JShort(Errors.CLUSTER_AUTHORIZATION_FAILED.code)))
        .toMap
      new StopReplicaResponse(Errors.CLUSTER_AUTHORIZATION_FAILED.code,
                              result.asJava)
    }
  ...
  def handleUpdateMetadataRequest(request: RequestChannel.Request) {
    ...
    val updateMetadataResponse = if (authorize(request.session,
                                               ClusterAction,
                                               Resource.ClusterResource)) {
      replicaManager.maybeUpdateMetadataCache(correlationId,
  ...
  private def authorize(session: Session,
                        operation: Operation,
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    val unauthorizedPartitionData = unauthorizedRequestInfo.mapValues { _ =>
      FetchResponsePartitionData(Errors.TOPIC_AUTHORIZATION_FAILED.code,
                                 -1,
                                 MessageSet.Empty)
    }
    ...
    def sendResponseCallback(
        responsePartitionData: Map[TopicAndPartition,
    ...
    val responseMap = authorizedRequestInfo.map(elem => {
      ...
      try {
        ...
        case e: Throwable =>
          ...
          (topicPartition,
           new ListOffsetResponse.PartitionData(Errors.forException(e).code,
                                                List[JLong]().asJava))
      }
    })
  ...
  private def fetchOffsetsBefore(log: Log,
                                 timestamp: Long,
                                 maxNumOffsets: Int): Seq[Long] = {
      ...
      if (!authorize(request.session,
        ...
        if (header.apiVersion == 0) {
          ...
          val offsets = coordinator
            .handleFetchOffsets(offsetFetchRequest.groupId,
  ...
  def handleSyncGroupRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(request.session,
      ...
      sendResponseCallback(Array[Byte](),
                           Errors.GROUP_AUTHORIZATION_FAILED.code)
    } else {
