...
import kafka.message.{
  ByteBufferMessageSet, InvalidMessageException, Message, MessageSet
}
import kafka.metrics.KafkaMetricsGroup
import kafka.utils._
import org.apache.kafka.common.errors.{
  OffsetOutOfRangeException, RecordBatchTooLargeException,
  ReplicaNotAvailableException, RecordTooLargeException, InvalidTopicException,
  ControllerMovedException, NotLeaderForPartitionException,
  CorruptRecordException, UnknownTopicOrPartitionException,
...
import org.apache.kafka.common.requests.{
  LeaderAndIsrRequest, StopReplicaRequest, UpdateMetadataRequest
}
...
class ReplicaManager(val config: KafkaConfig,
  ...
  def tryCompleteDelayedProduce(key: DelayedOperationKey) {
    ...
    debug(
        "Request key %s unblocked %d producer requests.".format(key.keyLabel,
                                                                completed))
  }
  ...
  def tryCompleteDelayedFetch(key: DelayedOperationKey) {
    ...
    debug(
        "Request key %s unblocked %d fetch requests.".format(key.keyLabel,
                                                             completed))
  }
  ...
  def appendMessages(
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      val localProduceResults = appendToLocalLog(internalTopicsAllowed,
                                                 ...
                                                 requiredAcks)
      debug(
          "Produce to local log in %d ms".format(
      ...
      if (delayedRequestRequired(requiredAcks,
                                 messagesPerPartition,
                                 localProduceResults)) {
    ...
    try {
        ...
        case (partition, partitionStateInfo) =>
          ...
          metadataCache.getAliveBrokers.find(_.id == newLeaderBrokerId) match {
            ...
            case Some(leaderBroker) =>
              if (partition.makeFollower(controllerId,
                                         partitionStateInfo,
                                         correlationId))
      ...
      partitionsToMakeFollower.foreach { partition =>
        val topicPartitionOperationKey =
          new TopicPartitionOperationKey(partition.topic,
                                         partition.partitionId)
