...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean)
  ...
  private def getMap(propName: String,
    ...
    try {
      ...
      case e: Exception =>
        throw new IllegalArgumentException(
          "Error parsing configuration property '%s': %s".format(propName,
                                                                 e.getMessage))
    }
  }
  ...
  private def validateUniquePortAndProtocol(listeners: String) {
    ...
    val endpoints = try {
      ...
      case e: Exception =>
        throw new IllegalArgumentException(
          "Error creating broker listeners from '%s': %s".format(listeners,
                                                                 e.getMessage))
    }
