...
class PartitionStateMachine(controller: KafkaController) extends Logging {
  ...
  def handleStateChanges(
    ...
    info("Invoking state change to %s for partitions %s"
  ...
  private def initializeLeaderAndIsrForPartition(
    ...
    liveAssignedReplicas.size match {
      ...
      case _ =>
        debug("Live assigned replicas for partition %s are: [%s]"
        ...
        val leaderIsrAndControllerEpoch = new LeaderIsrAndControllerEpoch(
            ...
            controller.epoch)
        debug("Initializing leader and isr for partition %s to %s"
        ...
        try {
          ...
          case e: ZkNodeExistsException =>
            ...
            stateChangeLogger.error(
                "Controller %d epoch %d ".format(controllerId,
