...
class Partition(val topic: String,
  ...
  /**
    ...
    */
  def updateReplicaLogReadResult(replicaId: Int,
  ...
  def checkEnoughReplicasReachOffset(requiredOffset: Long): (Boolean, Short) = {
    leaderReplicaIfLocal() match {
      case Some(leaderReplica) =>
        ...
        val numAcks = curInSyncReplicas.count(r => {
          if (!r.isLocal)
            if (r.logEndOffset.messageOffset >= requiredOffset) {
              trace("Replica %d of %s-%d received offset %d"
        ...
        trace("%d acks satisfied for %s-%d with acks = -1"
  ...
  private def maybeIncrementLeaderHW(leaderReplica: Replica): Boolean = {
    ...
    if (oldHighWatermark.messageOffset < newHighWatermark.messageOffset ||
      ...
      debug("High watermark for partition [%s,%d] updated to %s"
  ...
  private def updateIsr(newIsr: Set[Replica]) {
    ...
    if (updateSucceeded) {
      ...
      trace("ISR updated to [%s] and zkVersion updated to [%d]"
