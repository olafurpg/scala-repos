    ...
    def printStats() {
      ...
      println(
          "Error count: %d; Max:%f; Min: %f; Mean: %f; Commit count: %d"
            .format(commitThreads.map(_.numErrors.get).sum,
  ...
  class CommitThread(id: Int,
    ...
    override def doWork() {
      val commitRequest = OffsetCommitRequest(
          groupId,
          immutable.Map((1 to partitionCount).map(TopicAndPartition(
                      "topic-" + id,
                      _) -> OffsetAndMetadata(offset, metadata)): _*))
      try {
        ensureConnected()
    ...
    override def doWork() {
      ...
      try {
        ...
        try {
          ...
          case e2: IOException =>
            println("Error while fetching offset from %s:%d due to %s."
