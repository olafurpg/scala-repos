...
class PlaintextConsumerTest extends BaseConsumerTest {
  ...
  def testPatternSubscription() {
    ...
    subscriptions ++= Set(new TopicPartition(topic4, 0),
  ...
  def testPositionAndCommit() {
    ...
    consumeAndVerifyRecords(consumer = this.consumers(1),
                            numRecords = 1,
                            startingOffset = 5)
  }
  ...
  def testMultiConsumerRoundRobinAssignment() {
    ...
    val (rrConsumers, consumerPollers) =
      createConsumerGroupAndWaitForAssignment(10,
                                              List(topic1, topic2),
                                              subscriptions)

    // add one more consumer and validate re-assignment
    addConsumersToGroupAndWaitForGroupAssignment(1,
                                                 rrConsumers,
                                                 consumerPollers,
                                                 List(topic1, topic2),
                                                 subscriptions)
  ...
  def testMultiConsumerDefaultAssignment() {
    ...
    sendRecords(100, tp2)
    val topic1 = "topic1"
    val subscriptions = Set(tp, tp2) ++ createTopicAndSendRecords(topic1,
                                                                  5,
                                                                  100)
    ...
    addConsumersToGroupAndWaitForGroupAssignment(2,
                                                 consumers,
                                                 consumerPollers,
                                                 List(topic, topic1),
                                                 subscriptions)
  ...
  def testInterceptors() {
    ...
    val testProducer = new KafkaProducer[String, String](producerProps,
    ...
    for (i <- 0 until numRecords) {
      ...
      assertEquals(s"value $i$appendStr".toUpperCase,
  ...
  def runMultiConsumerSessionTimeoutTest(closeConsumer: Boolean): Unit = {
    ...
    for (consumer <- consumers)
      consumerPollers += subscribeConsumerAndStartPolling(consumer,
                                                          List(topic, topic1))
    ...
    val expandedConsumers = consumers ++ Buffer[KafkaConsumer[Array[Byte],
                                                              Array[Byte]]](
        timeoutConsumer)
    val timeoutPoller =
  ...
  def addConsumersToGroupAndWaitForGroupAssignment(
    ...
    for (i <- 0 until numOfConsumersToAdd) {
      ...
      consumerPollers += subscribeConsumerAndStartPolling(newConsumer,
                                                          topicsToSubscribe)
    }
