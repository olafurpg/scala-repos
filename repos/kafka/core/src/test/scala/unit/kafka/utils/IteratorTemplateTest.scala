...
class IteratorTemplateTest extends Assertions {
  ...
  def testIterator() {
    for (i <- 0 until 10) {
      ...
      assertEquals("Checking again shouldn't change anything.",
                   true,
                   iterator.hasNext)
      assertEquals("Peeking at the item should show the right thing.",
                   i,
                   iterator.peek)
      assertEquals("Peeking again shouldn't change anything", i, iterator.peek)
      assertEquals("Getting the item should give the right thing.",
                   i,
                   iterator.next)
    }
