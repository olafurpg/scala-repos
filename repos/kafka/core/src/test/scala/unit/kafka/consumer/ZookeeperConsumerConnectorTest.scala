...
class ZookeeperConsumerConnectorTest
  ...
  def testCompression() {
    ...
    // send some messages to each broker
    val sentMessages1 = sendMessages(servers,
                                     topic,
                                     nMessages,
                                     0,
    ...
    val topicMessageStreams2 = zkConsumerConnector2.createMessageStreams(
      ...
      new StringDecoder())
    // send some messages to each broker
    val sentMessages2 = sendMessages(servers,
                                     topic,
                                     nMessages,
                                     0,
                                     GZIPCompressionCodec) ++
    ...
    val topicMessageStreams3 = zkConsumerConnector3.createMessageStreams(
      ...
      new StringDecoder())
    // send some messages to each broker
    val sentMessages3 = sendMessages(servers,
                                     topic,
                                     nMessages,
                                     0,
                                     GZIPCompressionCodec) ++
  ...
  def testCompressionSetConsumption() {
    // send some messages to each broker
    val sentMessages = sendMessages(servers,
                                    topic,
                                    200,
                                    0,
  ...
  def testConsumerDecoder() {
    ...
    val topicMessageStreams =
      zkConsumerConnector.createMessageStreams(Map(topic -> 1),
                                               new StringDecoder(),
                                               new StringDecoder())
  ...
  private class TestConsumerRebalanceListener
    ...
    var partitionOwnership: java.util.Map[String,
    ...
    override def beforeReleasingPartitions(
        partitionOwnership: java.util.Map[String,
                                          java.util.Set[java.lang.Integer]]) {
