...
class OffsetIndexTest extends JUnitSuite {
  ...
  def testReopen() {
    ...
    assertWriteFails("Append should fail on read-only index",
                     idxRo,
                     53,
                     classOf[IllegalArgumentException])
  }
  ...
  def truncate() {
    ...
    idx.append(10, 10)
    idx.truncateTo(10)
    assertEquals("Index should be unchanged by truncate at the end",
                 OffsetPosition(9, 9),
                 idx.lookup(10))
    assertEquals("9 should be the last entry in the index", 9, idx.lastOffset)
    idx.append(10, 10)
    ...
    idx.truncateTo(9)
    assertEquals("Index should truncate off last entry",
                 OffsetPosition(8, 8),
                 idx.lookup(10))
    ...
    idx.truncateTo(5)
    assertEquals("4 should be the last entry in the index",
                 OffsetPosition(4, 4),
                 idx.lookup(10))
