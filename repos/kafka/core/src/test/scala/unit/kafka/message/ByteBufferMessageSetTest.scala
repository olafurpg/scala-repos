...
class ByteBufferMessageSetTest extends BaseMessageSetTestCases {
  ...
  def testEquals() {
    var messages = new ByteBufferMessageSet(DefaultCompressionCodec,
                                            new Message("hello".getBytes),
                                            new Message("there".getBytes))
    var moreMessages = new ByteBufferMessageSet(DefaultCompressionCodec,
                                                new Message("hello".getBytes),
                                                new Message("there".getBytes))

    assertTrue(messages.equals(moreMessages))

    messages = new ByteBufferMessageSet(NoCompressionCodec,
                                        new Message("hello".getBytes),
                                        new Message("there".getBytes))
    moreMessages = new ByteBufferMessageSet(NoCompressionCodec,
                                            new Message("hello".getBytes),
                                            new Message("there".getBytes))
  ...
  def testLogAppendTime() {
    ...
    validatedMessages.foreach(
        messageAndOffset => validateLogAppendTime(messageAndOffset.message))

    assertEquals("message set size should not change",
                 compressedMessagesWithRecompresion.size,
                 validatedCompressedMessages.size)
    validatedCompressedMessages.foreach(
        messageAndOffset => validateLogAppendTime(messageAndOffset.message))
    assertTrue(
    ...
    validatedCompressedMessagesWithoutRecompression.foreach(
        messageAndOffset => validateLogAppendTime(messageAndOffset.message))
  ...
  def testCreateTime() {
    ...
    val messages = getMessages(magicValue = Message.MagicValue_V1,
                               timestamp = now,
                               codec = NoCompressionCodec)
    val compressedMessages = getMessages(magicValue = Message.MagicValue_V1,
                                         timestamp = now,
                                         codec = DefaultCompressionCodec)
