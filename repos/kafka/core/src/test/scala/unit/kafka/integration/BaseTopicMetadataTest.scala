...
abstract class BaseTopicMetadataTest extends ZooKeeperTestHarness
  ...
  def testBasicTopicMetadata
    ...
    assertEquals(
        "Expecting metadata for 1 partition", 1, partitionMetadata.size)
  ...
  def testGetAllTopicMetadata
    ...
    assertEquals(
        "Expecting metadata for 1 partition", 1, partitionMetadataTopic1.size)
    assertEquals("Expecting partition id to be 0",
                 0,
                 partitionMetadataTopic1.head.partitionId)
    assertEquals(1, partitionMetadataTopic1.head.replicas.size)
    assertEquals(
        "Expecting metadata for 1 partition", 1, partitionMetadataTopic2.size)
    assertEquals("Expecting partition id to be 0",
                 0,
  ...
  def testAutoCreateTopic
    ...
    var topicsMetadata = ClientUtils
      ...
      .topicsMetadata
    assertEquals(
        Errors.LEADER_NOT_AVAILABLE.code, topicsMetadata.head.errorCode)
    assertEquals("Expecting metadata only for 1 topic", 1, topicsMetadata.size)
    assertEquals("Expecting metadata for the test topic",
                 topic,
                 topicsMetadata.head.topic)
  ...
  def testAutoCreateTopicWithCollision
    ...
    assertEquals(
        "Expecting metadata for topic1", topic1, topicsMetadata.head.topic)
    assertEquals(
        Errors.LEADER_NOT_AVAILABLE.code, topicsMetadata.head.errorCode)
    assertEquals(
        "Expecting metadata for topic2", topic2, topicsMetadata(1).topic)
  ...
  private def checkMetadata(
      servers: Seq[KafkaServer], expectedBrokersCount: Int): Unit =
    var topicMetadata: TopicMetadataResponse = new TopicMetadataResponse(
        Seq(), Seq(), -1)
