...
object TestUtils extends Logging {
  ...
  def randomString(len: Int): String = {
    ...
    for (i <- 0 until len)
      b.append(LettersAndDigits.charAt(
              seededRandom.nextInt(LettersAndDigits.length)))
    b.toString
  }
  ...
  def hexString(buffer: ByteBuffer): String = {
    ...
    for (i <- 0 until buffer.limit)
      builder.append(String
            .format("%x", Integer.valueOf(buffer.get(buffer.position + i))))
    builder.toString
  }
      ...
      leader match {
        case Some(l) =>
          if (newLeaderOpt.isDefined && newLeaderOpt.get == l) {
            ...
            trace("Leader %d is elected for partition [%s,%d]"
                  .format(l, topic, partition))
            isLeaderElectedOrChanged = true
          } else {
            trace("Current leader for partition [%s,%d] is %d"
                  .format(topic, partition, l))
          }
        case None =>
          trace("Leader for partition [%s,%d] is not elected yet"
                .format(topic, partition))
      }
  ...
  def isLeaderLocalOnBroker(topic: String,
                            partitionId: Int,
  ...
  def sendMessages(
    ...
    if (partition >= 0) {
      ...
      debug("Sent %d messages for partition [%s,%d]"
  ...
  def produceMessage(servers: Seq[KafkaServer],
                     topic: String,
                     message: String) {
    val producer = createNewProducer(
        TestUtils.getBrokerListStrFromServers(servers),
  ...
  def waitAndVerifyAcls(expected: Set[Acl],
  ...
  /**
    ...
    */
  def assertConcurrent(message: String,
                       functions: Seq[() => Any],
                       timeoutMs: Int) {
