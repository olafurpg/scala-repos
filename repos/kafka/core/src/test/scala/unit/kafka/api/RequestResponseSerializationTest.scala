...
object SerializationTestUtils
  ...
  private val topicDataFetchResponse =
    val groupedData = Array(topic1, topic2).flatMap(topic =>
          partitionDataFetchResponseMap.map(partitionAndData =>
                (TopicAndPartition(topic, partitionAndData._1),
                 partitionAndData._2)))
  ...
  private val brokers = List(
      new Broker(0,
                 Map(SecurityProtocol.PLAINTEXT -> EndPoint(
                         "localhost", 1011, SecurityProtocol.PLAINTEXT))),
      new Broker(1,
                 Map(SecurityProtocol.PLAINTEXT -> EndPoint(
                         "localhost", 1012, SecurityProtocol.PLAINTEXT))),
      new Broker(2,
                 Map(SecurityProtocol.PLAINTEXT -> EndPoint(
                         "localhost", 1013, SecurityProtocol.PLAINTEXT))))
  ...
  private val leaderAndIsr0 = new LeaderAndIsr(
      leader = brokers.head.id, isr = brokers.map(_.id))
  private val leaderAndIsr1 = new LeaderAndIsr(
      leader = brokers.head.id, isr = brokers.tail.map(_.id))
  private val leaderAndIsr2 = new LeaderAndIsr(
      leader = brokers.head.id, isr = brokers.map(_.id))
  private val leaderAndIsr3 = new LeaderAndIsr(
      leader = brokers.head.id, isr = brokers.tail.map(_.id))

  private val leaderIsrAndControllerEpoch0 = new LeaderIsrAndControllerEpoch(
      leaderAndIsr0, controllerEpoch = 0)
  private val leaderIsrAndControllerEpoch1 = new LeaderIsrAndControllerEpoch(
      leaderAndIsr1, controllerEpoch = 0)
  private val leaderIsrAndControllerEpoch2 = new LeaderIsrAndControllerEpoch(
      leaderAndIsr2, controllerEpoch = 0)
  private val leaderIsrAndControllerEpoch3 = new LeaderIsrAndControllerEpoch(
      leaderAndIsr3, controllerEpoch = 0)

  private val partitionStateInfo0 = new PartitionStateInfo(
      leaderIsrAndControllerEpoch0, brokers.map(_.id).toSet)
  private val partitionStateInfo1 = new PartitionStateInfo(
      leaderIsrAndControllerEpoch1, brokers.map(_.id).toSet)
  private val partitionStateInfo2 = new PartitionStateInfo(
      leaderIsrAndControllerEpoch2, brokers.map(_.id).toSet)
  private val partitionStateInfo3 = new PartitionStateInfo(
      leaderIsrAndControllerEpoch3, brokers.map(_.id).toSet)
  ...
  def createTestOffsetRequest = new OffsetRequest(
      collection.immutable.Map(
          TopicAndPartition(topic1, 1) -> PartitionOffsetRequestInfo(1000,
                                                                     200)),
      replicaId = 0
  )
    ...
    new OffsetFetchResponse(
        collection.immutable.Map(
            TopicAndPartition(topic1, 0) -> OffsetMetadataAndError(
                42L, "some metadata", Errors.NONE.code),
...
class RequestResponseSerializationTest extends JUnitSuite
  ...
  private val consumerMetadataResponseNoCoordinator = GroupCoordinatorResponse(
      None, ErrorMapping.ConsumerCoordinatorNotAvailableCode, 0)
  ...
  def testProduceResponseVersion()
    val oldClientResponse = ProducerResponse(
        1,
        Map(
            TopicAndPartition("t1", 0) -> ProducerResponseStatus(0.toShort,
                                                                 10001),
            TopicAndPartition("t2", 0) -> ProducerResponseStatus(0.toShort,
                                                                 20001)
        ))
    ...
    assertEquals(
        oldClientResponse.sizeInBytes + 4, newClientResponse.sizeInBytes)
