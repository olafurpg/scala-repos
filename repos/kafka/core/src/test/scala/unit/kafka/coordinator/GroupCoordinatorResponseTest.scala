...
class GroupCoordinatorResponseTest extends JUnitSuite {
  ...
  def setUp() {
    ...
    ret +=
      (TopicConstants.GROUP_METADATA_TOPIC_NAME -> Map(0 -> Seq(1),
                                                       1 -> Seq(1)))
  ...
  def testJoinGroupFromUnchangedFollowerDoesNotRebalance() {
    ...
    EasyMock.reset(replicaManager)
    val firstSyncResult = syncGroupLeader(groupId,
                                          firstGenerationId,
                                          firstMemberId,
    ...
    EasyMock.reset(replicaManager)
    val otherJoinFuture = sendJoinGroup(groupId,
                                        JoinGroupRequest.UNKNOWN_MEMBER_ID,
                                        DefaultSessionTimeout,
                                        protocolType,
                                        protocols)
    ...
    EasyMock.reset(replicaManager)
    val followerJoinResult = joinGroup(groupId,
    ...
    EasyMock
      ...
      .andAnswer(new IAnswer[Unit] {
        override def answer = capturedArgument.getValue.apply(
            Map(
                new TopicPartition(TopicConstants.GROUP_METADATA_TOPIC_NAME,
