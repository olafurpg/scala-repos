...
class HaltSpec extends ScalatraSpec {
  def is =
    "halt with no arguments should" ^ "behave like a common halt" ^ commonHalt(
      ...
      "/status-and-body") ^ "set the status" ! status("/status-and-body", 404) ^ "render the body" ! bodyEquals(
      "/status-and-body",
      ...
      "/all-args",
      "Go away") ^ "set the headers" ! hasHeader(
      "/all-args",
      ...
      "/action-result",
      "Not Acceptable") ^ "set the headers" ! hasHeader(
      "/action-result",
      "X-Action-Result",
