...
class SwaggerWithAuth(val swaggerVersion: String,
...
trait SwaggerAuthSupport[TypeForUser <: AnyRef]
  ...
  protected def extractOperation(
    ...
    val op =
      route.metadata
        .get(Symbols.Operation)
        .map(_.asInstanceOf[AuthOperation[TypeForUser]])
    op.map(_.copy(method = method)).getOrElse {
      ...
      val allows =
        route.metadata
          .get(Symbols.Allows)
          .map(_.asInstanceOf[Option[TypeForUser] => Boolean])
          .getOrElse(allowAll)
      val consumes =
        route.metadata
