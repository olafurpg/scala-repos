...
object Dependencies {
  ...
  def runtime(scalaVersion: String) =
    slf4j ++ Seq("akka-actor", "akka-slf4j").map(
        "com.typesafe.akka" %% _ % akkaVersion) ++ jacksons ++ Seq(
        ...
        scalaJava8Compat
    ) ++ scalaParserCombinators(scalaVersion) ++ specsBuild
      .map(_ % Test) ++ logback.map(_ % Test) ++ javaTestDeps
