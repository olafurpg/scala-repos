...
object JsonValidSpec extends Specification {
  ...
  "JSON caseclass/tuple validation" should {
    ...
    "validate simple case class reads/writes" in {
      ...
      implicit val userReads = {
        ...
        (at(JsPath \ "name")(Reads.minLength[String](5))
          .and(at(JsPath \ "age")(Reads.min(40))))(User)
      }
    ...
    "validate simple case class format" in {
      ...
      implicit val userFormats = {
        ...
        (at(JsPath \ "name")(Format(Reads.minLength[String](5), of[String]))
          .and(at(JsPath \ "age")(Format(Reads.min(40), of[Int]))))(
          User,
          unlift(User.unapply))
      }
    ...
    "Apply min/max correctly on any numeric type" in {
      ...
      implicit val numbersFormat = ((__ \ 'i)
        .format[Int](Reads.min(5).andKeep(Reads.max(100)))
        .and(__ \ 'l)
        .format[Long](Reads.min(5L).andKeep(Reads.max(100L)))
        .and(__ \ 'f)
        .format[Float](Reads.min(13.0F).andKeep(Reads.max(14.0F)))
        .and(__ \ 'd)
        .format[Double](Reads.min(0.1).andKeep(Reads.max(1.0)))
        .and(__ \ 'bd)
        .format[BigDecimal](Reads
          .min(BigDecimal(5))
          .andKeep(Reads
  ...
  "JSON generators" should {
    "Build JSON from JSON Reads" in {
      ...
      val jsonTransformer =
        ((__ \ "key1").json.pickBranch
          .and(__ \ "key2")
          .json
          .pickBranch(
            ((__ \ "key22").json
              .update((__ \ "key222").json.pick)
              .and(__ \ "key233")
              .json
              .copyFrom((__ \ "key23").json.pick))
              .reduce
          )
          .and(__ \ "key3")
          .json
          .pickBranch[JsArray](pure(Json.arr("delta")))
          .and(__ \ "key4")
          .json
          .put(
            Json.obj(
  ...
  "JSON Reads" should {
    "manage nullable/option" in {
      ...
      implicit val UserReads = ((__ \ 'name)
        .read[String]
        .and(__ \ 'coords \ 'email)
        .read(Reads.email)
        .and(__ \ 'coords \ 'phone)
    ...
    "report correct path for validation errors" in {
      ...
      implicit val UserReads = ((__ \ 'email)
        .read(Reads.email)
        .and(__ \ 'phone)
        .readNullable(Reads.minLength[String](8)))(User)
    ...
    "mix reads constraints" in {
      ...
      implicit val UserReads = ((__ \ 'id)
        .read[Long]
        .and(__ \ 'email)
        .read(Reads.email.andKeep(Reads.minLength[String](5)))
        .and(__ \ 'age)
    ...
    "recursive reads" in {
      ...
      implicit lazy val UserReads: Reads[User] = ((__ \ 'id)
        .read[Long]
        .and(__ \ 'name)
        .read[String]
        .and(__ \ 'friend)
    ...
    "recursive writes" in {
      ...
      implicit lazy val UserWrites: Writes[User] = ((__ \ 'id)
        .write[Long]
        .and(__ \ 'name)
        .write[String]
        .and(__ \ 'friend)
    ...
    "reduce Reads[JsObject]" in {
      ...
      val myReads: Reads[JsObject] = ((__ \ 'field1).json.pickBranch
        .and(__ \ 'field2)
        .json
        .pickBranch)
        .reduce
    ...
    "reduce Reads[JsArray]" in {
      ...
      val myReads: Reads[JsArray] = ((__ \ 'field1).json
        .pick[JsString]
        .and(__ \ 'field2)
        .json
        .pick[JsNumber]
    ...
    "reduce Reads[JsArray] no type" in {
      ...
      val myReads: Reads[JsArray] = ((__ \ 'field1).json.pick
        .and(__ \ 'field2)
        .json
        .pick
        .and(__ \ 'field3)
  ...
  "JSON Writes" should {
    "manage option" in {
      ...
      implicit val UserWrites = ((__ \ 'email)
        .write[String]
        .and(__ \ 'phone)
  ...
  "JSON Format" should {
    "manage option" in {
      ...
      implicit val UserFormat = ((__ \ 'email)
        .format(email)
        .and(__ \ 'phone)
