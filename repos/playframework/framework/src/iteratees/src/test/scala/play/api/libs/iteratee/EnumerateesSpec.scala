...
object EnumerateesSpec
  ...
  "Enumeratee.map" should {
    ...
    "add one to each of the ints enumerated" in {
      mustExecute(4) { mapEC =>
        ...
        Await
          .result(enumerator |>>> add1AndConsume, Duration.Inf) must equalTo(
            Seq(2, 3, 4, 5))
      }
    }
  ...
  "Enumeratee.flatten" should {
    ...
    "passAlong a future enumerator" in {
      mustExecute(9) { sumEC =>
        ...
        Await
          .result(enumerator |>>> passAlongFuture &>> sum, Duration.Inf) must equalTo(
            45)
      }
    }
  }
      ...
      mustExecute(9, 7, 3) { (mapInputEC, foldEC, mapEC) =>
        val folderIteratee =
          Enumeratee.mapInput[String] {
            ...
            case other => other
          }(mapInputEC) &>> Iteratee.fold[String, String]("")((s, e) => s + e)(
              foldEC)

        val result =
          Enumerator("He", "ll", "o", "Concat", "Wo", "r", "ld", "Concat", "!") &> Enumeratee
            .grouped(folderIteratee) ><> Enumeratee
            .map[String](List(_))(mapEC) |>>> Iteratee.consume[List[String]]()
  ...
  "Enumeratee.scanLeft" should {
    ...
    "transform elements using a sate" in {
      mustExecute(4) { mapEC =>
        val result =
          Enumerator(1, 2, 3, 4) &> Enumeratee
            .scanLeft[Int](0)(_ + _) ><> Enumeratee
