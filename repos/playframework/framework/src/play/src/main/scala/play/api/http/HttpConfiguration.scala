...
/**
  ...
  */
case class ParserConfiguration(maxMemoryBuffer: Int = 102400,
  ...
  def fromConfiguration(configuration: Configuration) = {
    ...
    HttpConfiguration(
      ...
      session = SessionConfiguration(
        cookieName =
          config.getDeprecated[String]("play.http.session.cookieName",
                                       "session.cookieName"),
        ...
        maxAge = config.getDeprecated[Option[FiniteDuration]](
          ...
          "session.maxAge"),
        httpOnly = config.getDeprecated[Boolean]("play.http.session.httpOnly",
                                                 "session.httpOnly"),
        domain =
          config.getDeprecated[Option[String]]("play.http.session.domain",
                                               "session.domain")
      ),
      flash = FlashConfiguration(
        cookieName = config.getDeprecated[String]("play.http.flash.cookieName",
                                                  "flash.cookieName"),
