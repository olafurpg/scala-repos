  ...
  override def buildVisitor(
    ...
    new ScalaElementVisitor
      override def visitXmlStartTag(s: ScXmlStartTag)
        ...
        def register(fixes: LocalQuickFix*)
          holder.registerProblem(
              s, ScalaBundle.message("xml.no.closing.tag"), fixes: _*)
      ...
      override def visitXmlEndTag(s: ScXmlEndTag)
        ...
        def register(fixes: LocalQuickFix*)
          holder.registerProblem(
              s, ScalaBundle.message("xml.no.opening.tag"), fixes: _*)
...
class DeleteUnmatchedTagQuickFix(s: ScalaPsiElement)
    extends AbstractFixOnPsiElement(
        ScalaBundle.message("xml.delete.unmatched.tag"), s)
...
class RenameClosingTagQuickFix(s: ScXmlStartTag)
    extends AbstractFixOnPsiElement(
        ScalaBundle.message("xml.rename.closing.tag"), s)
  override def getFamilyName: String = InspectionsUtil.SCALA
  ...
  def doApplyFix(project: Project)
    ...
    elem.getClosingTag.replace(ScalaPsiElementFactory.createXmlEndTag(
            elem.getTagName, elem.getManager))

class RenameOpeningTagQuickFix(s: ScXmlEndTag)
    extends AbstractFixOnPsiElement(
        ScalaBundle.message("xml.rename.opening.tag"), s)
  override def getFamilyName: String = InspectionsUtil.SCALA
