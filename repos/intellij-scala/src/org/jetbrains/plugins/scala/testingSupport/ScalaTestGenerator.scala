...
class ScalaTestGenerator extends TestGenerator {
  ...
  private def generateTestInternal(
      project: Project, d: CreateTestDialog): PsiFile = {
  ...
  private def addSuperClass(typeDefinition: ScTypeDefinition,
    ...
    def addExtendsRef(refName: String) = {
      val (extendsToken, classParents) =
        ScalaPsiElementFactory.createClassTemplateParents(
            refName, typeDefinition.getManager)
      val extendsAdded =
  ...
  private def addTestMethods(editor: Editor,
      ...
      case Some(body) =>
        ...
        if (isInheritor(typeDef, "org.scalatest.FeatureSpecLike") ||
          ...
          ScalaTestGenerator.generateScalaTestBeforeAndAfter(
              generateBefore, generateAfter, typeDef, editor.getProject)
          ScalaTestGenerator.addScalaTestFeatureSpecMethods(
              methodsList, psiManager, body)
          ...
          ScalaTestGenerator.addScalaTestFlatSpecMethods(
              methodsList, psiManager, body, className)
          ...
          ScalaTestGenerator.addSpecs2SpecificationMethods(
              methodsList, psiManager, body, className, editor.getProject)
        } else if (isInheritor(
                       typeDef, "org.specs2.mutable.SpecificationLike")) {
          ScalaTestGenerator.generateSpecs2BeforeAndAfter(
              generateBefore, generateAfter, typeDef, editor.getProject)
...
object ScalaTestGenerator {
  ...
  private def generateScalaTestBeforeAndAfter(generateBefore: Boolean,
                                              generateAfter: Boolean,
                                              typeDef: ScTypeDefinition,
                                              project: Project) {
    ...
    typeDef.extendsBlock.templateBody match {
      case Some(body) =>
        Option(
            ScalaPsiManager
              .instance(project)
              .getCachedClass("org.scalatest.BeforeAndAfterEach",
          ...
          case Some(beforeAndAfterTypeDef)
            ...
            ExtractSuperUtil.addExtendsTo(
                typeDef, beforeAndAfterTypeDef.asInstanceOf[ScTypeDefinition])
  ...
  private def generateSpecs2BeforeAndAfter(generateBefore: Boolean,
                                           generateAfter: Boolean,
                                           typeDef: ScTypeDefinition,
                                           project: Project) {
    if (!(generateBefore || generateAfter)) return
      ...
      case Some(body) =>
        ...
        if (generateBefore) {
          val beforeOpt = Option(
              ScalaPsiManager
                .instance(project)
                .getCachedClass("org.specs2.specification.BeforeEach",
                                GlobalSearchScope.allScope(project),
                                ScalaPsiManager.ClassCategory.TYPE))
          beforeOpt match {
            case Some(beforeTypeDef) =>
              ExtractSuperUtil.addExtendsTo(
                  typeDef, beforeTypeDef.asInstanceOf[ScTypeDefinition])
              body.addBefore(ScalaPsiElementFactory.createMethodFromText(
        ...
        if (generateAfter) {
          val afterOpt = Option(
              ScalaPsiManager
                .instance(project)
                .getCachedClass("org.specs2.specification.AfterEach",
                                GlobalSearchScope.allScope(project),
                                ScalaPsiManager.ClassCategory.TYPE))
          afterOpt match {
            case Some(afterTypeDef) =>
              ExtractSuperUtil.addExtendsTo(
                  typeDef, afterTypeDef.asInstanceOf[ScTypeDefinition])
              body.addBefore(ScalaPsiElementFactory.createMethodFromText(
  ...
  private def addScalaTestFeatureSpecMethods(methods: List[MemberInfo],
  ...
  private def addScalaTestFlatSpecMethods(methods: List[MemberInfo],
                                          psiManager: PsiManager,
                                          templateBody: ScTemplateBody,
                                          className: String) {
    if (methods.nonEmpty) {
      ...
      templateBody.addBefore(
          ScalaPsiElementFactory.createExpressionFromText(
              "behavior of \"" + className + "\"", psiManager),
          closingBrace)
      templateBody.addBefore(
  ...
  private def addScalaTestFreeSpecMethods(methods: List[MemberInfo],
                                          psiManager: PsiManager,
                                          templateBody: ScTemplateBody) {
    if (methods.nonEmpty) {
      templateBody.addBefore(
  ...
  private def addScalaTestFunSpecMethods(methods: List[MemberInfo],
                                         psiManager: PsiManager,
                                         templateBody: ScTemplateBody,
                                         className: String) {
    if (methods.nonEmpty) {
  ...
  private def addScalaTestFunSuiteMethods(methods: List[MemberInfo],
                                          psiManager: PsiManager,
                                          templateBody: ScTemplateBody) {
    if (methods.nonEmpty) {
      val closingBrace = templateBody.getLastChild
  ...
  private def addScalaTestPropSpecMethods(methods: List[MemberInfo],
                                          psiManager: PsiManager,
                                          templateBody: ScTemplateBody) {
    if (methods.nonEmpty) {
      val closingBrace = templateBody.getLastChild
  ...
  private def addScalaTestWordSpecMethods(methods: List[MemberInfo],
                                          psiManager: PsiManager,
                                          templateBody: ScTemplateBody,
                                          className: String) {
    if (methods.nonEmpty) {
  ...
  private def addSpecs2SpecificationMethods(methods: List[MemberInfo],
                                            psiManager: PsiManager,
                                            templateBody: ScTemplateBody,
                                            className: String,
                                            project: Project) {
  ...
  private def generateSpecs2ScriptSpecificationMethods(
    ...
    Option(
        ScalaPsiManager
          .instance(project)
          .getCachedClass("org.specs2.specification.Groups",
                          GlobalSearchScope.allScope(project),
                          ScalaPsiManager.ClassCategory.TYPE)) match {
      case Some(groupsTypeDef) =>
        ExtractSuperUtil.addExtendsTo(
            typeDef, groupsTypeDef.asInstanceOf[ScTypeDefinition])
        val testNames = methods.map("test" + _.getMember.getName.capitalize)
        val closingBrace = templateBody.getLastChild
        val normalIndentString = FormatterUtil.getNormalIndentString(project)
        val doubleIndent = normalIndentString + normalIndentString
  ...
  private def generateUTestMethods(methods: List[MemberInfo],
                                   psiManager: PsiManager,
                                   templateBody: ScTemplateBody,
                                   className: String,
                                   project: Project) {
    ...
    templateBody.addBefore(
        ScalaPsiElementFactory.createElement(
            "val tests = TestSuite{}", psiManager, Def.parse(_)),
        templateBody.getLastChild)
    if (methods.nonEmpty) {
      templateBody.addBefore(
          ScalaPsiElementFactory.createElement(
