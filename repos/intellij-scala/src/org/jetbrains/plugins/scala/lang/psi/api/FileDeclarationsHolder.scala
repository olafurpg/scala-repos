...
import org.jetbrains.plugins.scala.lang.psi.{
  ScDeclarationSequenceHolder, ScImportsHolder, ScalaPsiUtil
}
...
trait FileDeclarationsHolder
  ...
  override def processDeclarations(processor: PsiScopeProcessor,
    ...
    if (checkPredefinedClassesAndPackages) {
      ...
      while (synthIterator.hasNext) {
        ...
        ProgressManager.checkCanceled()
        if (!alreadyContains(synth.getName) &&
            !processor.execute(synth, state)) return false
      }
  ...
  private def importedObjects(manager: PsiManager,
