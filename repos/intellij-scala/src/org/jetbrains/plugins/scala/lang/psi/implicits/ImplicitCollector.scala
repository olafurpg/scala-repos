...
object ImplicitCollector {
  ...
  case class ImplicitState(
      ...
      predicate: Option[
        (ScalaResolveResult, ScSubstitutor) => Option[(ScalaResolveResult,
                                                       ScSubstitutor)]],
    ...
    def calc(): Seq[ScalaResolveResult] = {
      ...
      while (!stop) {
        if (placeForTreeWalkUp == null ||
            !placeForTreeWalkUp.processDeclarations(processor,
      ...
      for (obj <- ScalaPsiUtil.collectImplicitObjects(expandedTp,
                                                      place.getProject,
    ...
    def execute(element: PsiElement, state: ResolveState): Boolean = {
      ...
      named match {
        ...
        case patt: ScBindingPattern =>
          val memb = ScalaPsiUtil.getContextOfType(patt,
                                                   true,
                                                   classOf[ScValue],
                                                   classOf[ScVariable])
          memb match {
    ...
    def candidatesS(fullInfo: Boolean): collection.Set[ScalaResolveResult] = {
                      ...
                      if (typeParameters.isEmpty && lastImplicit.isEmpty)
                        ...
                        def inferValueType(
                          ...
                          if (isExtensionConversion) {
                            tp match {
                              case ScTypePolymorphicType(internalType,
                                                         typeParams) =>
      ...
      def mapCandidates(withLocalTypeInference: Boolean)
        ...
        while (candidatesSeq.nonEmpty) {
          val (next, rest) =
            mostSpecific.nextLayerSpecificForImplicitParameters(lastResult,
                                                                candidatesSeq)
          next match {
            case Some(c) =>
              candidatesSeq = rest
