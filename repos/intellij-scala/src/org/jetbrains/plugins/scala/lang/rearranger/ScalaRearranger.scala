...
class ScalaRearranger
  ...
  override def parse(root: PsiElement,
                     document: Document,
                     ranges: java.util.Collection[TextRange],
                     settings: ArrangementSettings) = {
  ...
  override def getBlankLines(settings: CodeStyleSettings,
...
object ScalaRearranger {
  ...
  private def addCondition(matchRules: immutable.List[ArrangementSectionRule],
