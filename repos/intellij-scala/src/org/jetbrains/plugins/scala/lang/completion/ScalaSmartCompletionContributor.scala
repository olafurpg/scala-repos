...
import com.intellij.patterns.{
  ElementPattern, PlatformPatterns, StandardPatterns
}
...
import org.jetbrains.plugins.scala.lang.completion.handlers.{
  ScalaConstructorInsertHandler, ScalaGenerateAnonymousFunctionInsertHandler
}
import org.jetbrains.plugins.scala.lang.completion.lookups.{
  LookupElementManager, ScalaChainLookupElement, ScalaLookupItem
}
...
class ScalaSmartCompletionContributor extends ScalaCompletionContributor {
  ...
  private def acceptTypes(typez: Seq[ScType],
                      ...
                      r match {
                        case r: ScalaResolveResult if !r.isNamedParameter =>
                          ...
                          val newElem = LookupElementManager
                            ...
                            .head
                          applyVariant(
                              new ScalaChainLookupElement(el, newElem))
                        case _ =>
                      }
                    }
                  })
            ...
            o.members.foreach {
              case function: ScFunction =>
                val lookup = LookupElementManager
                  .getLookupElement(new ScalaResolveResult(function),
                                    isClassName = true,
              ...
              case obj: ScObject =>
                val lookup = LookupElementManager
                  .getLookupElement(new ScalaResolveResult(obj),
                                    isClassName = true,
                                    isOverloadedForClassName = false,
                                    shouldImport = true,
                                    isInStableCodeReference = false)
    ...
    for (expected <- expects) {
      ...
      if (actualParams != null) {
        ...
        val builder = LookupElementBuilder
          ...
          .withInsertHandler(
              new ScalaGenerateAnonymousFunctionInsertHandler(params,
                                                              braceArgs))
        val lookupElement =
...
private[completion] object ScalaSmartCompletionContributor {
  ...
  val bracesCallPattern = superParentsPattern(classOf[ScReferenceExpression],
                                              classOf[ScBlockExpr],
