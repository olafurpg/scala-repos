...
object SafeDeleteProcessorUtil {
  ...
  def findTypeParameterExternalUsages(typeParameter: PsiTypeParameter,
...
//      validateOverridingMethods(psiMethod, references, Arrays.asList(overridingMethods : _*), methodToReferences, usages, allElementsToDelete)
  ...
  def validateOverridingMethods(
      ...
      methodToReferences: util.HashMap[PsiMethod,
                                       util.Collection[PsiReference]],
      usages: util.List[UsageInfo],
  ...
  def canBePrivate(method: PsiMethod,
    ...
    for (reference <- references) {
      val element: PsiElement = reference.getElement
      if (!isInside(element, allElementsToDelete) &&
          !isInside(element, deleted) && !facade.getResolveHelper
  ...
  def findParameterUsages(parameter: PsiParameter,
                          usages: util.List[UsageInfo]) {
    ...
    referenceSearch(parameter).forEach(new Processor[PsiReference] {
      def process(reference: PsiReference): Boolean = {
        ...
        if (docTag != null) {
          usages.add(new SafeDeleteReferenceJavaDeleteUsageInfo(docTag,
                                                                parameter,
                                                                true))
          return true
        }
        ...
        usages.add(new SafeDeleteReferenceJavaDeleteUsageInfo(element,
                                                              parameter,
                                                              isSafeDelete))
        true
      }
