...
class ScPrimaryConstructorWrapper(val constr: ScPrimaryConstructor,
...
/**
  ...
  * @since 27.02.12
  */
class ScFunctionWrapper(val function: ScFunction,
                        isStatic: Boolean,
                        isInterface: Boolean,
...
with LightScalaMethod {
  ...
  override def getReturnType: PsiType = {
    ...
    if (returnType == null && !isConstructor) {
      ...
      val generifySubst: ScSubstitutor = if (typeParameters.nonEmpty) {
...
object ScFunctionWrapper {
  ...
  /**
    ...
    */
  def methodText(function: ScMethodLike,
                 isStatic: Boolean,
                 isInterface: Boolean,
                 cClass: Option[PsiClass],
