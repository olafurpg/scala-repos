...
import com.intellij.psi.search.{
  GlobalSearchScope, GlobalSearchScopesCore, PsiSearchHelper
}
...
object ScopeSuggester {
  def suggestScopes(conflictsReporter: ConflictsReporter,
    ...
    def isSuitableParent(owners: Seq[ScTypeParametersOwner],
  ...
  protected def getAllAvailablePackages(
    ...
    def getDirectoriesContainigFileAndPackage(
      ...
      if (currentPackage != null && currentPackage.getName != null) {
        ...
        getDirectoriesContainigFileAndPackage(currentPackage.getParentPackage,
                                              module,
                                              result,
                                              dirContainingFile)
      } else {
    ...
    getDirectoriesContainigFileAndPackage(currentPackage,
                                          module,
                                          result,
  ...
  def handleOnePackage(typeElement: ScTypeElement,
    ...
    val fileEncloser =
      if (packageObject.isDefined)
        PsiTreeUtil.getChildOfType(
            PsiTreeUtil.getChildOfType(packageObject.get,
    ...
    val validator = ScalaCompositeTypeValidator(allValidators.toList,
                                                conflictsReporter,
                                                project,
                                                typeElement,
                                                occurrences.isEmpty,
    ...
    val result = PackageScopeItem(inPackage.getName,
                                  fileEncloser,
                                  needNewDir,
                                  possibleNames.toArray)
