...
class ScalaGenerateDelegateHandler extends GenerateDelegateHandler {
  ...
  private def methodBody(delegate: ClassMember,
                         prototype: ScFunction): ScExpression = {
    def typeParameterUsedIn(parameter: ScTypeParam,
      ...
      def unapply(srr: ScalaResolveResult): Option[PhysicalSignature] = {
        ...
        srr.getElement match {
          ...
          case meth: PsiMethod
            ...
            None
          case meth: PsiMethod
              if !ResolveUtils.isAccessible(meth, place, forCompletion = true) =>
            None
          case meth: PsiMethod =>
  ...
  @Nullable
  private def chooseTarget(file: PsiFile,
                           editor: Editor,
  ...
  private def targetElements(file: PsiFile,
  ...
  private def canBeTargetInClass(member: ClassMember,
  ...
  private def parentClasses(file: PsiFile,
