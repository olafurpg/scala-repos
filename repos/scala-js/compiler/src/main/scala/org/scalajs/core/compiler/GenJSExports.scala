...
trait GenJSExports extends SubComponent { self: GenJSCode =>
  ...
  trait JSExportsPhase { this: JSCodePhase =>
    ...
    /**
      ...
      */
    def genMemberExports(classSym: Symbol,
    ...
    private def genMemberExport(classSym: Symbol, name: TermName): js.Tree = {
      ...
      genMemberExportOrDispatcher(classSym,
                                  jsName,
                                  isProp,
                                  alts,
                                  isDispatcher = false)
    ...
    private def genExportProperty(alts: List[Symbol], jsName: String) = {
      ...
      js.PropertyDef(js.StringLiteral(jsName),
                     getTree,
                     genFormalArg(1),
                     setTree)
    }
    ...
    private def genExportMethod(alts0: List[Exported], jsName: String) = {
      ...
      val cases = for {
        ...
        caseBody = genExportSameArgc(minArgc,
                                     needsRestParam,
                                     methods.toList,
                                     0,
                                     Some(argcs.min))
    ...
    private def genApplyForSymJSSuperCall(minArgc: Int,
                                          hasRestParam: Boolean,
                                          sym: Symbol): js.Tree = {
    ...
    private def genApplyForSymNonJSSuperCall(minArgc: Int,
      ...
      val jsVarArgPrep =
        repeatedTpe map { tpe =>
          ...
          js.VarDef(js.Ident("prep" + normalArgc),
                    rhs.tpe,
                    mutable = false,
                    rhs)
        }
    ...
    private case class ExportedBody(params: List[Type],
                                    body: js.Tree,
                                    name: String,
                                    pos: Position)
        extends Exported {
  ...
  private def genFormalArg(index: Int)(implicit pos: Position): js.ParamDef = {
    js.ParamDef(js.Ident("arg$" + index),
                jstpe.AnyType,
                mutable = false,
                rest = false)
  }
