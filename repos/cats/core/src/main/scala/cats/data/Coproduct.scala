...
final case class Coproduct[F[_], G[_], A](run: F[A] Xor G[A]) {
  ...
  def coflatMap[B](f: Coproduct[F, G, A] => B)(
    ...
    Coproduct(
        run.bimap(a => F.coflatMap(a)(x => f(leftc(x))), a =>
              G.coflatMap(a)(x => f(rightc(x))))
    )

  def coflatten(implicit F: CoflatMap[F],
                G: CoflatMap[G]): Coproduct[F, G, Coproduct[F, G, A]] =
    Coproduct(run.bimap(x => F.coflatMap(x)(a => leftc(a)), x =>
              G.coflatMap(x)(a => rightc(a))))
  ...
  def foldLeft[B](z: B)(f: (B, A) => B)(implicit F: Foldable[F],
                                        G: Foldable[G]): B =
...
private[data] trait CoproductTraverse[F[_], G[_]]
  ...
  override def map[A, B](a: Coproduct[F, G, A])(
      f: A => B): Coproduct[F, G, B] =
    a map f
