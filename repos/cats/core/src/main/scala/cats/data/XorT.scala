...
final case class XorT[F[_], A, B](value: F[A Xor B]) {
  ...
  def applyAlt[D](
      ff: XorT[F, A, B => D])(implicit F: Apply[F]): XorT[F, A, D] =
    XorT[F, A, D](
  ...
  def combine(that: XorT[F, A, B])(implicit F: Apply[F],
                                   A: Semigroup[A],
...
private[data] abstract class XorTInstances1 extends XorTInstances2 {
  ...
  implicit def xorTMonoidK[F[_], L](implicit F: Monad[F],
...
private[data] trait XorTMonadError[F[_], L]
  ...
  def handleErrorWith[A](
      fea: XorT[F, L, A])(f: L => XorT[F, L, A]): XorT[F, L, A] =
