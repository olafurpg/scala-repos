...
object eq
  ...
  implicit def function1Eq[A, B](
      implicit A: Arbitrary[A], B: Eq[B]): Eq[A => B] = new Eq[A => B]
  ...
  implicit def tuple3Eq[A, B, C](
      implicit EqA: Eq[A], EqB: Eq[B], EqC: Eq[C]): Eq[(A, B, C)] =
    new Eq[(A, B, C)]
  ...
  implicit def semigroupEq[A](
      implicit arbAA: Arbitrary[(A, A)], eqA: Eq[A]): Eq[Semigroup[A]] =
    function1Eq[(A, A), A].on(f => Function.tupled((x, y) => f.combine(x, y)))

  implicit def monoidEq[A](
      implicit eqSA: Eq[Semigroup[A]], eqA: Eq[A]): Eq[Monoid[A]] =
    new Eq[Monoid[A]]
