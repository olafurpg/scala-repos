...
class GoldSitesTestIT {
  ...
  @Test
  def testArticleElementedArticle() {
    // to verify issue #56 is resolved
  ...
  def businessWeek2() {
    ...
    TestUtils.runArticleAssertions(
        article = article, expectedStart = content, expectedImage = image)
  }
  ...
  def buzznetImages() {
    ...
    TestUtils.runArticleAssertions(
        article = article,
        expectedStart =
          "We've had so many false hopes with the new No Doubt CD.",
        expectedImage =
