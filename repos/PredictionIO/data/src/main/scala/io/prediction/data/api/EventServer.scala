...
class EventServiceActor(val eventClient: LEvents,
...
class EventServerActor(val eventClient: LEvents,
                       val accessKeysClient: AccessKeys,
                       val channelsClient: Channels,
                       val config: EventServerConfig)
    extends Actor
...
object EventServer {
  def createEventServer(config: EventServerConfig): Unit = {
    ...
    val serverActor = system.actorOf(
      Props(classOf[EventServerActor],
            eventClient,
            accessKeysClient,
            channelsClient,
            config),
