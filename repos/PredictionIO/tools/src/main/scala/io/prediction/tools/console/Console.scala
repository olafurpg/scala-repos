...
case class ConsoleArgs(common: CommonArgs = CommonArgs(),
...
case class DeployArgs(ip: String = "0.0.0.0",
                      port: Int = 8000,
                      logUrl: Option[String] = None,
                      logPrefix: Option[String] = None)

case class EventServerArgs(enabled: Boolean = false,
                           ip: String = "0.0.0.0",
                           port: Int = 7070,
                           stats: Boolean = false)
...
object Console extends Logging {
  def main(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[ConsoleArgs]("pio") {
      ...
      cmd("build").text("Build an engine at the current directory.").action {
        ...
        (opt[String]("sbt-extra") action { (x, c) =>
            ...
            opt[Unit]("clean") action { (x, c) =>
              ...
              c.copy(
                  build = c.build.copy(sbtAssemblyPackageDependency = false))
      ...
      cmd("deploy")
        ...
        (opt[String]("batch") action { (x, c) =>
            ...
            } text ("Engine instance ID."), opt[String]("ip") action {
              (x, c) =>
      ...
      cmd("app").text("Manage apps.\n").action { (_, c) =>
        ...
        (cmd("new").text("Create a new app key to app ID mapping.").action {
              ...
              (opt[Int]("id") action { (x, c) =>
                  ...
                  }), note(""), cmd("list").text("List all apps.").action {
              (_, c) =>
            ...
            cmd("channel-new")
              ...
              (arg[String]("<name>") action { (x, c) =>
                    c.copy(app = c.app.copy(name = x))
                  } text ("App name."), arg[String]("<channel>") action {
                    (x, c) =>
    ...
    val (consoleArgs, theRest) = if (separatorIndex == -1) {
      (args, Array[String]())
    } else {
    ...
    withRegisteredManifest(ca.common.manifestJson,
      ...
      engineInstance map { r =>
        ...
        ca.engineInstanceId map { eid =>
          ...
          error(s"No valid engine instance found for engine ${em.id} " +
  ...
  def compile(ca: ConsoleArgs): Unit = {
    ...
    val asm = if (ca.build.sbtAssemblyPackageDependency) {
    ...
    try {
      val r = if (ca.common.verbose) {
        ...
        buildCmd.!(ProcessLogger(line => outputSbtError(line),
  ...
  def coreAssembly(pioHome: String): File = {
    ...
    val coreDir = if (new File(pioHome + File.separator + "RELEASE").exists) {
  ...
  def regenerateManifestJson(json: File): Unit = {
    ...
    if (json.exists) {
      ...
      if (em.description == Some(manifestAutogenTag) && ha != em.version) {
        ...
        info(
            s"Using existing engine manifest JSON at ${json.getCanonicalPath}")
      }
    } else {
      generateManifestJson(json)
    }
