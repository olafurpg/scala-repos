...
object KeyIndex {
  ...
  def combine(indices: Seq[KeyIndex]): KeyIndex = new KeyIndex {
    ...
    def tasks(
        proj: Option[ResolvedReference], conf: Option[String], key: String) =
...
trait KeyIndex {
  ...
  def isEmpty(proj: Option[ResolvedReference],
              conf: Option[String],
              task: Option[AttributeKey[_]]): Boolean =
    keys(proj, conf, task).isEmpty
...
trait ExtendableKeyIndex extends KeyIndex {
  ...
  def addAggregated(
      scoped: ScopedKey[_], extra: BuildUtil[_]): ExtendableKeyIndex
}
// task axis <-> key
private final class AKeyIndex(
  ...
  def tasks(key: String): Set[AttributeKey[_]] =
    data.reverse(key).flatten.toSet
}
...
private final class KeyIndex0(val data: BuildIndex)
    extends ExtendableKeyIndex {
  ...
  def tasks(proj: Option[ResolvedReference],
            conf: Option[String],
            key: String): Set[AttributeKey[_]] =
    keyIndex(proj, conf).tasks(key)
  def keys(proj: Option[ResolvedReference]): Set[String] =
    (Set.empty[String] /: optConfigs(proj)) { (s, c) =>
      s ++ keys(proj, c)
    }
  def keys(
      proj: Option[ResolvedReference], conf: Option[String]): Set[String] =
  ...
  def keyIndex(
      proj: Option[ResolvedReference], conf: Option[String]): AKeyIndex =
