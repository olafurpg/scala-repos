...
object Tests {
  ...
  /**
    ...
    */
  final case class Argument(framework: Option[TestFramework],
                            args: List[String])
      extends TestOption
  ...
  /**
    ...
    */
  final case class Execution(options: Seq[TestOption],
                             parallel: Boolean,
                             tags: Seq[(Tag, Int)])
  ...
  def foldTasks(results: Seq[Task[Output]], parallel: Boolean): Task[Output] =
    if (results.isEmpty)
      ...
      def sequence(tasks: List[Task[Output]],
