...
class ConstraintsTest extends MarathonSpec with GivenWhenThen with Matchers {
  ...
  test("Select tasks to kill for a single group by works") {
    Given(
      "app with hostname group_by and 20 tasks even distributed on 2 hosts")
  ...
  test("Select tasks to kill for multiple group by works") {
    ...
    val app = AppDefinition(
      constraints = Set(makeConstraint("rack", Operator.GROUP_BY, ""),
  ...
  test("Does not select any task without constraint") {
    Given(
      "app with hostname group_by and 10 tasks even distributed on 5 hosts")
    val app = AppDefinition()
    val tasks = 0
      .to(9)
      .map(num =>
  ...
  test("RackConstraints") {
    ...
    val clusterFreshRackMet = Constraints.meetsConstraint(
      freshRack,
      makeOffer("foohost",
                Set(TextAttribute("foo", "bar"),
                    TextAttribute("rackid", "rack-1"))),
      clusterByRackId)
    ...
    val clusterRackNotMet = Constraints.meetsConstraint(
      ...
      makeOffer("foohost",
                Set(TextAttribute("foo", "bar"),
                    TextAttribute("rackid", "rack-2"))),
      clusterByRackId)

    assert(!clusterRackNotMet, "Should not meet cluster constraint.")

    val clusterNoAttributeNotMet =
      Constraints.meetsConstraint(freshRack,
  ...
  test("RackGroupedByConstraints") {
    ...
    val groupByNoAttributeNotMet =
      Constraints.meetsConstraint(sameRack,
                                  makeOffer("foohost", Set()),
                                  group2ByRack)
  ...
  test("HostnameGroupedByConstraints") {
    ...
    val groupByFreshHostMet =
      Constraints.meetsConstraint(groupHost,
