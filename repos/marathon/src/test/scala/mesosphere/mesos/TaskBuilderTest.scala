...
class TaskBuilderTest extends MarathonSpec with Matchers {
  ...
  test("RackAndHostConstraints") {
    ...
    val offerRack1HostB = MarathonTestHelper
      ...
      .addAttributes(TextAttribute("rackid", "1"))
      .build
    shouldNotBuildTask("Should not take offer for the same rack",
                       offerRack1HostB)
  ...
  test("UniqueHostNameAndClusterAttribute") {
    ...
    val offerHostA = MarathonTestHelper
      ...
      .build
    shouldNotBuildTask("Should not take an offer with spark:disabled",
                       offerHostA)
