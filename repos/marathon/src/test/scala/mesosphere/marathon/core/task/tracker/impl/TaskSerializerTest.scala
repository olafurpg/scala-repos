...
class TaskSerializerTest
  ...
  test("minimal marathonTask => Task") {
    ...
    val expectedState =
      MarathonTestHelper.mininimalTask(f.taskId.idString, now)
  ...
  test("full marathonTask with NetworkInfoList => Task") {
    ...
    Given("a MarathonTask with all fields and host ports")
    val marathonTask =
      f.completeTask.toBuilder.addAllNetworks(f.sampleNetworks.asJava).build()
  ...
  class Fixture {
    ...
    val fullSampleTaskStateWithoutNetworking: Task.LaunchedOnReservation =
      Task.LaunchedOnReservation(
        ...
        reservation =
          Task.Reservation(Seq(LocalVolumeId(appId, "my-volume", "uuid-123")),
    ...
    private[this] def attribute(name: String,
