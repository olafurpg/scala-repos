...
class AppTaskLauncherActorTest extends MarathonSpec with GivenWhenThen {
  ...
  test("Process task launch") {
    ...
    Await
      .result(launcherRef ? ActorOfferMatcher.MatchOffer(
                  clock.now() + 1.seconds, offer),
              3.seconds)
  ...
  for (update <- Seq(
    ...
    test(
      ...
      launcherRef ! RateLimiterActor.DelayUpdate(appWithConstraints,
                                                 clock.now())
      ...
      Await
        .result(launcherRef ? update.withTaskId(marathonTask.taskId).wrapped,
                3.seconds)
        .asInstanceOf[QueuedTaskInfo]
