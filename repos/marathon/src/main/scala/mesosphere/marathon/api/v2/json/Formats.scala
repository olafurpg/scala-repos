...
trait Formats
  ...
  implicit lazy val ParameterFormat: Format[Parameter] =
    ((__ \ "key").format[String] ~ (__ \ "value")
      .format[String])(Parameter(_, _), unlift(Parameter.unapply))
...
trait ContainerFormats {
  ...
  implicit lazy val DockerNetworkFormat: Format[DockerInfo.Network] =
    enumFormat(DockerInfo.Network.valueOf,
  ...
  implicit lazy val PortMappingFormat: Format[Docker.PortMapping] =
    ((__ \ "containerPort")
      ...
      .formatNullable[Int]
      .withDefault(AppDefinition.RandomPortValue) ~ (__ \ "servicePort")
      .formatNullable[Int]
      .withDefault(AppDefinition.RandomPortValue) ~ (__ \ "protocol")
      .formatNullable[String]
      .withDefault("tcp") ~ (__ \ "name").formatNullable[String] ~
      (__ \ "labels")
        .formatNullable[Map[String, String]]
        .withDefault(Map.empty[String, String]))(PortMapping(_, _, _, _, _, _),
                                                 unlift(PortMapping.unapply))

  implicit lazy val DockerFormat: Format[Docker] = ((__ \ "image")
    .format[String] ~ (__ \ "network").formatNullable[DockerInfo.Network] ~
  ...
  implicit lazy val VolumeFormat: Format[Volume] = ((__ \ "containerPath")
    .format[String] ~ (__ \ "hostPath").formatNullable[String] ~
    (__ \ "mode").format[mesos.Volume.Mode] ~ (__ \ "persistent")
    .formatNullable[PersistentVolumeInfo])(Volume(_, _, _, _),
                                           unlift(Volume.unapply))

  implicit lazy val ContainerTypeFormat: Format[mesos.ContainerInfo.Type] =
    enumFormat(mesos.ContainerInfo.Type.valueOf,
               str => s"$str is not a valid container type")

  implicit lazy val ContainerFormat: Format[Container] = ((__ \ "type")
    .formatNullable[mesos.ContainerInfo.Type]
    .withDefault(mesos.ContainerInfo.Type.DOCKER) ~ (__ \ "volumes")
    .formatNullable[Seq[Volume]]
    .withDefault(Nil) ~ (__ \ "docker")
...
trait IpAddressFormats {
  ...
  private[this] lazy val ValidPorts: Reads[Seq[DiscoveryInfo.Port]] = {
    ...
    implicitly[Reads[Seq[DiscoveryInfo.Port]]]
      .filter(ValidationError("Port names are not unique."))(
        hasUniquePortNames)
  ...
  implicit lazy val IpAddressFormat: Format[IpAddress] =
    ((__ \ "groups").formatNullable[Seq[String]].withDefault(Nil) ~
      ...
      .withDefault(DiscoveryInfo.empty))(IpAddress(_, _, _),
...
trait DeploymentFormats {
  ...
  implicit lazy val GroupUpdateFormat: Format[GroupUpdate] =
    ((__ \ "id").formatNullable[PathId] ~ (__ \ "apps")
      ...
      .formatNullable[Double] ~ (__ \ "version")
      .formatNullable[Timestamp])(GroupUpdate(_, _, _, _, _, _),
...
trait EventFormats {
  ...
  implicit lazy val ApiPostEventWrites: Writes[ApiPostEvent] = Writes {
    event =>
      Json.obj(
  ...
  implicit lazy val MesosStatusUpdateEventWrites: Writes[
    MesosStatusUpdateEvent] = Json.writes[MesosStatusUpdateEvent]
  implicit lazy val MesosFrameworkMessageEventWrites: Writes[
    MesosFrameworkMessageEvent] = Json.writes[MesosFrameworkMessageEvent]
  implicit lazy val SchedulerDisconnectedEventWrites: Writes[
    SchedulerDisconnectedEvent] = Json.writes[SchedulerDisconnectedEvent]
  implicit lazy val SchedulerRegisteredEventWritesWrites: Writes[
    SchedulerRegisteredEvent] = Json.writes[SchedulerRegisteredEvent]
  implicit lazy val SchedulerReregisteredEventWritesWrites: Writes[
    SchedulerReregisteredEvent] = Json.writes[SchedulerReregisteredEvent]
...
trait HealthCheckFormats {
  ...
  implicit lazy val HealthCheckFormat: Format[HealthCheck] = {
    ...
    ((__ \ "path").formatNullable[String] ~ (__ \ "protocol")
      ...
      .withDefault(DefaultProtocol) ~ (__ \ "portIndex")
      .formatNullable[Int] ~ (__ \ "command").formatNullable[Command] ~
      (__ \ "gracePeriodSeconds")
        .formatNullable[Long]
        .withDefault(DefaultGracePeriod.toSeconds)
        .asSeconds ~ (__ \ "intervalSeconds")
      .formatNullable[Long]
      .withDefault(DefaultInterval.toSeconds)
      .asSeconds ~ (__ \ "timeoutSeconds")
      .formatNullable[Long]
      .withDefault(DefaultTimeout.toSeconds)
      .asSeconds ~ (__ \ "maxConsecutiveFailures")
      .formatNullable[Int]
      .withDefault(DefaultMaxConsecutiveFailures) ~ (__ \ "ignoreHttp1xx")
      .formatNullable[Boolean]
      .withDefault(DefaultIgnoreHttp1xx) ~ (__ \ "port")
...
trait FetchUriFormats {
  ...
  implicit lazy val FetchUriFormat: Format[FetchUri] = {
    ((__ \ "uri").format[String] ~ (__ \ "extract")
      .formatNullable[Boolean]
      .withDefault(true) ~ (__ \ "executable")
      .formatNullable[Boolean]
      .withDefault(false) ~ (__ \ "cache")
      .formatNullable[Boolean]
...
trait AppAndGroupFormats {
  ...
  implicit lazy val AppDefinitionReads: Reads[AppDefinition] = {
    ...
    ((__ \ "id").read[PathId].filterNot(_.isRoot) ~ (__ \ "cmd")
      .readNullable[String](Reads.minLength(1)) ~ (__ \ "args")
      .readNullable[Seq[String]] ~ (__ \ "user").readNullable[String] ~
      (__ \ "env")
        .readNullable[Map[String, String]]
        .withDefault(AppDefinition.DefaultEnv) ~ (__ \ "instances")
      .readNullable[Int]
      .withDefault(AppDefinition.DefaultInstances) ~ (__ \ "cpus")
      .readNullable[Double]
      .withDefault(AppDefinition.DefaultCpus) ~ (__ \ "mem")
      .readNullable[Double]
      .withDefault(AppDefinition.DefaultMem) ~ (__ \ "disk")
      .readNullable[Double]
      .withDefault(AppDefinition.DefaultDisk) ~ (__ \ "executor")
      .readNullable[String](Reads.pattern(executorPattern))
      .withDefault(AppDefinition.DefaultExecutor) ~ (__ \ "constraints")
      .readNullable[Set[Constraint]]
      .withDefault(AppDefinition.DefaultConstraints) ~ (__ \ "storeUrls")
      .readNullable[Seq[String]]
      .withDefault(AppDefinition.DefaultStoreUrls) ~ (__ \ "requirePorts")
      .readNullable[Boolean]
      .withDefault(AppDefinition.DefaultRequirePorts) ~
      (__ \ "backoffSeconds")
        .readNullable[Long]
        .withDefault(AppDefinition.DefaultBackoff.toSeconds)
        .asSeconds ~ (__ \ "backoffFactor")
      .readNullable[Double]
      ...
      val extraReads: Reads[ExtraFields] = ((__ \ "uris")
        .readNullable[Seq[String]]
        ...
        .withDefault(Timestamp.now()) ~ (__ \ "residency")
        .readNullable[Residency] ~ (__ \ "portDefinitions")
        ...
        .filter(ValidationError(
          ...
          val appWithoutPorts =
            extra.maybePorts.forall(_.isEmpty) &&
  ...
  implicit lazy val AppUpdateReads: Reads[AppUpdate] =
    ((__ \ "id").readNullable[PathId].filterNot(_.exists(_.isRoot)) ~
      (__ \ "cmd").readNullable[String](Reads.minLength(1)) ~ (__ \ "args")
      .readNullable[Seq[String]] ~ (__ \ "user").readNullable[String] ~
      (__ \ "env").readNullable[Map[String, String]] ~ (__ \ "instances")
      .readNullable[Int] ~ (__ \ "cpus").readNullable[Double] ~
      (__ \ "mem").readNullable[Double] ~ (__ \ "disk").readNullable[Double] ~
      (__ \ "executor").readNullable[String](
        Reads.pattern("^(//cmd)|(/?[^/]+(/[^/]+)*)|$".r)) ~
      (__ \ "constraints").readNullable[Set[Constraint]] ~ (__ \ "storeUrls")
      .readNullable[Seq[String]] ~ (__ \ "requirePorts")
      .readNullable[Boolean] ~ (__ \ "backoffSeconds")
      .readNullable[Long]
      .map(_.map(_.seconds)) ~ (__ \ "backoffFactor").readNullable[Double] ~
      ...
      .flatMap { update =>
        ...
        val extraReads: Reads[ExtraFields] =
          ((__ \ "uris").readNullable[Seq[String]] ~ (__ \ "fetch")
            ...
            .readNullable[Residency] ~ (__ \ "ports").readNullable[Seq[Int]](
            uniquePorts) ~ (__ \ "portDefinitions")
            .readNullable[Seq[PortDefinition]])(ExtraFields)
  ...
  implicit lazy val GroupFormat: Format[Group] = ((__ \ "id").format[PathId] ~
    (__ \ "apps")
      .formatNullable[Set[AppDefinition]]
      .withDefault(Group.defaultApps) ~ (__ \ "groups")
    .lazyFormatNullable(implicitly[Format[Set[Group]]])
    .withDefault(Group.defaultGroups) ~ (__ \ "dependencies")
    .formatNullable[Set[PathId]]
    .withDefault(Group.defaultDependencies) ~ (__ \ "version")
    .formatNullable[Timestamp]
    .withDefault(Group.defaultVersion))(Group(_, _, _, _, _),
...
trait PluginFormats {
  ...
  implicit lazy val pluginDefinitionFormat: Writes[PluginDefinition] =
    ((__ \ "id").write[String] ~ (__ \ "plugin").write[String] ~
      ...
      .writeNullable[Set[String]] ~ (__ \ "info").writeNullable[JsObject])(d =>
