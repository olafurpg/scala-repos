...
private[manager] object OfferMatcherManagerActor {
  def props(metrics: OfferMatcherManagerActorMetrics,
            random: Random,
            clock: Clock,
  ...
  private case class OfferData(offer: Offer,
                               deadline: Timestamp,
                               sender: ActorRef,
...
private[impl] class OfferMatcherManagerActor private (
  ...
  private[this] def receiveProcessOffer: Receive = {
    case ActorOfferMatcher.MatchOffer(deadline, offer: Offer)
      ...
      sender() ! OfferMatcher.MatchedTaskOps(offer.getId,
                                             Seq.empty,
    ...
    case ActorOfferMatcher.MatchOffer(deadline, offer: Offer) =>
      ...
      val data = OfferMatcherManagerActor.OfferData(offer,
                                                    deadline,
                                                    sender(),
                                                    randomizedMatchers,
                                                    Seq.empty)
  ...
  private[this] def sendMatchResult(data: OfferData,
    ...
    data.sender ! OfferMatcher.MatchedTaskOps(data.offer.getId,
                                              data.ops,
                                              resendThisOffer)
