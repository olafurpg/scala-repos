...
import mesosphere.marathon.core.launchqueue.impl.{AppTaskLauncherActor, LaunchQueueActor, LaunchQueueDelegate, RateLimiter, RateLimiterActor}
...
/**
  ...
  */
class LaunchQueueModule(config: LaunchQueueConfig,
  ...
  private[this] val rateLimiterActor: ActorRef = {
    val props =
      RateLimiterActor.props(rateLimiter, appRepository, launchQueueActorRef)
