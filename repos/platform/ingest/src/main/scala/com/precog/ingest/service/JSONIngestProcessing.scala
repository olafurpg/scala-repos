...
final class JSONIngestProcessing(
  ...
  final class IngestProcessor extends IngestProcessorLike {
    def ingestJSONChunk(
      ...
      def ingestAllOrNothing(state: JSONParseState,
      ...
      def ingestUnbuffered(state: JSONParseState,
                           stream: StreamT[Future, Array[Byte]],
      ...
      def ingestFinalBlock(parsed: AsyncParse,
      ...
      def ingestBlock(parsed: AsyncParse,
                      updatedParser: AsyncParser,
                      state: JSONParseState,
                      streamRef: StreamRef)(
        ...
        (errorHandling: @unchecked) match {
          case IngestAllPossible =>
            ...
            storage
              .store(apiKey, path, authorities, toIngest, jobId, streamRef)
              .flatMap {
                _.fold(
                  storeFailure =>
                    sys.error(
    ...
    def ingest(durability: Durability,
      ...
      durability match {
        ...
        case GlobalDurability(jobId) =>
          ingestJSONChunk(errorHandling, storeMode, Some(jobId), dataStream)
            .map {
              case IngestReport(ingested, errors) =>
                BatchResult(ingested + errors.size,
                            ingested,
