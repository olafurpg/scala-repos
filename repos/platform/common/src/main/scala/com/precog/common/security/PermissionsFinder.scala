...
class PermissionsFinder[M[+ _]: Monad](val apiKeyFinder: APIKeyFinder[M],
  ...
  def inferWriteAuthorities(apiKey: APIKey,
                            path: Path,
    ...
    def selectWriter(
      ...
      eithers
        .traverse(_.bisequence[M, Option[Authorities], Option[Authorities]])
        .map { (perms: List[Option[Authorities] \/ Option[Authorities]]) =>
          perms
            .collectFirst {
  ...
  def checkWriteAuthorities(authorities: Authorities,
                            apiKey: APIKey,
                            path: Path,
