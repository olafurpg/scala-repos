...
object EventEncoding extends EncodingFlags with Logging {
  def toMessageBytes(event: Event) = {
    ...
    writeHeader(bytes, event.fold(_ => jsonIngestFlag, _ =>
              jsonArchiveFlag, _ => storeFileFlag))
    bytes.put(msgBuffer)
    bytes.flip()
    bytes
  }
...
object EventMessageEncoding extends EncodingFlags with Logging {
  def toMessageBytes(msg: EventMessage) = {
    ...
    val msgBuffer = charset.encode(serialized)
    val bytes = ByteBuffer.allocate(msgBuffer.limit + 3)
    writeHeader(bytes, msg.fold(_ => jsonIngestMessageFlag, _ =>
              jsonArchiveMessageFlag, _ => storeFileFlag))
    bytes.put(msgBuffer)
    bytes.flip()
    bytes
  }
    ...
    for {
      ...
      message <- msgType match {
                  case `jsonIngestMessageFlag` =>
                    jv.validated[EventMessageExtraction](
