        ...
        request.parameters
          ...
          .map { apiKey =>
            jobs.listJobs(apiKey).map { jobs =>
              HttpResponse(OK,
                           content =
                             Some(JArray((jobs.map(_.serialize)).toList)))
            }
          }
          .getOrElse {
            Future(
...
class CreateJobHandler(jobs: JobManager[Future],
              ...
              case (name, tpe, _) =>
                Future(
                  HttpResponse[JValue](
                    BadRequest,
                    content = Some(
                      JString("Expected `name` and `type` to be strings, but found '%s' and '%s'."
                        .format(name, tpe)))))
            }
          })
        }
            ...
            (obj \ "state") match {
              case JString("started") =>
                transition(obj) { (timestamp, _) =>
                  jobs
                    .start(jobId, timestamp)
                    .map(Validation.fromEither(_))
                    .map(_.map(_.state))
                }
              ...
              case JString(state) =>
                Future(
                  HttpResponse[JValue](
                    ...
                    content = Some(
                      JString(
                        "Invalid 'state '%s'. Expected one of 'started', 'cancelled', 'finished', 'aborted' or 'expired'."
                          .format(state)
                      ))))
...
class GetResultHandler(jobs: JobManager[Future])(
    implicit ctx: ExecutionContext)
    extends CustomHttpService[ByteChunk, Future[HttpResponse[ByteChunk]]] {
  private implicit val M = new FutureMonad(ctx)
