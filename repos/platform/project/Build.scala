...
object PlatformBuild extends Build {
  ...
  val jprofilerSettings = Seq(
    ...
    jprofilerId := "116",
    javaOptions in profileTask <<= (javaOptions,
                                    jprofilerLib,
                                    jprofilerConf,
                                    jprofilerId,
                                    baseDirectory) map {
  ...
  val commonAssemblySettings =
    sbtassembly.Plugin.assemblySettings ++ Seq(test in assembly := {}) ++ commonNexusSettings
  ...
  lazy val gjallerhorn =
    Project(id = "gjallerhorn", base = file("gjallerhorn"))
      .settings(commonAssemblySettings: _*)
      .dependsOn(quirrel,
                 mimir,
  ...
  lazy val performance =
    Project(id = "performance", base = file("performance"))
      .settings(commonNexusSettings: _*)
      .dependsOn(ingest,
                 common % "compile->compile;test->test",
                 quirrel,
