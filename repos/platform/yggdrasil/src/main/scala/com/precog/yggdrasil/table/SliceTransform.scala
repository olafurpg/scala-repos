...
trait SliceTransforms[M[+ _]]
  ...
  protected object SliceTransform {
    ...
    def left[A](initial: A) =
      SliceTransform2
        .liftM[A](initial, (a: A, sl: Slice, sr: Slice) => (a, sl))
    def right[A](initial: A) =
      SliceTransform2
        .liftM[A](initial, (a: A, sl: Slice, sr: Slice) => (a, sr))
          ...
          predTransform.zip2(leftTransform, rightTransform) {
            (predS, leftS, rightS) =>
              new Slice {
                ...
                val columns: Map[ColumnRef, Column] = {
                  predS.columns
                    .get(ColumnRef(CPath.Identity, CBoolean))
                    .map { predC =>
                      val leftMask =
  ...
  protected sealed trait SliceTransform1[A] {
    ...
    def zip2[B, C](t: SliceTransform1[B], t2: SliceTransform1[C])(
        combine: (Slice, Slice, Slice) => Slice)
      : SliceTransform1[(A, B, C)] = {
...
trait ObjectConcatHelpers extends ConcatHelpers {
  ...
  def buildEmptyObjects(emptyBits: BitSet) = {
    if (emptyBits.isEmpty) Map.empty[ColumnRef, Column]
      ...
      Map(
        ColumnRef(CPath.Identity, CEmptyObject) -> EmptyObjectColumn(
          emptyBits))
  }
