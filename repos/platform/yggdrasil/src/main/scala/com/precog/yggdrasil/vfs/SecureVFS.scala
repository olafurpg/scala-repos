        ...
        case other =>
          for {
            ...
            permitted <- EitherT.right(
                          permissionsFinder.findBrowsableChildren(apiKey,
                                                                  path))
          } yield {
            children filter {
    ...
    final def executeStoredQuery(
      ...
      def fallBack =
        if (onlyIfCached) {
          ...
          for {
            basePath <- pathPrefix
            caching <- executeAndCache(platform,
                                       path,
                                       ctx,
                                       queryOptions,
      ...
      EitherT.right(vfs.currentVersion(cachePath)) flatMap {
        case Some(VersionEntry(id, _, timestamp))
          ...
          logger.debug(
            "Found fresh cache entry (%s) for query on %s".format(timestamp,
                                                                  path))
        ...
        result <- cacheAt match {
                   case Some(cachePath) =>
                     for {
                       ...
                       // best.
                       val stream = persistingStream(ctx.apiKey,
                                                     cachePath,
