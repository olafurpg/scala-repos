...
trait ActorVFSModule extends VFSModule[Future, Slice] {
  ...
  /**
    ...
    */
  class ResourceBuilder(actorSystem: ActorSystem,
                        clock: Clock,
                        chef: ActorRef,
                        cookThreshold: Int,
      ...
      for {
        ...
        blobResult <- IOT {
          writeResult.traverse { size =>
            ...
            IOUtils
              .writeToFile(metadata.serialize.renderCompact,
                           new File(versionDir, blobMetadataFilename))
              .map { _ =>
  ...
  final case class FileBlobResource(dataFile: File, metadata: BlobMetadata)
    ...
    override def fold[A](blobResource: BlobResource => A,
  ...
  class ActorVFS(projectionsActor: ActorRef,
    ...
    def writeAllSync(data: Seq[(Long, EventMessage)])
      ...
      for {
        ...
        allResults <- (data
          .groupBy { case (offset, msg) => msg.path })
          .toStream
          .traverse {
            case (path, subset) =>
  ...
  case class IngestBundle(data: Seq[(Long, EventMessage)],
                          perms: Map[APIKey, Set[WritePermission]])

  class PathRoutingActor(baseDir: File,
    ...
    def receive = {
      ...
      case FindPathMetadata(path) =>
        ...
        val eio =
          VFSPathUtils
            .currentPathMetadata(baseDir, path)
            .map { pathMetadata =>
                ...
                targetActor(path)
                  .map { pathActor =>
                    pathMessages
                      ...
                      .traverse { apiKey =>
                        permissionsFinder
                          .writePermissions(apiKey, path, clock.instant())
                          .map {
                            apiKey -> _
                          }
                      }
                        ...
                        val (totalArchives, totalEvents, totalStoreFiles) =
                          pathMessages.foldLeft((0, 0, 0)) {
                            case ((archived, events, storeFiles),
                                  (_,
                                   IngestMessage(_, _, _, data, _, _, _))) =>
                  ...
                  .except {
                    case t: Throwable =>
                      IO(
                        logger.error(
                          "Failure during version log open on " + path,
                          t))
                  }
            }
  ...
  final class PathManagerActor(path: Path,
    ...
    private def performCreate(apiKey: APIKey,
                              data: PathData,
                              version: UUID,
                              writeAs: Authorities,
    ...
    def processEventMessages(msgs: Stream[(Long, EventMessage)],
        ...
        openResource(version).flatMap {
          _.fold(
            blob =>
              left(
                IO(
                  NotFound("Located resource on %s is a BLOB, not a projection"
                    .format(path.path)))),
            db => right(IO(db))
          )
        }
      ...
      def persistNIHDB(createIfAbsent: Boolean,
                       offset: Long,
                       msg: IngestMessage,
                       streamId: UUID,
            ...
            .fold[IO[PrecogUnit]](
              ...
              resource =>
                for {
                  ...
                  maybeCompleteJob(msg, terminal, UpdateSuccess(msg.path))
                    .pipeTo(requestor)
                  PrecogUnit
              }
            )
      ...
      def persistFile(createIfAbsent: Boolean,
                      offset: Long,
                      msg: StoreFileMessage,
                      streamId: UUID,
                      terminal: Boolean): IO[PrecogUnit] = {
        logger.debug(
          "Persisting file on %s for offset %d".format(path, offset))
    ...
    def receive = {
      case ReceiveTimeout =>
        ...
        val quiesce =
          versions.values.toStream
            .collect { case NIHDBResource(db) => db }
            .traverse(_.quiesce)
