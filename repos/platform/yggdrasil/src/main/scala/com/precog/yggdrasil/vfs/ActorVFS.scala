        ...
        blobResult <- IOT {
                       writeResult traverse { size =>
                         ...
                         IOUtils.writeToFile(
                             ...
                             new File(versionDir, blobMetadataFilename)) map {
                           _ =>
    ...
    def writeAllSync(data: Seq[(Long, EventMessage)])
      ...
      for {
        ...
        allResults <- (data groupBy { case (offset, msg) => msg.path }).toStream traverse {
                       case (path, subset) =>
                         (projectionsActor ? IngestData(subset))
                           .mapTo[WriteResult]
                     }
      } yield {
          ...
          messages
            ...
            case (path, pathMessages) =>
              targetActor(path) map { pathActor =>
                ...
                case t: Throwable =>
                  IO(logger.error("Failure during version log open on " + path,
                                  t))
              }
          }
      ...
      versions.get(version) map { r =>
        ...
        versionLog.find(version) map {
          case VersionEntry(v, _, _) =>
            ...
            val openf =
              if (NIHDB
                    .hasProjection(dir)) { resourceBuilder.openNIHDB _ } else {
              ...
              for {
                ...
                _ <- complete.whenM(
                        versionLog.completeVersion(version) >> versionLog
                          .setHead(version) >> maybeExpireCache(apiKey,
                                                                resource))
              } yield PrecogUnit
            }
      } yield {
            ...
            .fold[IO[PrecogUnit]](
                ...
                resource =>
                  for {
                    ...
                    logger.trace(
                        "Sent insert message for " + msg + " to nihdb")
