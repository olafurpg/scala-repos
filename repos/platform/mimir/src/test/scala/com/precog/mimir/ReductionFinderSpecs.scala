...
trait ReductionFinderSpecs[M[+ _]]
  ...
  "mega reduce" should {
    ...
    "in a reduction of a singleton" in {
      ...
      val megaR =
        dag.MegaReduce(List((trans.Leaf(trans.Source), List(input.red))),
    ...
    "in joins where transpecs are eq, wrap object, operate, filter" in {
      ...
      val obj = Join(
          WrapObject, Cross(None), Const(CString("bar"))(line), clicks)(line)
      ...
      val nonEqTrans =
        trans.Map1(trans.Equal(fooDerefTrans,
                               trans.ConstLiteral(CNum(5), fooDerefTrans)),
      ...
      val filterTrans =
        trans.Filter(trans.Leaf(trans.Source),
                     trans.Equal(bazDerefTrans,
                                 trans.ConstLiteral(CNum(12), bazDerefTrans)))

      val reductions: List[(trans.TransSpec1, List[Reduction])] =
        List((filterTrans, List(StdDev)),
             (opTrans, List(Max)),
      ...
      val input = Join(Add,
                       ...
                       Join(Add,
                            ...
                            Join(Add,
                                 Cross(Some(CrossRight)),
                                 r3,
                                 Join(Add, Cross(Some(CrossRight)), r4, r5)(
                                     line))(line))(line))(line)
    ...
    "where three reductions use three different trans specs" in {
      ...
      val mega = dag.MegaReduce(
          List(
              (DerefObjectStatic(Leaf(Source), CPathField("userId")),
               List(r1.red)),
    ...
    "in a split" in {
      ...
      val parent = Filter(IdentitySort,
                          nums,
                          Join(Lt, Cross(None), nums, SplitParam(0, id)(line))(
                              line))(line) //TODO need a window function

      val megaR =
        MegaReduce(List((trans.Leaf(trans.Source), List(reduction))), parent)

      val expected = dag.Split(dag.Group(1, nums, UnfixedSolution(0, nums)),
                               Join(Add,
                                    Cross(None),
                                    SplitGroup(1, nums.identities, id)(line),
                                    joinDeref(megaR, 0, 0, line))(line),
    ...
    "in a split that contains two reductions of the same dataset" in {
          ...
          Join(JoinObject,
               ...
               Join(JoinObject,
                    ...
                    Join(WrapObject,
                         ...
                         dag.Reduce(Min,
                                    SplitGroup(1,
                                               Identities.Specs(Vector(LoadIds(
                                                           "/clicks"))),
                                               id)(line))(line))(line),
                    Join(WrapObject,
                         Cross(None),
      ...
      val megaR = MegaReduce(
          List((trans.Leaf(trans.Source), List(red1.red, red2.red))), parent)
          ...
          Join(JoinObject,
               ...
               Join(JoinObject,
                    ...
                    Join(WrapObject,
                         Cross(None),
                         Const(CString("max"))(line),
                         joinDeref(megaR, 0, 0, line))(line))(line))(line),
          id)(line)
  ...
  "reduction finder" should {
    ...
    "findReductions inside a Split" in {
      ...
      val input = dag.Split(dag.Group(1, clicks, UnfixedSolution(0, count)),
        ...
        dag.Split(group1,
                  Join(JoinObject,
                       ...
                       Join(JoinObject,
                            ...
                            Join(WrapObject, Cross(None), maxRoot, r2)(line))(
                           line))(line),
                  id)(line)
