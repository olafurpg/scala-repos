...
trait EvaluatorSpecs[M[+ _]]
  ...
  "evaluator" should {
    ...
    "evaluate a rewrite/eval of a 3-way mega reduce" in {
      ...
      val id = Join(DerefObject,
                    Cross(None),
                    load,
                    Const(CString("userId"))(line))(line)
      val height = Join(DerefObject,
                        Cross(None),
                        load,
                        Const(CString("height"))(line))(line)
      val weight = Join(DerefObject,
                        Cross(None),
                        load,
    ...
    "MegaReduce must return an array" in {
      ...
      val input =
        dag.MegaReduce(List((trans.Leaf(trans.Source), List(Count, Sum))),
                       parent)
            ...
            dag.Reduce(Max,
                       Filter(IdentitySort,
                              ...
                              Join(Lt,
                                   Cross(None),
                                   nums,
                                   SplitParam(0, id)(line))(line))(line))(line)
          )(line),
          id
        )(line)
