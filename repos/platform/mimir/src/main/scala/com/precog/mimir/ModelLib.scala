  ...
  trait ModelSupport {
      ...
      trait ModelCompanion {
        def idRes(cols: Map[ColumnRef, Column],
          ...
          val keyCols =
            cols filter {
              case (
                  ColumnRef(CPath(paths.Key, CPathIndex(_), _ @_ *), _),
                  _) =>
                true
              case _ => false
            }
    ...
    trait RegressionModelBase extends ModelBase with RegressionSupport {
      ...
      trait RegressionModelCompanion extends ModelCompanion {
        ...
        case class ScannerPrelims(includedModel: Map[ColumnRef, Column],
                                  definedModel: BitSet,
                                  cpaths: Seq[CPath],
                                  includedCols: Map[CPath, DoubleColumn],
                                  resultArray: Array[Double])

        def makePrelims(model: Model,
          ...
          val includedCols = includedModel
            .collect {
    ...
    trait LinearModelBase extends RegressionModelBase {
      ...
      protected val reducer: CReducer[Models] = new CReducer[Models] {
        def reduce(schema: CSchema, range: Range): Models = {
          ...
          val modelNames: Set[String] = schema.columnRefs
            .collect {
              case ColumnRef(
              ...
              .collect {
                case ColumnRef(
                    path @ CPath(
                      ...
                      rest @ _ *),
                    _)
                    if rest.length > 0 && rest.last == CPathField(
                      `estimate`) =>
                  (modelName, path)
              }
              .groupBy(_._1) map {
