...
trait StdLibEvaluatorStack[M[+ _]]
    extends EvaluatorModule[M] with StdLibModule[M]
    with StdLibOpFinderModule[M] with StdLibStaticInlinerModule[M]
    with ReductionFinderModule[M]
    with JoinOptimizerModule[M] with PredicatePullupsModule[M] {
  ...
  abstract class Evaluator[N[+ _]](N0: Monad[N])(
      ...
      extends EvaluatorLike[N](N0)(mn, nm) with StdLibOpFinder
