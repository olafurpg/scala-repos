...
trait Solver extends parser.AST with typer.Binder {
  ...
  def solve(tree: Expr, sigma: Sigma)(
    ...
    lazy val Rules: Set[PartialFunction[Expr, Set[Expr]]] = Set({
      ...
      case Add(loc, Mul(loc2, w, x), Mul(loc3, y, z)) if x equalsIgnoreLoc z =>
        Set(Mul(loc2, Add(loc, w, y), x))
    }, {
      case Sub(loc, Mul(loc2, x, y), z) if y equalsIgnoreLoc z =>
        Set(Mul(loc2, Sub(loc, x, NumLit(loc, "1")), y))
    }, {
      case Sub(loc, Mul(loc2, w, x), Mul(loc3, y, z)) if x equalsIgnoreLoc z =>
    ...
    def search(work: Set[List[Expr]],
      ...
      if (filteredWork.isEmpty) {
        ...
        val (results2, newWork) =
          filteredWork partition { xs =>
  ...
  def solveComplement(
      c: Comp, sigma: Sigma): PartialFunction[Node, Boolean] => Option[Expr] =
    c.child match {
