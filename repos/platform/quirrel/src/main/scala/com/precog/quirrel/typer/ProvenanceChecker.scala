...
trait ProvenanceChecker extends parser.AST with Binder {
    ...
    def handleUnion(expr: Expr,
      ...
      val (provenance, errors, constr) =
        if (left.provenance == InfiniteProvenance ||
          ...
          handleUnionLike(UnionProvenanceDifferentLength)(left.provenance,
                                                          right.provenance)
        }
    ...
    def handleIntersect(expr: Expr,
      ...
      val (prov, errors, constr) = {
        if (left.provenance == right.provenance)
          ...
          val commonality = Commonality(left.provenance,
                                        right.provenance,
      ...
      val back: (Set[Error], Set[ProvConstraint]) = expr match {
        ...
        case expr @ Dispatch(_, name, actuals) => {
          expr.binding match {
            ...
            case Op2Binding(_) =>
              val (provenance, result) = handleBinary(expr,
                                                      actuals(0),
                                                      actuals(1),
                                                      relations,
                                                      constraints)
  ...
  sealed trait Provenance {
    ...
    private def findChildren(prov: Provenance,
