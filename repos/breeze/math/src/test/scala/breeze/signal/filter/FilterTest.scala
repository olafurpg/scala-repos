...
class FilterTest extends FunSuite {
  ...
  test(
    ...
    val spTestSignalFiltered1 = DenseVector(-0.01789505, -0.02088871,
      0.14061118, 0.38912801, 0.17161298, -0.54231618, -0.93826582,
      -0.69101276, 0.1064299, 0.91305795, 0.67988086, -0.05227671, 0.01563969,
      0.30337291, -0.00920251, -0.42358677, -0.48613482, 0.04960337, 0.6784219,
      0.23758627)

    val firwin1 =
      designFilterFirwin[Double](11, DenseVector(0.25, 0.5), zeroPass = false)
    assert(norm(spFirwin1 - firwin1.kernel) < testNormThreshold,
           "generated kernel is incorrect!")

    val filtered1 = filterBP(testSignal,
                             (0.25, 0.5),
                             2d,
                             11,
