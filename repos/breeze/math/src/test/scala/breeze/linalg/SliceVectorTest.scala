...
class SliceVectorTest extends FunSuite {
  ...
  test("negative indexing tests") {
    ...
    assert(tempDV(4 to -1) == DenseVector(4, 5, 6),
           "Failed> tempDV( 4 to -1 ) =  " + tempDV(4 to -1))
    assert(tempDV(-3 to -1) == DenseVector(4, 5, 6),
    ...
    assert(tempDV(0 to 4 by 2) == DenseVector(0, 2, 4),
           "Failed> tempDV( 0 to 4 by 2 ) = " + tempDV(0 to 4 by 2))
    assert(tempDV(4 to 0 by -2) == DenseVector(4, 2, 0),
