...
//  }
  ...
  final def readInt32(n: Int): Array[Int] = {
    ...
    var c = 0
    while (c < n) {
      tr(c) = converter.bytesToInt32(ba(c * 4),
                                     ba(c * 4 + 1),
                                     ba(c * 4 + 2),
                                     ba(c * 4 + 3))
      c += 1
  ...
  final def readUInt32(n: Int): Array[Long] = {
    ...
    var c = 0
    while (c < n) {
      tr(c) = converter.bytesToUInt32(ba(c * 4),
                                      ba(c * 4 + 1),
                                      ba(c * 4 + 2),
                                      ba(c * 4 + 3))
      c += 1
...
abstract class ByteConverter {
  ...
  final def bytesToUInt64(b0: Byte,
                          b1: Byte,
                          b2: Byte,
                          b3: Byte,
                          b4: Byte,
...
object ByteConverterLittleEndian extends ByteConverter {
  ...
  override def bytesToInt64(b0: Byte,
                            b1: Byte,
                            b2: Byte,
                            b3: Byte,
                            b4: Byte,
