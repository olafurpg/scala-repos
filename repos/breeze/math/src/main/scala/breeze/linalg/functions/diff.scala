...
object diff extends UFunc with diffLowPrio {
  @expand
  @expand.valify
  implicit def implDV_Double_DV[
      @expand.args(Int, Long, Float, Double) T]: Impl2[
      DenseVector[T], Int, DenseVector[T]] = {
  ...
  @expand
  @expand.valify
  implicit def implDV_DV[@expand.args(Int, Long, Float, Double) T]: Impl[
...
sealed trait diffLowPrio {
  this: diff.type =>
  implicit def implVec[T, Vec](implicit vec: Vec <:< Vector[T],
                               ct: ClassTag[T],
    ...
    new Impl[Vec, DenseVector[T]] {
      override def apply(v: Vec): DenseVector[T] = {
        if (v.length <= 1) DenseVector[T]()
          ...
          DenseVector.tabulate(v.length - 1)(
              index => ring.-(v(index + 1), v(index)))
      }
    }
  }
}
