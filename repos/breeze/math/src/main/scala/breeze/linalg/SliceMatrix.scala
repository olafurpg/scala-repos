...
object SliceMatrix {
  implicit def canMapKeyValuePairs[
      K1, K2, V, V2 : ClassTag : Zero]: CanMapKeyValuePairs[
    ...
    new CanMapKeyValuePairs[
      ...
      override def map(from: SliceMatrix[K1, K2, V],
        ...
        DenseMatrix.tabulate(from.rows, from.cols)(
            (i, j) => fn((i, j), from(i, j)))
      }
  ...
  implicit def canMapValues[K1, K2, V, V2 : ClassTag : Zero]: CanMapValues[
      SliceMatrix[K1, K2, V], V, V2, DenseMatrix[V2]] = {
  ...
  implicit def canCreateZerosLike[
      K1, K2, V : ClassTag : Zero]: CanCreateZerosLike[
      SliceMatrix[K1, K2, V], DenseMatrix[V]] = {
  ...
  implicit def canIterateKeyValuePairs[K1, K2, V]: CanTraverseKeyValuePairs[
      SliceMatrix[K1, K2, V], (Int, Int), V] = {
  ...
  implicit def canTransformValues[K1, K2, V]: CanTransformValues[
      SliceMatrix[K1, K2, V], V] = {
