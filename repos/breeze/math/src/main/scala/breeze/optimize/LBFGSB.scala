...
class LBFGSB(lowerBounds: DenseVector[Double],
    ...
    extends FirstOrderMinimizer[DenseVector[Double],
        ...
        LBFGSB.defaultConvergenceCheck(lowerBounds,
                                       upperBounds,
                                       tolerance,
                                       maxIter))
  ...
  override protected def initialHistory(f: DiffFunction[DenseVector[Double]],
  ...
  /**
    ...
    */
  protected def findAlpha(xCauchy: DenseVector[Double],
                          du: Vector[Double],
  ...
  protected def updateSkYkHessianApproxMat(history: History,
    ...
    val newHistory = {
      ...
      if (0 == yHistory.cols) {
        ...
        history.copy(
            yHistory = DenseMatrix.horzcat(yHistory(::, 1 until m),
                                           newY.toDenseMatrix.t),
            sHistory = DenseMatrix.horzcat(sHistory(::, 1 until m),
                                           newS.toDenseMatrix.t)
        )
      }
    }
