...
object Expressions {
  ...
  val NUMBER: P[Ast.expr.Num] =
    P(Lexical.floatnumber | Lexical.longinteger | Lexical.integer | Lexical.imagnumber)
      .map(Ast.expr.Num)
  ...
  val atom: P[Ast.expr] = {
    ...
    P(
      empty_tuple | empty_list | empty_dict | "(" ~ (yield_expr | generator | tuple) ~ ")" | "[" ~
        ...
        .rep(1)
        .map(_.mkString)
        .map(Ast.expr.Str) | NAME
        .map(Ast.expr.Name(_, Ast.expr_context.Load)) | NUMBER
    )
  }
