...
abstract class GenJSCode
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    private def isTrivialConstructor(sym: Symbol,
                                     params: List[Symbol],
                                     rhs: Tree): Boolean = {
      if (!sym.isClassConstructor) {
        false
        ...
        rhs match {
          ...
          case Block(List(Apply(fun @ Select(_: Super, _), args)),
                     Literal(_)) =>
    ...
    def genReflCallProxies(sym: Symbol): List[js.MethodDef] = {
      ...
      val methods = sym.tpe.findMembers(excludedFlags = excludedFlags,
                                        requiredFlags = Flags.METHOD)
    ...
    private def genSuperCall(tree: Apply): js.Tree = {
      ...
      if (sym == Object_getClass) {
        ...
        val superCall = genApplyMethodStatically(genThis()(sup.pos),
                                                 sym,
    ...
    private def genNormalApply(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      if (sym.owner == StringClass && !isStringMethodFromObject) {
        ...
        genApplyMethodStatically(genExpr(receiver),
                                 sym,
                                 genActualArgs(sym, args))
      } else {
    ...
    def genIsInstanceOf(value: js.Tree, to: Type)(
      ...
      if (sym == ObjectClass) {
        ...
        if (sym.isTrait) {
          ...
          js.Unbox(js.JSBinaryOp(js.JSBinaryOp.instanceof,
                                 value,
            ...
            case NEG =>
              (resultType: @unchecked) match {
                ...
                case jstpe.FloatType =>
                  js.BinaryOp(js.BinaryOp.Float_-,
    ...
    /** Gen JS code for string concatenation.
      */
    private def genStringConcat(tree: Apply,
                                receiver: Tree,
    ...
    private def genApplyDynamic(tree: ApplyDynamic): js.Tree = {
      ...
      if (isEqOrNeq) {
        ...
        val callTrgVarDef = js.VarDef(callTrgIdent,
                                      receiverType,
                                      mutable = false,
                                      genExpr(receiver))
              ...
              sym.name match {
                ...
                case nme.apply =>
                  genRTCall(currentRun.runDefinitions.arrayApplyMethod,
                            ...
                            arguments(0))
                case nme.length =>
                  genRTCall(currentRun.runDefinitions.arrayLengthMethod,
                            callTrg)
                case nme.clone_ =>
        ...
        (genArgs match {
          ...
          case List(arg) =>
            ...
            def genFunctionToJSFunction(isThisFunction: Boolean): js.Tree = {
              ...
              val fCaptureParam = js.ParamDef(js.Ident("f"),
                                              inputIRType,
                                              mutable = false,
                                              rest = false)
              val jsArity =
      ...
      val boxedResult = sym.name match {
        ...
        case _ =>
          ...
          if (sym.hasFlag(reflect.internal.Flags.DEFAULTPARAM)) {
            ...
            js.JSBracketMethodApply(receiver,
    ...
    private def genNewString(tree: Apply): js.Tree = {
      ...
      if (compMembers.isEmpty) {
        ...
        genApplyMethod(genLoadModule(RuntimeStringModule),
                       compMembers.head,
                       args)
      }
    }
    ...
    private def genAnonFunction(originalFunction: Function): js.Tree = {
      ...
      val (allFormalCaptures, body, allActualCaptures) =
        if (!isInImplClass) {
          ...
          val (thisFormalCapture :: formalCaptures,
