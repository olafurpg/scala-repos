...
class InterpreterSpec extends AkkaSpec with GraphInterpreterSpecKit {
  ...
  "Interpreter" must {
    ...
    "implement chain of maps correctly" in new OneBoundedSetup[Int](
        Seq(Map((x: Int) ⇒ x + 1, stoppingDecider), Map((x: Int) ⇒
                  x * 2, stoppingDecider), Map((x: Int) ⇒
                  x + 1, stoppingDecider))) {
    ...
    "implement take inside a chain" in new OneBoundedSetup[Int](
        Seq(Filter((x: Int) ⇒ x != 0, stoppingDecider), Take(2), Map((x: Int) ⇒
                  x + 1, stoppingDecider))) {
    ...
    "implement batch (conflate)" in new OneBoundedSetup[Int](
        Batch(1L, ConstantFun.zeroLong, (in: Int) ⇒ in, (agg: Int, x: Int) ⇒
              agg + x)) {
    ...
    "work with jumpback table and completed elements" in new OneBoundedSetup[
        Int](Seq(Map((x: Int) ⇒ x, stoppingDecider), Map((x: Int) ⇒
                  x, stoppingDecider), KeepGoing(), Map((x: Int) ⇒
                  x, stoppingDecider), Map((x: Int) ⇒ x, stoppingDecider))) {
    ...
    "work with pushAndFinish if upstream completes with pushAndFinish and downstream immediately pulls" in new OneBoundedSetup[
        Int](Seq(new PushFinishStage, Fold(0, (x: Int, y: Int) ⇒
                  x + y, stoppingDecider))) {
