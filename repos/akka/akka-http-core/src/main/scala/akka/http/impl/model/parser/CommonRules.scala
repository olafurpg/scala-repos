  ...
  def `transfer-coding` =
    rule(
      ignoreCase("chunked") ~ OWS ~ push(TransferEncodings.chunked) | ignoreCase(
        "gzip") ~ OWS ~ push(TransferEncodings.gzip) | ignoreCase("deflate") ~ OWS ~ push(
        TransferEncodings.deflate) | ignoreCase("compress") ~ OWS ~ push(
        TransferEncodings.compress) | `transfer-extension`)
