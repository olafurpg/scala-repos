...
trait Conductor { this: TestConductorExt ⇒
  ...
  def throttle(node: RoleName,
    ...
    (controller ? Throttle(node, target, direction, rateMBit.toFloat))
      .mapTo(classTag[Done])
  }
  ...
  def exit(node: RoleName, exitValue: Int): Future[Done] = {
    ...
    (controller ? Terminate(node, Right(exitValue)))
      .mapTo(classTag[Done])
      .recover {
...
/**
  ...
  * INTERNAL API.
  */
private[akka] class ConductorHandler(_createTimeout: Timeout,
                                     controller: ActorRef,
                                     log: LoggingAdapter)
  ...
  override def channelConnected(ctx: ChannelHandlerContext,
    ...
    val fsm: ActorRef = Await.result(
      (controller ? Controller.CreateServerFSM(channel))
        .mapTo(classTag[ActorRef]),
      Duration.Inf)
...
/**
  ...
  * INTERNAL API.
  */
private[akka] class Controller(private var initialParticipants: Int,
    ...
    case op: CommandOp ⇒
      op match {
        case Throttle(node, target, direction, rateMBit) ⇒
          val t = nodes(target)
          nodes(node).fsm
            .forward(ToClient(ThrottleMsg(t.addr, direction, rateMBit)))
