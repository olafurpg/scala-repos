...
object Relation extends LilaController {
  ...
  private def renderActions(userId: String, mini: Boolean)(
    ...
    ((ctx.userId ?? { env.api.fetchRelation(_, userId) }))
      .zip(ctx.isAuth ?? { Env.pref.api.followable(userId) })
      .zip(ctx.userId ?? { env.api.fetchBlocks(userId, _) })
      .flatMap {
  ...
  def following(username: String, page: Int) = Open { implicit ctx =>
    Reasonable(page, 20) {
      OptionFuResult(UserRepo.named(username)) { user =>
        RelatedPager(env.api.followingPaginatorAdapter(user.id), page)
          .flatMap { pag =>
  ...
  def followers(username: String, page: Int) = Open { implicit ctx =>
    Reasonable(page, 20) {
      OptionFuResult(UserRepo.named(username)) { user =>
        RelatedPager(env.api.followersPaginatorAdapter(user.id), page)
          .flatMap { pag =>
  ...
  private def RelatedPager(adapter: AdapterLike[String], page: Int)(
      implicit ctx: Context) =
    Paginator(adapter = adapter.mapFutureList(followship),
              currentPage = page,
              maxPerPage = 30)
  ...
  private def followship(userIds: Seq[String])(
    ...
    (UserRepo byIds userIds).flatMap { users =>
      ((ctx.isAuth ?? { Env.pref.api.followableIds(users.map(_.id)) }))
        .flatMap { followables =>
          users.map { u =>
