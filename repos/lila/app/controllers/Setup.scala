...
object Setup extends LilaController with TheftPrevention {
          ...
          .fold(
            ...
            config =>
              ((ctx.userId ?? Env.relation.api.fetchBlocking)).flatMap {
                blocking =>
                  env.processor
                    .hook(config, uid, HTTPRequest.sid(req), blocking)
                    .map(hookResponse)
                    .recover {
          ...
          GameRepo
            ...
            .flatMap { config =>
              ((ctx.userId ?? Env.relation.api.fetchBlocking)).flatMap {
                blocking =>
                  env.processor
                    .hook(config, uid, HTTPRequest.sid(ctx.req), blocking)
                    .map(hookResponse)
                    .recover {
                      case e: IllegalArgumentException =>
                        BadRequest(jsonError(e.getMessage)).as(JSON)
  ...
  private[controllers] def redirectPov(pov: Pov)(implicit ctx: Context) = {
    ...
    if (ctx.isAuth) redir
      ...
      redir.withCookies(
        LilaCookie.cookie(AnonCookie.name,
                          pov.playerId,
