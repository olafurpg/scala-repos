...
object Auth extends LilaController {
  ...
  private def mobileUserOk(u: UserModel): Fu[Result] =
    lila.game.GameRepo.urgentGames(u).map { povs =>
      Ok {
        Env.user.jsonView(u) ++ Json.obj("nowPlaying" -> JsArray(
        ...
        .flatMap { sessionId =>
          negotiate(
            html = Redirect {
              get("referrer")
                .filter(_.nonEmpty)
                .orElse(req.session.get(api.AccessUri))
                .getOrElse(routes.Lobby.home.url)
            }.fuccess,
  ...
  private def doSignup(username: String,
                       password: String,
    ...
    api
      ...
      .map { sessionId =>
        Redirect(routes.User.show(user.username))
          .withCookies(LilaCookie.session("sessionId", sessionId))
      }
      .recoverWith(authRecovery)
  }
