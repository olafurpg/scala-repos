...
object Puzzle extends LilaController {
  ...
  def daily = Open { implicit ctx =>
    OptionFuResult(env.daily().flatMap {
      ...
      negotiate(
        html = ((ctx.me ?? { env.api.attempt.hasPlayed(_, puzzle).map(!_) }))
          .flatMap { asPlay =>
  ...
  private def puzzleJson(puzzle: PuzzleModel)(implicit ctx: Context) =
    (env
      .userInfos(ctx.me))
      .zip(ctx.me ?? { env.api.attempt.hasPlayed(_, puzzle).map(!_) })
      .map {
        case (infos, asPlay) =>
          JsData(puzzle,
                 infos,
  ...
  def difficulty = AuthBody { implicit ctx => me =>
    ...
    env.forms.difficulty.bindFromRequest
      .fold(
  ...
  def attempt(id: PuzzleId) = OpenBody { implicit ctx =>
    ...
    OptionFuResult(env.api.puzzle find id) { puzzle =>
      env.forms.attempt.bindFromRequest
        .fold(
          err => fuccess(BadRequest(errorsAsJson(err))),
          data =>
            ctx.me match {
                ...
                env.finisher(puzzle, me, data).flatMap {
                  case (newAttempt, None) =>
                    (UserRepo byId me.id).map(_ | me).flatMap { me2 =>
                      (env.api.puzzle find id)
                        .zip(env.userInfos(me2.some))
                        .zip(env.api.attempt.hasVoted(me2))
                        .map {
                          case ((p2, infos), voted) =>
                            Ok {
                              JsData(p2 | puzzle,
  ...
  def vote(id: PuzzleId) = AuthBody { implicit ctx => me =>
    ...
    OptionFuResult(env.api.attempt.find(id, me.id)) { attempt =>
      env.forms.vote.bindFromRequest
        .fold(
          err => fuccess(BadRequest(errorsAsJson(err))),
          vote =>
