        ...
        case ((number, lang), topicUserIds) =>
          ...
          (PostRepo findDuplicate post).flatMap {
            ...
            case _ =>
              ...
              } >> $update(categ.withTopic(post)) >>- (indexer ! InsertPost(
                post)) >>
                (env.recent.invalidate inject post) >>- ctx.userId.?? {
                userId =>
                  ...
                  timeline ! Propagate(
                    ...
                    prop =>
                      post.isStaff.fold(
                        ...
                        (prop toFollowersOf userId toUsers topicUserIds)
                          .exceptUser(userId)
                    ))
                }
