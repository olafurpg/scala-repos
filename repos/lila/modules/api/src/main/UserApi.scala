...
private[api] final class UserApi(jsonView: lila.user.JsonView,
  ...
  def list(teamId: String,
    ...
    lila.team.MemberRepo
      .userIdsByTeam(teamId)
      .map(_.take(makeNb(nb)))
  ...
  def one(username: String)(implicit ctx: Context): Fu[Option[JsObject]] =
    UserRepo.named(username).flatMap {
      ...
      case Some(u) =>
        GameRepo
          .mostUrgentGame(u)
          .zip(ctx.me.filter(u !=) ?? { me =>
            crosstableApi.nbGames(me.id, u.id)
          })
          .zip(relationApi.countFollowing(u.id))
          .zip(relationApi.countFollowers(u.id))
          .zip(ctx.isAuth.?? { prefApi.followable(u.id) })
          .zip(ctx.userId
            .?? {
              relationApi.fetchRelation(_, u.id)
            })
          .zip(ctx.userId.?? { relationApi.fetchFollows(u.id, _) })
          .map {
