...
private[api] final class Cli(bus: lila.common.Bus, renderer: ActorSelection)
  ...
  private def processors =
    lila.user.Env.current.cli.process
      .orElse(lila.security.Env.current.cli.process)
      .orElse(lila.wiki.Env.current.cli.process)
      .orElse(lila.i18n.Env.current.cli.process)
      .orElse(lila.game.Env.current.cli.process)
      .orElse(lila.gameSearch.Env.current.cli.process)
      .orElse(lila.teamSearch.Env.current.cli.process)
      .orElse(lila.forum.Env.current.cli.process)
      .orElse(lila.forumSearch.Env.current.cli.process)
      .orElse(lila.team.Env.current.cli.process)
      .orElse(lila.round.Env.current.cli.process)
      .orElse(lila.puzzle.Env.current.cli.process)
      .orElse(lila.tournament.Env.current.cli.process)
      .orElse(lila.explorer.Env.current.cli.process)
      .orElse(lila.fishnet.Env.current.cli.process)
      .orElse(process)
}
