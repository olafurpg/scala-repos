...
private[api] final class RoundApi(
  ...
  def userAnalysisJson(pov: Pov,
    ...
    owner.??(forecastApi loadForDisplay pov).flatMap { fco =>
      jsonView
        .userAnalysisJson(pov, pref, orientation, owner = owner) map withSteps(
          pov,
          none,
          initialFen,
          withOpening = true) _ map withForecast(pov, owner, fco) _
    }

  private def withSteps(pov: Pov,
      ...
      json +
        ("tournament" -> Json
          .obj(
              ...
              "ranks" -> data.tour.isStarted.option(Json
                .obj("white" -> data.whiteRank, "black" -> data.blackRank))
          )
          .noNull)
    }
