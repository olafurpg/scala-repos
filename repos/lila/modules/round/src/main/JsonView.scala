...
final class JsonView(chatApi: lila.chat.ChatApi,
  ...
  def playerJson(pov: Pov,
    ...
    getSocketStatus(pov.game.id)
      .zip(pov.opponent.userId ?? UserRepo.byId)
      .zip(canTakeback(pov.game))
      .zip(getPlayerChat(pov.game, playerUser))
      .map {
  ...
  def watcherJson(pov: Pov,
    ...
    getSocketStatus(pov.game.id)
      .zip(getWatcherChat(pov.game, user))
      .zip(UserRepo
        .pair(pov.player.userId, pov.opponent.userId))
      .map {
  ...
  private def getWatcherChat(
    ...
    forUser ?? { user =>
      (chatApi.userChat find s"${game.id}/w")
        .map(_.forUser(user.some))
        .map(_.some)
    }
