...
object $update {
  ...
  def apply[A: InColl, B: BSONDocumentWriter](selector: JsObject,
                                              update: B,
  ...
  def doc[ID: Writes, A <: Identified[ID]: TubeInColl](
      id: ID)(op: A => JsObject): Funit =
    $find byId id flatten "[db] cannot update missing doc" flatMap { doc =>
      apply($select(id), op(doc))
    }

  def docBson[ID: Writes, A <: Identified[ID]: TubeInColl](
      id: ID)(op: A => BSONDocument): Funit =
    $find byId id flatten "[db] cannot update missing doc" flatMap { doc =>
      apply($select(id), op(doc))
    }
  ...
  def bsonField[ID: Writes, A: InColl](
      id: ID, name: String, value: BSONValue, upsert: Boolean = false): Funit =
    apply($select(id),
