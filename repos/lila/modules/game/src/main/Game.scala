...
case class Game(
  ...
  private def lastMoveTime: Option[Long] =
    castleLastMoveTime.lastMoveTime
      .map {
  ...
  def lastMoveTimeInSeconds: Option[Int] =
    lastMoveTime.map(x => (x / 10).toInt)
  ...
  def update(game: ChessGame,
    ...
    val clockEvent =
      updated.clock.map(Event.Clock.apply).orElse {
        updated.playableCorrespondenceClock.map(
  ...
  private def outoftimeClock(playerLag: Color => Int): Boolean = clock ?? {
    c =>
  ...
  def estimateTotalTime =
    estimateClockTotalTime
      .orElse(correspondenceClock.map(_.estimateTotalTime))
      .getOrElse(1200)
  ...
  def unplayed =
    !bothPlayersHaveMoved && (createdAt.isBefore(Game.unplayedDate))
