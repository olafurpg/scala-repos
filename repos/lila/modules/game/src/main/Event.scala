  ...
  case class Move(orig: Pos,
    ...
    def data =
      MoveOrDrop.data(fen,
                      ...
                      possibleDrops,
                      crazyData) {
        Json.obj("uci" -> s"${orig.key}${dest.key}",
                 "san" -> san,
  ...
  case class Drop(role: chess.Role,
    ...
    def data =
      MoveOrDrop.data(fen,
                      ...
                      possibleDrops,
                      crazyData) {
        Json.obj("role" -> role.name,
                 "uci" -> s"${role.pgn}@${pos.key}",
                 "san" -> san)
      }
  }
  object Drop {
    def apply(drop: ChessDrop,
              situation: Situation,
              state: State,
              clock: Option[Event],
