...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testGroupBy = {
    ...
    db.run {
      ...
      db.run(mark("q6", q6.result))
        .map(_ shouldBe Set((1, 3, 3, 3, 3, 3),
                            (2, 3, 3, 3, 3, 3),
                            (3, 2, 2, 2, 2, 2),
                            (4, 1, 1, 1, 1, 0)))
    }.flatMap { _ =>
      ...
      val q8b = for ((key, group) <- us.map(_ => "x").groupBy(co => co)) yield
        (key, group.map(co => co).max)
      val q8c = for ((key, group) <- us.map(_ => 5).groupBy(co => co)) yield
        (key, group.map(co => co + co).sum)
      val q8d =
  ...
  def testGroup3 = {
    case class Tab(
        col1: String, col2: String, col3: String, col4: Int, col5: Int)
    ...
    for {
      ...
      q1 = Tabs
        ...
        .to[Set]
      _ <- q1.result.map(_ shouldBe Set(("baz", "quux", Some(4)),
                                        ("foo", "quux", Some(3)),
                                        ("foo", "bar", Some(3))))
      q2 = Tabs
        .groupBy(t => ((t.col1, t.col2), t.col3))
        ...
        .to[Set]
      _ <- q2.result.map(_ shouldBe Set(("baz", "quux", Some(4)),
                                        ("foo", "quux", Some(3)),
                                        ("foo", "bar", Some(3))))
      q3 = Tabs
        .groupBy(_.col1)
  ...
  def testMultiMapAggregates = {
    class B(tag: Tag)
        extends Table[(Long, String, String)](tag, "b_multimap") {
