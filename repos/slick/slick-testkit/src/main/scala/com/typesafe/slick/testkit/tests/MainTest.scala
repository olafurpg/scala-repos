...
class MainTest extends AsyncTest[JdbcTestDB] { mainTest =>
  ...
  def test = {
    ...
    materialize(p1.mapResult { case (id, f, l) => User(id, f, l.orNull) }).flatMap {
      allUsers =>
      ...
      val ordersInserts = for {
        u <- allUsers if u.first != "Apu" &&
          u.first != "Snowball"
        i <- 1 to 2
      } yield
