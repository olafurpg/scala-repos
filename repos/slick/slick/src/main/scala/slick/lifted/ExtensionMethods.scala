    ...
    if (seq.isEmpty) om(LiteralColumn(false))
      ...
      om.column(Library.In,
                ...
                ProductNode(ConstArray.from(seq.map(v =>
                              LiteralNode(
                                  implicitly[TypedType[B1]], v, vol = true)))))
...
final class OptionNumericColumnExtensionMethods[B1](val c: Rep[Option[B1]])
    extends AnyVal with NumericColumnExtensionMethods[B1, Option[B1]]
    with OptionExtensionMethods[B1]
...
trait ExtensionMethodConversions {
  ...
  implicit def numericOptionColumnExtensionMethods[B1](
      c: Rep[Option[B1]])(implicit tm: BaseTypedType[B1] with NumericTypedType)
  ...
  implicit def booleanOptionColumnExtensionMethods(c: Rep[Option[Boolean]])
    : BooleanColumnExtensionMethods[Option[Boolean]] =
