...
import java.io.{
  InputStream, ByteArrayInputStream, File, FileInputStream, FileOutputStream
}
import scala.xml._
...
/**
  ...
  */
class InMemFileParamHolder(override val name: String,
...
/**
  ...
  */
class OnDiskFileParamHolder(override val name: String,
                            override val mimeType: String,
                            override val fileName: String,
                            val localFile: File)
    extends FileParamHolder(name, mimeType, fileName) {
  ...
  private def _fixHref(contextPath: String,
    ...
    Text(
        if (fixURL && rewrite.isDefined && !updated.startsWith("mailto:") &&
            ...
            !updated.startsWith("https://") && !updated.startsWith("//") &&
            !updated.startsWith("#"))
