...
trait DB extends Loggable {
  ...
  private def releaseConnectionNamed(name: ConnectionIdentifier,
                                     rollback: Boolean) {
    logger.trace("Request to release %s on thread %s, auto rollback=%s"
    ...
    (info.get(name): @unchecked) match {
      case Some(ConnectionHolder(c, 1, post, manualRollback)) => {
        ...
        try {
          ...
          logger.trace("Invoking %d postTransaction functions. rollback=%s"
  ...
  def appendPostTransaction(name: ConnectionIdentifier,
    ...
    info.get(name) match {
      case Some(ConnectionHolder(c, n, post, rb)) =>
        ...
        logger.trace("Appended postTransaction function on %s, new count=%d"
    ...
    synchronized {
      pool match {
        case Nil if poolSize < tempMaxSize =>
          ...
          ret.foreach(_.setAutoCommit(false))
          poolSize = poolSize + 1
          logger.debug("Created new pool entry. name=%s, poolSize=%d"
