...
object DBLog {
  ...
  private def proxyPreparedStatement(stmt: => PreparedStatement,
                                     query: String) = {
    try {
      Proxy
        .newProxyInstance(
    ...
    protected def chain(method: Method, args: Array[Object]): Object =
      try {
        ...
        case nsme: NoSuchMethodException =>
          logger.warn("Could not locate method %s for %s : %s"
    ...
    override def invoke(proxy: Object,
      ...
      method.getName match {
        ...
        case "setArray" | "setBigDecimal" | "setBoolean" | "setByte" |
            ...
            "setNString" | "setRef" | "setRowId" | "setShort" |
            "setSQLXML" => {
        ...
        case "setBlob" => {
          paramMap +=
          args(0).asInstanceOf[Int] -> "(Blob : %s (%d bytes))".format(args(1),
                                                                       args(2))
          chain(method, args)
        }
        ...
        case "setClob" => {
          paramMap +=
          args(0).asInstanceOf[Int] -> "(Clob : %s (%d bytes))".format(args(1),
                                                                       args(2))
          chain(method, args)
        }
