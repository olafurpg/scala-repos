...
trait DBLog
  ...
  protected def logStatement[T](description: T => String)(f: => T): T =
    Helpers.calcTime(f) match
      case (duration, result) =>
        executedStatements ::= DBStatementEntry(description(result), duration); result
  ...
  protected def logMeta[T](description: T => String)(f: => T): T =
    Helpers.calcTime(f) match
      case (duration, result) =>
        executedStatements ::= DBMetaEntry(description(result), duration); result
...
object DBLog
  ...
  def prepareStatement(
      conn: Connection, query: String, autoKeys: Array[String]) =
    proxyPreparedStatement(conn.prepareStatement(query, autoKeys), query)

  private def proxyPreparedStatement(
      stmt: => PreparedStatement, query: String) =
    try
      Proxy
        .newProxyInstance(
      ...
      case sqle: SQLException =>
        throw new SQLException(
            "Error preparing statement: \"%s\"".format(query), sqle)
  ...
  sealed private[DBLog] class LoggedStatementHandler(underlying: Statement)
      extends InvocationHandler with DBLog with Loggable
    ...
    def invoke(proxy: Object, method: Method, args: Array[Object]): Object =
      method.getName match
        ...
        case "execute" =>
            logStatement( ret: Object =>
              "Exec \"%s\", Auto-gen keys for columns %s".format(
                  args(0), args(1).asInstanceOf[Array[_]].mkString(", "), ret)
            )
              chain(method, args)
        case "executeBatch" =>
        ...
        case "setFetchDirection" =>
            logMeta("Set fetch direction = " + StatementConstantDescriptions
    ...
    protected def chain(method: Method, args: Array[Object]): Object =
      try
        val m = representative.getMethod(
            method.getName, method.getParameterTypes: _*)
        ...
        case nsme: NoSuchMethodException =>
          logger.warn("Could not locate method %s for %s : %s".format(
                  method.getName, underlyingClassname, nsme.getMessage))
          throw nsme
  ...
  sealed private[DBLog] class LoggedPreparedStatementHandler(
      stmt: String, underlying: PreparedStatement)
    ...
    override def invoke(
        proxy: Object, method: Method, args: Array[Object]): Object =
      method.getName match
        ...
        case "setBinaryStream" if args.length == 2 =>
            paramMap +=
              args(0).asInstanceOf[Int] -> "(Binary Stream: %s)".format(
                args(1))
            chain(method, args)
        ...
        case "setNCharacterStream" if args.length == 2 =>
            paramMap +=
              args(0).asInstanceOf[Int] -> "(NChar Stream : %s)".format(
                args(1))
            chain(method, args)
