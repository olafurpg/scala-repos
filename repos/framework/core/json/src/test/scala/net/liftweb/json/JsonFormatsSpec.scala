...
object JsonFormatsSpec extends Specification with TypeHintExamples {
  ...
  "hintsFor across composite formats" in {
    ((formats.typeHints.hintFor(classOf[Fish]) mustEqual (hintsForFish)))
      .and(formats.typeHints.hintFor(classOf[Dog]) mustEqual (hintsForDog))
      .and(
  ...
  "classFor across composite formats" in {
    ((formats.typeHints.classFor(hintsForFish) mustEqual
      ...
      .and(formats.typeHints.classFor(hintsForDog) mustEqual
        (ShortTypeHintExamples.formats.typeHints.classFor(hintsForDog)))
      .and(formats.typeHints.classFor(hintsForAnimal) mustEqual
